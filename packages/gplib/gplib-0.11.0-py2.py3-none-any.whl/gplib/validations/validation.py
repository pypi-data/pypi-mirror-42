...
class Validation(object):
    ...
    def normalize(data):
        ...
        """
        return {
            key: Validation.get_transform_matrices(value)
            for key, value in data.items()
        }

    @staticmethod
    def get_transform_matrices(matrix):
        """
        ...
        scale_inv = np.diag(np.max(matrix, axis=0) - min_x)
    ...
    @staticmethod
    def scale(data, transform, inv=False):
        """
        ...
        return {
            key: np.dot(
                np.hstack((value, np.ones((len(value), 1)))),
                transform[key][1 if inv else 0]
            )
            for key, value in data.items()
        }
