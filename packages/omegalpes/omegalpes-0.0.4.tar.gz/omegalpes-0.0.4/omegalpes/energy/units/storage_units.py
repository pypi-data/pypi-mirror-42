...
class StorageUnit(EnergyUnit):
    ...
    **Attributes**
        ...
        * e_f_min (ExternalConstraint) : e_f value is constrained above
        soc_min*capacity
        e_f_max (ExternalConstraint) : e_f value is constrained below
        soc_max*capacity
        * set_e_f (ExternalConstraint) : when e_f is given, it is set in the
         same way the energy is, but after the last time step
         * calc_e_f (ExternalConstraint) : when e_f is not given,
    ...
    def __init__(self, time, name='StU1', pc_min=1e-5, pc_max=1e+5,
        ...
        EnergyUnit.__init__(self, time, name, flow_direction='in', p=None,
                            p_min=-pd_max, e_min=-1e+5,
        ...
        if e_f is not None:
            ...
            if isinstance(soc_min, (int, float)):
                self.e_f_min = ExternalConstraint(
            ...
            if isinstance(soc_max, (int, float)):
                self.e_f_max = ExternalConstraint(
                    exp='{0}_e_f <= {1} * {0}_capacity'.format(self.name,
                                                               soc_max),
            ...
            self.set_e_f = ExternalConstraint \
                (name='set_e_f',
            ...
            # e_f calculation
            self.calc_e_f = ExternalConstraint \
                (name='calc_e_f',
                 exp='{0}_e_f-{0}_e[{1}] == {2}*({0}_pc[{1}]*{3}-'
                     '{0}_pd[{1}]*1/{4}-{5}*{0}_e[{1}]-{6}*{0}_capacity)'
                 .format(self.name, time.I[-1], time.DT, eff_c,
...
class ThermoclineStorage(StorageUnit):
    ...
    **Attributes**
    ...
    def __init__(self, time, name='Thermocline', pc_min=1e-5, pc_max=1e+5,
