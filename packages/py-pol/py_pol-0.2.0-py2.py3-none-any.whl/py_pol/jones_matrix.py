...
class Jones_matrix(object):
    ...
    def get(self):
        ...
        Returns:
            (numpy.matrix): self.M, Jones matrixS
        """
    ...
    def reciprocal(self, keep=True, returns_matrix=False):
        ...
        """
        M2 = Jones_matrix()
        M2.from_matrix(X_jones * M2.M.T * X_jones)
        if keep:
            self.M = M2.M
        if returns_matrix:
            return M2.M
    ...
    def retarder_material(self,
        ...
        .. math::  phase=2*\pi \frac{(ne-no)*d}{/lambda}.
    ...
    @_actualize_
    def quarter_wave(self, angle=0 * degrees, global_phase=0):
    ...
    @_actualize_
    def half_wave(self, angle=0, global_phase=0):
