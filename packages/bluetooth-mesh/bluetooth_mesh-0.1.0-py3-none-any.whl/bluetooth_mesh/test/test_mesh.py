...
def health_current_status_message():
    ...
    return AccessMessage(payload, ttl=0x03, seq=0x000007, src=0x1201, dst=0xffff)


@fixture
def config_appkey_status_message():
    payload = bytes.fromhex('800300563412')
    return AccessMessage(payload, ttl=0x0b, seq=0x000006, src=0x1201, dst=0x0003)


@fixture
def config_appkey_add_message():
    payload = bytes.fromhex('0056341263964771734fbd76e3b40519d1d94a48')
    return AccessMessage(payload, ttl=0x04, seq=0x3129ab, src=0x0003, dst=0x1201)
...
def test_network_nonce(config_appkey_status_message):
    assert config_appkey_status_message.network_nonce(iv_index=0x12345678) == bytes.fromhex('000b0000061201000012345678')


def test_device_nonce(config_appkey_status_message):
    assert config_appkey_status_message.device_nonce(iv_index=0x12345678) == bytes.fromhex('02000000061201000312345678')


def test_application_nonce(health_current_status_message):
    assert health_current_status_message.application_nonce(iv_index=0x12345678) == bytes.fromhex('01000000071201ffff12345678')


def test_device_transport_pdu(config_appkey_status_message, dev_key):
    (seq, transport_pdu), = \
        config_appkey_status_message.transport_pdu(dev_key, iv_index=0x12345678)

    assert seq == config_appkey_status_message.seq
...
def test_application_transport_pdu(health_current_status_message, app_key):
    (seq, transport_pdu), = \
        health_current_status_message.transport_pdu(app_key, iv_index=0x12345678)

    assert seq == health_current_status_message.seq
...
def test_application_transport_pdu_segmented(config_appkey_add_message, dev_key):
    (seq0, transport_pdu0), (seq1, transport_pdu1) = \
        config_appkey_add_message.transport_pdu(dev_key, iv_index=0x12345678)

    assert seq0 == config_appkey_add_message.seq
    assert transport_pdu0 == bytes.fromhex('8026ac01ee9dddfd2169326d23f3afdf')

    assert seq1 == config_appkey_add_message.seq + 1
...
def test_application_network_pdu(health_current_status_message, app_key, net_key):
    ...
    (seq, network_pdu), = \
        network_message.network_pdu(app_key, net_key, iv_index=0x12345678)
