...
def downloadModel(modelName,urlModel,urlWeights):
    ...
    if not os.path.exists(pathModel + os.sep + modelName+'.json'):
        if not os.path.exists(pathModel):
            os.makedirs(pathModel)
        r = requests.get(urlModel)
        with open(pathModel + os.sep + modelName+'.json', 'wb') as f:
            f.write(r.content)
    if not os.path.exists(pathWeights + os.sep + modelName+'.h5'):
        if not os.path.exists(pathWeights):
            os.makedirs(pathWeights)
        r = requests.get(urlWeights)
        with open(pathWeights + os.sep + modelName+'.h5', 'wb') as f:
            f.write(r.content)
