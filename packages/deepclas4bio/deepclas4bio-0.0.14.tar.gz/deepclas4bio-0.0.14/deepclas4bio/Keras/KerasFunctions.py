...
def resnetisickerasload():
    ...
    if not os.path.exists(pathModel+os.sep+'ResNetISIC.json'):
        ...
        wget.download('https://unirioja-my.sharepoint.com/:u:/g/personal/adines_unirioja_es/EVNjE2vxXAFJvBcMzke1iHYBVL0GGTCkX3WKCrVMzCLrdA?download=1',out=pathModel+os.sep+'ResNetISIC.json')
    if not os.path.exists(pathWeights+ os.sep + 'ResNetISIC.h5'):
        if not os.path.exists(pathWeights):
            os.makedirs(pathWeights)
        wget.download('https://unirioja-my.sharepoint.com/:u:/g/personal/adines_unirioja_es/Eb61Ld1zqdFJlMxANXS1ANABjRMZ-FZnLuadeGn-2bFnCA?download=1', out=pathWeights+os.sep+'ResNetISIC.h5')
