...
class Bridge(object):
    ...
    def encryptionNamespaceGet(self, name, callback=None, errback=None):
        ...
        if callback is not None:
            ...
            kwargs['reply_handler'] = callback
            kwargs['error_handler'] = error_handler
        return unicode(self.db_core_iface.encryptionNamespaceGet(name, **kwargs))
    ...
    def encryptionTrustUIGet(self, to_jid, namespace, profile_key, callback=None, errback=None):
        if callback is None:
            ...
            if errback is None:
                errback = log.error
            error_handler = lambda err:errback(dbus_to_bridge_exception(err))
        return unicode(self.db_core_iface.encryptionTrustUIGet(to_jid, namespace, profile_key, timeout=const_TIMEOUT, reply_handler=callback, error_handler=error_handler))
