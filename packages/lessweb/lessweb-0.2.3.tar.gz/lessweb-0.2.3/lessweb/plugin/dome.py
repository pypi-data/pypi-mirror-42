...
def watch(srcpath, finish_flag, distpath='static/js'):
    """
    example:
        import threading
        finish_flag = []
        thread = threading.Thread(target=watch('pages', finish_flag))
        thread.start()
        ......
...
class Kit:
    ...
    @staticmethod
    def request(method, url, data=None, json=False, headers=None, form=False, onsuccess=None, onerror=None):
        contentType = False
        if json:
