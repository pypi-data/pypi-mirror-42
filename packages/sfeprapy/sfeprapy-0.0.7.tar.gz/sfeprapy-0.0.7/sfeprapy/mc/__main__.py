...
def plot_figures(list_path_out_csv, plot_teq_xlim, reliability_target, path_work, figure_name='t_eq.png', **surplus_args):
...
def run():
    ...
    if len(list_path_input_json) == 0:
        ...
        if len(list_path_out_csv_all) >= 1:
            list_pd_out_csv = []
            for path_out_csv in list_path_out_csv_all:
                # print(path_out_csv)
                list_pd_out_csv.append(pd.read_csv(path_out_csv, index_col='INDEX'))
    ...
    for i, dict_mc_params in enumerate(list_dict_input_files):
        ...
        if 'path_mc_params_csv' in dict_mc_params:  # Read MC parameters. If USER provided MC parameters in a *.csv file
            ...
            # only sample the defined probability for total number of simulation
            if 'simulations_multiplier' in dict_mc_params:
                dict_mc_params['simulations'] = int(dict_mc_params['simulations'] * dict_mc_params['simulations_multiplier']) + 1
