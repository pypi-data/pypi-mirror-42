...
def list_of_pairs_format_csv(csv_file):
    ...
    with open(csv_file, 'r') as csv_object:
        ...
        for body_line in body:
            body_fields.append(split_csv_line(body_line))

        for index, field in enumerate(header_fields):
            print(field + ":\t" + str([body_fields[j][index] for j in
                  range(len(body_fields))]))
        ...
        for header_field in header_fields:
            ...
            for file_header_field in file_header_fields:
                if header_field == file_header_field:
                    position = file_header_fields.index(file_header_field)
                    new_csv_line +=\
                        format_RFC4180(file_body_fields[position]) + ','
                    match = True
