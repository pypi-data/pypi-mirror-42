...
from .doc import paths, tags as doc_tags, Schema
...
def build_openapi_spec(app, _):
    ...
    hide_excluded = not show_excluded
...
def _build_openapi_spec(
    ...
    for _uri, _route in app.router.routes_all.items():
        ...
        uri_parsed = _uri
        parameters = OrderedDict()  # type: OrderedDict[str, Parameter]
    ...
    if not show_unused_tags:
        ...
        in_use_tags = []
        for tag in _v3_tags:
            for path in _v3_paths:
                for op_name in Operation.OPERATION_NAMES:
                    op = getattr(path, op_name)  # type: Operation
                    if op:
    ...
    if servers:
        if not isinstance(servers, list):
            raise AssertionError(
                "You app.config's `OPENAPI_SERVERS` is not a list (of `Server`): {}".format(
                    type(servers)
                )
            )
