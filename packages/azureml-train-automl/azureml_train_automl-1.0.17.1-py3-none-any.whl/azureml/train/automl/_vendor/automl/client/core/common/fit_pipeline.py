...
def fit_pipeline(automl_pipeline: AutoMLPipeline,
    ...
    logger.info(
        "[ParentRunId:{}] X datatype is {}, shape is {}, datasize is {}.".format(
            automl_pipeline.run_id, type(X), X.shape, memory_utilities.get_data_memory_size(X)
        )
    )
    logger.info(
        "[ParentRunId:{}] y datatype is {}, shape is {}, datasize is {}.".format(
            automl_pipeline.run_id, type(y), y.shape, memory_utilities.get_data_memory_size(y)
        )
    )
    if X_valid is not None:
        logger.info(
            "[ParentRunId:{}] X_valid datatype is {}, shape is {}, datasize is {}.".format(
                automl_pipeline.run_id, type(X_valid), X_valid.shape, memory_utilities.get_data_memory_size(X_valid)
            )
        )
    if y_valid is not None:
        logger.info(
            "[ParentRunId:{}] y_valid datatype is {}, shape is {}, datasize is {}.".format(
                automl_pipeline.run_id, type(y_valid), y_valid.shape, memory_utilities.get_data_memory_size(y_valid)
            )
        )
    ...
    try:
        ...
        with logging_utilities.log_activity(logger,
                                            ...
                                            custom_dimensions={'run_id': automl_pipeline.run_id}):
                ...
                if automl_settings.model_explainability:
                    telemetry_logger.send_usage_telemetry_log(
                        prefix_message="[RunId:{}][Start model explain in fit pipeline]".format(
                            automl_pipeline.run_id),
                        is_sending_telemetry=automl_settings.send_telemetry)
                    _explain_model_in_fit(run, fit_output.fitted_pipeline,
