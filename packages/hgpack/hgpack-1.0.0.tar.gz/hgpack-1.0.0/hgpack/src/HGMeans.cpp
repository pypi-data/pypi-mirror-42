...
void HGMeans::Go(char* filename, int size_population, int it, bool evaluation, const std::vector<int>& m) {
    
    string filenme_str(filename);

    InputValidator command(filenme_str, size_population, it, evaluation, m);
...
int main(int argc, char** argv) {
    ...
    HGMeans hgmeans;
    hgmeans.Go(argv[1], command.GetPiMin(), command.GetMaxIt(), command.ExternalEval(), command.GetNbClusters());
    return 0;
}