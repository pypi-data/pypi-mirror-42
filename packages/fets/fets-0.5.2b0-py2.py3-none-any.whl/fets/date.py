...
class TSNumericalToDate(BaseEstimator, TransformerMixin):
    ...
    def transform(self, input_s):
        ...
        year_res = input_s.dropna()[(input_s.values >= 1e3) & (input_s.values < 1e4)].apply(lambda x: x*10000+101)
        # Transforms a month resolution date to day resolution on first of the given month
        month_res = input_s.dropna()[(input_s.values >= 1e5) & (input_s.values < 1e6)].apply(lambda x: x*100+1)
        # Don't Transforms day resolution dates, just select
        day_res   = input_s.dropna()[(input_s.values >= 1e7) & (input_s.values < 1e8)]
