...
if __name__ == '__main__':
    ...
    # test_attr_provider(synth)
    # print(attr.asdict(synth.inferred_attribute()))

    inferred_attribute = synth.inferred_attribute("profile.interestIn.company", synth.weighted_value())
    declared_attribute = synth.declared_attribute("profile.favorite_color", synth.string_value())
    observed_attribute = synth.observed_attribute("profile.likes.insights.relatedToMicrosoft", synth.counter_value())
    assigned_attribute = synth.assigned_attribute("profile.types", synth.profile_type_value())

    attributes = {
        inferred_attribute.context: dict(inferred_attribute),
        declared_attribute.context: dict(declared_attribute),
        observed_attribute.context: dict(observed_attribute),
        assigned_attribute.context: dict(assigned_attribute),
    }

    # filepath = os.path.expanduser("~/Workspace/git/cortex-graph/test/attribute-value-samples/attribute-values.js")
    with open("{}/../../samples/good-attributes.js".format(os.path.dirname(__file__)), "w") as fh:
        fh.write("const GoodAttributes = ")
        json.dump(attributes, fh, indent=4)
        fh.write(";\n")
        fh.write("module.exports = { GoodAttributeValues } ;")

    for x in [inferred_attribute, declared_attribute, observed_attribute, assigned_attribute]:
        with open("{}/../../samples/attributes/{}.json".format(os.path.dirname(__file__), get_var_names(x)[0]), "w") as fh:
            # json.dump(x.external_representation(), fh, indent=4)
            json.dump(dict(x), fh, indent=4)
