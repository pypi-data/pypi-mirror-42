...
def make_pipeline(pipeline_path=''):
    ...
    class SerialPhase(ph.SerialPhase):
        ...
        def pass_priors(self, previous_results):
            ...
            previous_total_density = previous_results[-1].constant.serial_species[0].density

            self.serial_species[0].density = prior.UniformPrior(lower_limit=0.0, upper_limit=previous_total_density)
            self.serial_species[1].density = prior.UniformPrior(lower_limit=0.0, upper_limit=previous_total_density)
            self.serial_species[2].density = prior.UniformPrior(lower_limit=0.0, upper_limit=previous_total_density)
            self.serial_species[0].lifetime = prior.UniformPrior(lower_limit=0.0, upper_limit=30.0)
            self.serial_species[1].lifetime = prior.UniformPrior(lower_limit=0.0, upper_limit=30.0)
            self.serial_species[2].lifetime = prior.UniformPrior(lower_limit=0.0, upper_limit=30.0)
    ...
    class SerialHyperFixedPhase(ph.SerialHyperPhase):
        ...
        def pass_priors(self, previous_results):
            ...
            self.hyper_noise_scaler_serial_trails = previous_results[2].constant.hyper_noise_scaler_serial_trails
