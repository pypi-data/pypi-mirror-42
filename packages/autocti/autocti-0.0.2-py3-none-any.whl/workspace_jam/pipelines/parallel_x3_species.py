...
def make_pipeline(pipeline_path=''):
    ...
    class ParallelPhase(ph.ParallelPhase):
        ...
        def pass_priors(self, previous_results):
            ...
            previous_total_density = previous_results[-1].constant.parallel_species[0].density

            self.parallel_species[0].density = prior.UniformPrior(lower_limit=0.0, upper_limit=previous_total_density)
            self.parallel_species[1].density = prior.UniformPrior(lower_limit=0.0, upper_limit=previous_total_density)
            self.parallel_species[2].density = prior.UniformPrior(lower_limit=0.0, upper_limit=previous_total_density)
            self.parallel_species[0].lifetime = prior.UniformPrior(lower_limit=0.0, upper_limit=30.0)
            self.parallel_species[1].lifetime = prior.UniformPrior(lower_limit=0.0, upper_limit=30.0)
            self.parallel_species[2].lifetime = prior.UniformPrior(lower_limit=0.0, upper_limit=30.0)
    ...
    phase2 = ParallelPhase(parallel_species=[prior_model.PriorModel(arctic_params.Species),
                         ...
                         phase_name=pipeline_path + '/phase_x3_species_initialize')
    ...
    class ParallelHyperFixedPhase(ph.ParallelHyperPhase):
        ...
        def pass_priors(self, previous_results):
            ...
            self.hyper_noise_scaler_ci_regions = previous_results[2].constant.hyper_noise_scaler_ci_regions
            self.hyper_noise_scaler_parallel_trails = previous_results[2].constant.hyper_noise_scaler_parallel_trails
