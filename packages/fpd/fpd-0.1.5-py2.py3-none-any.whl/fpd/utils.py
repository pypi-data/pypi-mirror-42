...
def snr_single_image(image, s=1, w=2, order=1, hw=5, plot=True):
    '''
    Single image SNR calculation using auto-correlation and
    polynomial extrapolation, based on [1].
    ...
    w : int
        Width of region used in the polynomial fit.
    order : int
    ...
    snr_res: namedtuple
        Named tuple of `snrt snry snrx st nt sy ny sx nx`, where:
            snrX is the SNR, sX is the signal, and nX is the noise.
    ...
    im_pad = np.pad(image, hw, mode='median')#, constant_values=image.mean())
    # auto-correlation
