...
from probator.exceptions import ProbatorFatalError
        ...
        while self._tries > 0:
            try:
                ...
                if rex == 'OptInRequired':
                    ...
                    if worker.__class__.__name__ == 'AWSRegionCollector':
                ...
                elif rex == 'AccessDenied':
                    worker = args[0]

                    self.log.warning(f'Access denied for {worker.account.account_name}. Ensure role trust and permissions are correct')
                    raise ProbatorFatalError('Access Denied') from ex
