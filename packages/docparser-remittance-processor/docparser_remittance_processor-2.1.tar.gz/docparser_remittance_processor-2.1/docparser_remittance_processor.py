...
for remittance in remittance_data:
    tag = remittance.get("tagger", "Missing")
    if tag == "aub_group":
        (Path.cwd() / "test_data" / "sample_remittances.json").write_text(
            json.dumps(remittance)
        )
...
def process_standard_rows(header, rows, columns, money_columns):
    transformed_rows = []
    for row in rows:
        for item, value in row.items():
            if item in money_columns:
                print(item, money(value))
...
process_standard_rows(
    test_header, test_remittance["rows"], portrait_columns, money_columns_all
)
