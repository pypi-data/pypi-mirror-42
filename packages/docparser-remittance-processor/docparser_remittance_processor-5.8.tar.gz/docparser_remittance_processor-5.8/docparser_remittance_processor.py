...
def save_daily_files(all_dfs, column, insurers_folder):
    insurers_folder.mkdir(parents=True, exist_ok=True)
    groups = create_files_grouped_by_column(all_dfs, column)
    for group in groups:
        display(group[0])
        excel_filename = (email_folder / "insurers" / group[0]).with_suffix(".xlsx")
...
def make_zip_file(df_list):
    zipfile_name = (
        f"{df_list[0]['insurer_short_name']}_{df_list[0]['remittance_date']}.zip"
    )
    with zipfile.ZipFile(zipfile_name, "w") as new_zip:
        excel_file = df_list[0]["excel_file"]
        new_zip.write(excel_file, arcname=f"{df_list[0]['insurer_short_name']}.xlsx")
        for row in df_list:
            print(row)
...
df_list = get_all_daily_insurer_files(email_folder)
df_list
