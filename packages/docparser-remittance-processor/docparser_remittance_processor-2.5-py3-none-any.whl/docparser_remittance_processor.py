...
def process_remittance(
    header, rows, columns, money_columns, money_columns_add, money_columns_subtract
):
    transformed_rows = []
    add = 0
    substract = 0
    for row in rows:
        for item, value in row.items():
            if item in money_columns:
                ...
                if item in money_columns_add:
                    add += value
                elif item in money_columns_subtract:
                    subtract += value
            row["add_total"] = add
            row["subtract_total"] = subtract
        final = {**header, **row}
