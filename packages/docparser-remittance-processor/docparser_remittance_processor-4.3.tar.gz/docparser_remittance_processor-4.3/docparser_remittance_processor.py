...
test_remittance = json.loads(
    (Path.cwd() / "test_data" / "ins_set_statement.json").read_text()
)
# display(test_remittance)
header = make_header(test_remittance, strings_to_delete)
# header['broker'] = None
display(header)
...
def normalise_header(
    ...
    header["broker"] = replace_blanks(header["broker"], broker_mapping, test_remittance)
