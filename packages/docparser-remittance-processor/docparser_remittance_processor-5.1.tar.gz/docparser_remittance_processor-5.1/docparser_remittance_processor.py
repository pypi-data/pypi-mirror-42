...
def make_hyperlink(folder):
    url = (
        f'=HYPERLINK("..\\..\\remittances\\{folder.parent.name}\\{folder.name}", "PDF")'
    )
    return url
...
all_dfs.to_excel(email_folder / f"remittances_{email_arrival_date}.xlsx", index=False)


# In[62]:


failed_validation = all_dfs[
    (all_dfs.total_difference < -1) | (all_dfs.total_difference > 1)
]
if failed_validation.shape[0] > 0:
    failed_validation.to_excel(
        email_folder / f"failed_validation_{email_arrival_date}.xlsx", index=False
    )
...
tag_df = pd.DataFrame(missing_tagger)
if tag_df.shape[0] > 0:
    tag_df.to_excel(
        email_folder / f"missing_map_{email_arrival_date}.xlsx", index=False
    )
