...
def set_header_field(row, lines, field_name, line_no, position):
    row[field_name] = lines[line_no, position]
    return row
...
def process_remittance(header, rows, return_columns, money_columns, row_builder={}):
    transformed_rows = []
    add = 0
    subtract = 0
    for row in rows:
