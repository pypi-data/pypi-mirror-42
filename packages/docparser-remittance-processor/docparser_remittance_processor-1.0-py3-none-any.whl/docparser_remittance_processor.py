...
def make_header(remittance):
    ...
    header["insurer"] = header["insurer"].lower().replace("ltd", "")
    header["insurer"] = header["insurer"].lower().replace("limited", "")
    header["insurer"] = header["insurer"].lower().replace("pty", "")
    header["insurer"] = header["insurer"].lower().replace("p/l", "")
    header["insurer"] = header["insurer"].lower().replace("&", "and")
    header["insurer"] = " ".join(header["insurer"].split())
    ...
    header["doc_format"] = remittance.get("tagger", "missing_tagger")
    header["filename"] = remittance.get("file_name", "missing_filename")
    return header
...
def process_aub_group_rows(header, rows, columns="full"):
    fee_mapping = {
        "P": "premium_ex_gst",
        "U": "underwriting_fee",
        "G": "gst_premium",
...
def process_portrait_rows(header, rows, columns="full"):
    transformed_rows = []
    for row in rows:
        final = {**header, **row}
        transformed_rows.append(final)
...
def process_brokerplus_rows(header, rows, columns="full"):
    transformed_rows = []
    for row in rows:
        final = {**header, **row}
        transformed_rows.append(final)
...
def process_brokers_advantage_rows(header, rows, columns="full"):
    transformed_rows = []
    for row in rows:
        final = {**header, **row}
        transformed_rows.append(final)
