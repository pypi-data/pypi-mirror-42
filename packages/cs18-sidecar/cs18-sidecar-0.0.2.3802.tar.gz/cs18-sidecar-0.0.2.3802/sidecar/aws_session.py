        ...
        if not self._stack_name:
            if self.aws_config.production_id:
                response = self.cf_client.describe_stack_resource(
                    StackName=f'production-{self.aws_config.production_id}',
                    LogicalResourceId=f'sandbox{self.aws_config.sandbox_id}')
                self._stack_name = response['StackResourceDetail']['PhysicalResourceId']
            else:
