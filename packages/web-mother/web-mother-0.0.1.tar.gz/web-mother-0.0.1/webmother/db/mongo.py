...
def init():
    ...
    _mongo_client = pymongo.MongoClient(host=mongo_cfg['host'],
                                        port=mongo_cfg['port'],
                                        username=mongo_cfg['user'],
                                        password=mongo_cfg['pwd'],
                                        authSource=mongo_cfg['db'])
    _db = _mongo_client[mongo_cfg['db']]

    # collections
    catalog = _db.catalog
    product = _db.product
    org = _db.org
    org2user = _db.org2user
    catalog2org = _db.catalog2org
...
def start_session():
    return _mongo_client.start_session()
