...
def add(cid, prod_meta, *auth_args):
    ...
    # 授权检查
    Passport().verify(*auth_args).operable(c.get('node'), 'element_create')
    # END
    ...
    return _read(result.inserted_id)
...
def read(cid, pid, *auth_args):
    ...
    # 授权检查
    Passport().verify(*auth_args).operable(c.get('node'), 'element_read')
    # END
...
def update(cid, pid, prod_meta, *auth_args):
    ...
    # 授权检查
    Passport().verify(*auth_args).operable(c.get('node'), 'element_update')
    # END
...
def change_status(cid, pid, action, *auth_args):
    ...
    # 授权检查
    Passport().verify(*auth_args).operable(c.get('node'), f'element_{action}')
    # END
...
def move(cid, pid, cid_to, *auth_args):
    ...
    :return:
    """
    c = ctrl_catalog._read(cid, raw=True)
    c_to = ctrl_catalog._read(cid_to, raw=True)

    if cid == cid_to:
        raise ErrException(ERROR.E40000, extra='the same node, not need move')

    # 授权检查
    Passport().verify(*auth_args).operable(c['node'], 'element_remove')
    Passport().verify(*auth_args).operable(c_to['node'], 'element_create')
    # END
