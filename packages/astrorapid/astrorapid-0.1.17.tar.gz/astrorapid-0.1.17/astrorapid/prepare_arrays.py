...
class PrepareArrays(object):
    ...
    def update_X_nointerp(self, X, i, data, tinterp, len_t, objid, contextual_info, otherinfo):
        pb_wave = {'u': 357, 'g': 477, 'r': 621, 'i': 754, 'z': 871, 'Y': 1004}
        ...
        for pb in self.passbands:
            data[pb, 'passband'] = pb_wave[pb]
        data_stackedpb = pd.concat([data[pb].dropna() for pb in self.passbands])
        data_stackedpb = data_stackedpb.sort_values('time')
        data_stackedpb['deltat'] = np.insert(np.diff(data_stackedpb['time']), 0, 0)

        len_t = data_stackedpb.shape[0]
        if len_t >= self.nobs:
            data_stackedpb = data_stackedpb[data_stackedpb['time'] >= self.mintime]
            len_t = data_stackedpb.shape[0]
            if len_t >= self.nobs:
                tinterp = data_stackedpb[data_stackedpb['time'] <= self.maxtime]
                len_t = data_stackedpb.shape[0]

        X[i][0][0:len_t] = data_stackedpb['deltat']
        X[i][1][0:len_t] = data_stackedpb['flux']
        X[i][2][0:len_t] = data_stackedpb['passband']
        # X[i][3][0:len_t] = data_stackedpb['fluxErr']
        lr = 2  # last row

        # Add contextual information
        for jj, c_idx in enumerate(contextual_info, 1):
            try:
                X[i][lr + jj][0:len_t] = otherinfo[c_idx] * np.ones(len_t)
            except Exception as e:
                X[i][lr + jj][0:len_t] = otherinfo[c_idx].values[0] * np.ones(len_t)
