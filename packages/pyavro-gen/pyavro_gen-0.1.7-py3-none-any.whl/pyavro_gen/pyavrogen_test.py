...
from confluent_kafka.avro.serializer import AvroSerializer, TopicRecordNameStrategy, \
    AvroDeserializer
...
def do_test_generated_classes(  # pylint: disable=R0914
    ...
    for test_class_item in test_module.testing_classes.test_classes:  # type: ignore
        ...
        for i in range(number_of_cycles):
            ...
            rec = GenericAvroRecord(
                avro.loads(instance._schema),  # pylint: disable=W0212
                asdict(instance)
            )
            rec_bytes = value_serializer('mytopic', rec)

            des_dict = value_deserializer('mytopic', rec_bytes)
            des_instance = klass(**des_dict)

            try:
                assert are_almost_equal(
                    instance, des_instance, max_abs_ratio_diff=1.000001, max_abs_diff=-1)
            except AssertionError:
                print("expected:", instance)
                print("actual:  ", des_instance)
                print("des dict:", des_dict)
                clean_path()
                raise
