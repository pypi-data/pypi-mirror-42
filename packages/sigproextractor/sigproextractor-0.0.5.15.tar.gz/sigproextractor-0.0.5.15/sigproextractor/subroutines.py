...
def export_information(loopResults, mutation_context, output, index, colnames):
    ...
    exposures.to_csv(subdirectory+"/sig_activities.txt", "\t", index_label=[exposures.columns.name]) 
    ...
    exposureAvg = pd.read_csv(subdirectory+"/sig_activities.txt", sep="\t", index_col=0)
...
def make_final_solution(processAvg, allgenomes, allsigids, layer_directory, m, index, allcolnames):
    ...
    exposures = exposures.rename_axis("Samples", axis="columns")
    exposures.to_csv(layer_directory+"/sig_activities.txt", "\t", index_label=[exposures.columns.name]) 
    ...
    exposureAvg = pd.read_csv(layer_directory+"/sig_activities.txt", sep="\t", index_col=0)
    probability = probabilities(processAvg, exposureAvg)
    probability=probability.set_index("Sample")
