...
class PositionService:
    ...
    def __init__(self,
                 frequency: float,
                 g: float,
                 decawave_std_dev: float,
                 odometer_std_dev: float,
                 accelerometer_std_dev: float,
        ...
        self._g = g
        self._decawave_std_dev = decawave_std_dev
        self._odometer_std_dev = odometer_std_dev
        self._accelerometer_std_dev = accelerometer_std_dev
    ...
    def run(self) -> int:
        try:
            ...
            # Immediately start tracking our position
            self.track_position()
    ...
    def track_position(self):
        ...
        position_algorithm = KalmanPositionAlgorithm(self._frequency, self._decawave_std_dev, self._odometer_std_dev, self._accelerometer_std_dev)
        position_filter = PositionFilter(position_algorithm, 'position')
        self._tracking_filter = self._nine_dof_filters.add(ScalingFilter(self._imu_scaler))
        # attitude_filter = AttitudeFilter(ModifiedMadgwick(), name='attitude')
        attitude_filter = SwitchingAttitudeFilter(ModifiedMadgwick(), SimpleAttitudeAlgorithm(), name='attitude')
        self._tracking_filter.add(attitude_filter)\
            .add(AttitudeOutputToPositionInputFilter(self._g))\
            .add(position_filter)
    ...
    def stop_tracking_position(self):
        if self._tracking_filter:
            self._publish_attitude_data_toggle.stop()
            self._publish_position_data_toggle.stop()
            self._publish_decawave_scaled_data_toggle.stop()
            self._nine_dof_filters.remove(self._tracking_filter).close()
            self._tracking_filter = None
