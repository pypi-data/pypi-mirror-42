...
def makeDichotomicSound(ch1, ch2):
    '''
    Read two sound arrays (stereo format), then put them into two channels of one sound array (stereo format).
    ...
    output: array
        The sound data
    '''
    ch1 = [int(i) for i in ch1[::2]]
    ch2 = [int(i) for i in ch2[::2]]

    if len(ch1)>len(ch2):
        ch2 += array('h', [0]*(len(ch1)-len(ch2)))
    else:
        ch1 += array('h', [0]*(len(ch2)-len(ch1)))

    output = b''
    for i in range(0, len(ch1)):
        output+=struct.pack('<hh', ch1[i], ch2[i])
