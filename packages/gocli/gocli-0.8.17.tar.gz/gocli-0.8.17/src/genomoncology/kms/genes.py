...
@curry
def sync_boundaries(data, sdk=None, fields=("chromosome", "start", "end", "gene"), build=GRCh37):
    batch = ensure_collection(data)
    ...
    if gene_list.not_found:
        message = 'The following genes could not be found: ' + ', '.join(gene_list.not_found)
