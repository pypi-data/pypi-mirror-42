...
import inspect
from python_state_machine.models import Event, State, InvalidStateTransition
                ...
                def event_meta_method(event_name, event_description):
                    def f(self, *args, **kwargs):
                        ...
                        if self.__class__.callback_cache and \
                            ...
                            for validator in self.__class__.callback_cache[_adaptor.original_class.__name__]['before'][event_name]:
                                callback = validator["callback"]
                                result = callback(self, *args, **kwargs)
                                print("result", result)
                                if result is not None and "parameter" in validator:
                                    parameters_replacement[validator["parameter"]] = result
                                print("parameters_replacement", parameters_replacement)
                            ...
                            if self.__class__.callback_cache and \
                                ...
                                for callback in self.__class__.callback_cache[_adaptor.original_class.__name__]['after'][event_name]:
                                    if event_description is not None:
                                        if len(parameters_replacement)==0:
                                            print("call1", args, kwargs)
                                            callback(self, *args, **kwargs)
                                        else:
                                            print("call2", parameters_replacement)
                                            callback(self, **parameters_replacement)
