...
from python_state_machine.models import wrap_with_tuple
...
def before(before_what, parameter=None):
    def wrapper(func):
        ...
        #calling_class_dict['before'].setdefault(before_what, []).append({'callback': func, 'result':parameter})
        for what in wrap_with_tuple(before_what):
            calling_class_dict['before'].setdefault(what, []).append({'callback': func, 'result':parameter})
