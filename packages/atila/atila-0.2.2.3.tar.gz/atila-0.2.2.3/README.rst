...
*CAUTION:* Futures escape ealry from current requet handling thread and enter to main event loop. It means they will make connections to targets as possible as they can. If hundreds of clients request resource using database/upstream server, It will make error on target server like "too manty connection error". If you use Futures response, you SHOULD make sure these factors.       
