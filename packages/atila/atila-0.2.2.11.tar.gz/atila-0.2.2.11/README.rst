...
*Atila* is simple and minimal WSGI framework integrated with `Skitai App Engine`_ for exporting component and backend services.
...
.. code:: python
  ...
  @app.parameters_required (scope, required = None, ints = None, floats = None)
...
Usig Skitai Async Requests Services
------------------------------------------------------
...
.. code:: python
  ...
  def request (was):
    ...
    results = Tasks (reqs, timeout = 3).results
...
1. Futures escape ealry from current requet handling thread and enter to main event loop. It means they will make connections to targets as possible as they can. If hundreds of clients request resource using database/upstream server, It will make error on target server like "too manty connection error". If you use Futures response, you SHOULD make sure these factors.
2. Futures might be the most efficient in 2 conditions, a few simultaneously requesting clients, streaming data is relatively small size. Lot of clients will consume connection resources fast and large data stream make blanch coroutine advantages caused of expensive networking cost.
...
WS_FAST works out of threading pool and cannot use was.response, was.app and was.subapp. 
In this case, websocket response has IO blocking operation, entire Skitai event loop will be blocked. 
  
.. code:: python

  @app.route ("/websocket")
  @app.websocket (skitai.WS_SIMPLE | skitai.WS_FAST, 1200, onopen)
  def websocket (was, message, option):
    return 'you said: ' + message
