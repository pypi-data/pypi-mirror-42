...
class Decorators:    
    ...
    # parameter helpers ------------------------------------------------   
    RX_EMAIL = re.compile ("[a-z0-9][-.a-z0-9]*@[-a-z0-9]+\.[-.a-z0-9]{2,}", re.I)
    RX_UUID = re.compile ("[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}", re.I)
    ...
    def parameters_required (self, scope, required = None, ints = None, floats = None, emails = None, uuids = None, **kargs):
        def decorator(f):
            ...
            return wrapper
        return decorator
    param_required = parameters_required
