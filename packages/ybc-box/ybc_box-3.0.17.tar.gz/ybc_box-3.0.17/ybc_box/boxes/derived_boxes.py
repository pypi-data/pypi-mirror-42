...
def ynbox(msg="Shall I continue?", title=" ", choices=("[<F1>]Yes", "[<F2>]No"),
           image=None,
...
def integerbox(msg="", title=" ", default=None,
    ...
    while True:
        reply = enterbox(msg=msg, title=title, default=default, image=image, root=root)
        if reply is None:
            return None
        try:
...
# enterbox
# -------------------------------------------------------------------
def enterbox(msg="Enter something.", image=None, title=" ", default="",
             strip=True, root=None):
    """
...
def exceptionbox(msg=None, title=None):
    ...
    codebox(msg=msg, title=title, text=ut.exception_format())
...
def codebox(msg="", text="", title=" "):
    ...
    """
    return tb.textbox(msg=msg, text=text, title=title, codebox=True)
