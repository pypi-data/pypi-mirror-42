...
def lang_obj(user):
    ...
    command_config_file = Path("./lang.json")
...
def lang(path: str, user=None) -> str:
    """
    Ignore
    :param path: Ignore
    :param user: Ignore
    :return: Ignore
    """
    language = lang_obj(user)

    parts = path.split('.')

    for part in parts:
        if part in language:
            language = language[part]
        else:
            return None
...
def attachments_to_json(atts):
    ...
    for att in atts:
        dic = {'filename': att.filename, 'url': att.url, 'proxy_url': att.proxy_url}
        arr.append(dic)
