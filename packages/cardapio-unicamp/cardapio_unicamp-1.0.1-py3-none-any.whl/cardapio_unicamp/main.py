...
def is_valid_date(date):
    date_re = re.compile(r'^20\d\d-(0?[1-9]|1(1|2))-(0?[1-9]|[12]\d|3(0|1))$')
...
def get_next_meal(meal=''):
    ...
    # None means today or, on the weekneds, monday, since the http request
    # sends no d parameter
    ...
    elif weekday == 5 or weekday == 6 or now < 14:
        if meal:
            next_meal['meal'] = meal
        else:
