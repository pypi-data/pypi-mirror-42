...
@click.option(
    "--debug",
    is_flag=True,
    help="Show extra output,error messages, and exception stack trace",
    default=False,
)
@click.pass_context
def cli(ctx, debug):
...
@click.argument("env_args_string", nargs=-1, type=click.Path())
@click.pass_context
def run(ctx, wait, analyzer_directory, code, env_args_string):
    ...
    if not dep_version:
        ...
        sys.exit(1)
    # TODO redo this with org specific dependency
    artifact_link = (
...
def init(ctx):
    ...
    output_type = click.prompt(
        "Output type", default="json", type=click.Choice(["filesystem", "json", "both"])
    )
