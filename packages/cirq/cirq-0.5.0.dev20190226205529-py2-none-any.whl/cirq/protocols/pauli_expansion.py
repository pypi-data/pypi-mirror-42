...
def _filter_coefficients(expansion,
                         tolerance):
    u"""Drops insignificant coefficients."""
    return dict((n, c) for n, c in expansion.items() if abs(c) > tolerance)


def pauli_expansion(
    val, **_3to2kwargs
):
    if 'tolerance' in _3to2kwargs: tolerance = _3to2kwargs['tolerance']; del _3to2kwargs['tolerance']
    else: tolerance =  1e-9
