...
class GoodGateExplicitPauliExpansion(cirq.SingleQubitGate):
    ...
    def _pauli_expansion_(self):
        return {u'X': np.sqrt(1/2), u'Y': np.sqrt(1/3), u'Z': np.sqrt(1/6)}
...
class GoodGateNoPauliExpansion(cirq.MultiQubitGate):
    def __init__(self):
        super(GoodGateNoPauliExpansion, self).__init__(num_qubits=4)
...
class GoodGateNoUnitary(cirq.SingleQubitGate):
    def _pauli_expansion_(self):
        return {u'X': np.sqrt(1/2), u'Y': np.sqrt(1/2)}
...
class BadGateInconsistentPauliExpansion(cirq.SingleQubitGate):
    ...
    def _pauli_expansion_(self):
        return {u'X': np.sqrt(1/6), u'Y': np.sqrt(1/3), u'Z': np.sqrt(1/2)}
