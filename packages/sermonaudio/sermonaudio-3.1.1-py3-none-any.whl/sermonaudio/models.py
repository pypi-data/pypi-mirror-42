...
class MediaSet(Model):
    def __init__(self, obj: dict):
        ...
        self.audio = filter(is_valid, [Media.parse(rec) for rec in obj['audio']])
        self.video = filter(is_valid, [Media.parse(rec) for rec in obj['video']])
        self.text = filter(is_valid, [Media.parse(rec) for rec in obj['text']])
