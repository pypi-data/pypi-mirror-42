...
from .engineapi.typedefinitions import ActivityReference, DataField, ExecuteInspectorCommonArguments, FieldType, InspectorArguments, ProfileResult, Moments, Quantiles, HistogramBin, ValueCount, TypeCount, STypeCount
...
class DataProfile:
    ...
    @property
    def stype_counts(self) -> Dict[str, List[Tuple[str,int]]]:
        """
...
class ColumnProfile(ProfileResult):
    ...
    def _prepare_stype_counts(entries: List[Any]):
        if entries is not None and len(entries) > 0:
            result = [
                STypeCount(s_type = str(entry['sType']), count = int(entry['count']))
