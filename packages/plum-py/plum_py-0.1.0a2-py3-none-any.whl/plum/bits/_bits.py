...
from ._bitstype import BitsType


class Bits(Plum, metaclass=BitsType, nbytes=4, byteorder='little', fill=0, ignore=0):
        ...
        if args:
            ...
            try:
                ...
                try:
                    ...
                    raise TypeError(f'{args[0]!r} object is not an int or bit field dict')
                if kwargs:
    ...
    def __repr__(self):
        ...
        if flag_cls:
            flag_instance = flag_cls(int(self))
            return f'{cls.__name__}({repr(flag_instance)})'

        return self.__str__()

    def __str__(self):
        flag_cls = self._flag_cls
        if flag_cls:
            flag_instance = flag_cls(int(self))
