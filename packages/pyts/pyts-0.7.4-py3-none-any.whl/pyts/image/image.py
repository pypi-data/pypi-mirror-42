...
class MTF(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if self.quantiles == 'gaussian':
            ...
            bins = np.percentile(X,
                                 ...
                                 axis=1)
            mask = np.r_[
                ~np.isclose(0, np.diff(bins, axis=0), rtol=0, atol=1e-8),
                np.full((1, n_samples), True)
            ]
            X_binned = np.array([np.digitize(X[i], bins[:, i][mask[:, i]])
    ...
    def _mtf(self, binned_ts, ts_size, image_size, n_bins, overlapping,
        ...
        if remainder == 0:
            ...
            window_size += 1
            start, end, _ = segmentation(ts_size, window_size,
                                         overlapping, image_size)
