...
def log_metric(db: str, q: mp.Queue):
    ...
    while True:
        ...
        try:
            ...
            logger.warning(f'{type(e)}: {e} - Unable to insert {ins_sql} {vals}')
...
class SqliteMetric(Metric):
    def __init__(self, db: str, name: str, labels: Set[str]) -> None:
        ...
        self._proc.start()
        logger.info(f'Started writer process with pid {self._proc.pid}')
    ...
    def range(self, start=None, end=None, **filters) -> Tuple[float, float]:
        sel_sql_filters, vals = self._gen_filter(start, end, **filters)
        sel_sql = f'SELECT min(value), max(value) FROM {self._table} WHERE {sel_sql_filters}'
        logger.debug(sel_sql)
        logger.debug(vals)
        cur = self._conn.cursor()
        cur.execute(sel_sql, vals)
