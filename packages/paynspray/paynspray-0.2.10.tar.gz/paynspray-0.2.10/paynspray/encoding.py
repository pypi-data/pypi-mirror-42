...
def html_encode(string, mode = 'hex'):
    """Encodes a string for use in HTTP URIs and parameters"""
    if mode == 'hex':
        return "".join(map(lambda i: "&#x" + "{:02x}".format(i) + ";", amncore.compat.unpack_s('{}B'.format(len(string)), string)))
    elif mode == 'int':
        return "".join(map(lambda i: "&#" + str(i) + ";", amncore.compat.unpack_s('{}B'.format(len(string)), string)))
    elif mode == 'int-wide':
        return "".join(map(lambda i: "&#" + ("0" * (7 - len(str(i)))) + str(i) + ";", amncore.compat.unpack_s('{}B'.format(len(string)), string)))
    else:
        raise ValueError("Invalid mode '{}'".format(mode))
