...
def to_unicode(string = '', type = 'utf-16le', mode = '', size = ''):
    ...
    if type == 'utf-16le':
        return amncore.compat.pack_s('<{}H'.format(len(string)), *amncore.compat.unpack_s('{}B'.format(len(string)), string))
    elif type == 'utf-16be':
        return amncore.compat.pack_s('>{}H'.format(len(string)), *amncore.compat.unpack_s('{}B'.format(len(string)), string))
    elif type == 'utf-32le':
        return amncore.compat.pack_s('<{}L'.format(len(string)), *amncore.compat.unpack_s('{}B'.format(len(string)), string))
    elif type == 'utf-32be':
        return amncore.compat.pack_s('>{}L'.format(len(string)), *amncore.compat.unpack_s('{}B'.format(len(string)), string))
    elif type == 'utf-7':
        if mode == 'all':
        ...
        if size >= 2 and size <= 7:
            ...
            for a in bytearray(string):
                if (a < 21 or a > 0x7f) or mode != '':
                    ...
                    _string += amncore.compat.pack_s('B', a)
            return _string
        else:
