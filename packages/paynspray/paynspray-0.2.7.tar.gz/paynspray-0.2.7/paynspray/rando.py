...
try:
    ...
    HighAscii = str(struct.pack("128B", *list(range(0x80, 0xff + 1))).decode('latin-1'))
    LowAscii = str(struct.pack("32B", *list(range(0x00, 0x1f + 1))).decode('latin-1'))
    AllChars = str(struct.pack("256B", *list(range(0x00, 0xff + 1))).decode('latin-1'))
    Punctuation = str(struct.pack("31B", *(list(range(0x21, 0x2f + 1)) + list(range(0x3a, 0x3F + 1)) + list(range(0x5b, 0x60 + 1)) + list(range(0x7b, 0x7e + 1)))).decode('latin-1'))
...
def rand_base(length, bad, *foo):
    ...
    try:
        list_one = list((struct.unpack("{}B".format(len("".join(foo))), bytes("".join(foo)))))
        list_two = list((struct.unpack("{}B".format(len(bytes(bad))), bytes(bad))))
    except TypeError:
        list_one = list((struct.unpack("{}B".format(len("".join(foo))), bytes("".join(foo), 'latin-1'))))
        list_two = list((struct.unpack("{}B".format(len(bytes(bad, 'latin-1'))), bytes(bad, 'latin-1'))))
    ...
    return str(struct.pack("{}B".format(length), *outp).decode('latin-1'))
