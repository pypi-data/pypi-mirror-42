...
def to_hex(string, prefix = b"\\x", count = 1):
    ...
    return re.sub(b".{%d}" % (count * 2), lambda s: prefix + s.group(), binascii.hexlify(string))

def to_hex_ascii(string, prefix = b"\\x", count = 1, suffix = b""):
...
def hexdump(string, width = 16, base = 0):
    ...
    while (idx < len(string)):
        ...
        for c in amncore.compat.unpack_s("{}B".format(len(chunk)), chunk):
