...
class MyTestCase(unittest.TestCase):
    def test_browser(self):
        self.assertIsNotNone(openwebsite("猿辅导"))

    def test_open_browser(self):
        self.assertEqual(-1, openwebsite("1234567"))
    ...
    def test_browser_ParameterTypeError(self):
        with self.assertRaisesRegex(ParameterTypeError, "^参数类型错误 : 调用openwebsite方法时，'text'参数类型错误。$"):
            openwebsite(1)

    def test_browser_ParameterValueError(self):
        with self.assertRaisesRegex(ParameterValueError, "^参数数值错误 : 调用openwebsite方法时，'text'参数不在允许范围内。$"):
            openwebsite('')
