...
class KinesisPipeline:
    ...
    def __init__(self, settings, stats):
        ...
        self.kinesis = boto3.client('kinesis', region_name=settings['KINESISSTREAM_REGION'])
        self.items = []
        self.chunk_number = 0
    ...
    def process_item(self, item, spider):
        ...
        kinesis_record = self._generate_record(item)
        self.items.append(kinesis_record)
    ...
    def _generate_record(self, item):
        """
        Generate Kinesis Record
        """
        data = json.dumps(item, default=self._json_serial)
        return {'Data': bytes(data, 'utf-8'), 'PartitionKey': self.partition_key}
