...
def processExcelToJsonArgs(arumentList):
    ...
    try:
        ...
        # If help is an argument, show the help text and quit the program
        if "help" in argumentsDict.keys():
            print(helpText)
            quit()
...
def processExtractTranslationsArgs(arumentList):
    # Init helpText
    helpText = "Not documented yet..."
    try:
        options, arguments = parseArgs(arumentList, "", [], [])
        argumentsDict = {}
        mapShortToLong = {}
        #processValuelessArguments(["help", "quiet", "verbose"], options, argumentsDict, mapShortToLong)
        ...
        # If there is only one argument (the path to the base directory)
        if len(arguments) == 1:
            # Process arguments
            for o, v in options:
                if (v != ""):
