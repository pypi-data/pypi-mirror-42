...
class ArtifactUploader(ChainedIdentity):
    ...
    def upload_single_artifact_list(self, summary_object, artifact_tuple_list, explanation_id):
        ...
        :param artifact_tuple_list: A list with names, values, and optional metadata for each data type
        :type artifact_tuple_list: (str, list, dict or None
        ...
        for name, values, optional_metadata in artifact_tuple_list:
            self._logger.debug('Uploading single {} artifact'.format(name))
            self._upload_artifact(upload_dir, name, values)
            artifact_info = [{
