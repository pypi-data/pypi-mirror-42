...
class UserLogStream:
    ...
    def stream_log(self, log_type, file_path) -> None:
        ...
        :return: None
        """
        alive, _ = self.task_context.monitor()
        while alive:
            if exists(file_path):
    ...
    def upload_user_log(self, logs) -> bool:
        ...
        while retries > 0:
            ...
            if success:
                print("send logs: {}".format(json.dumps(logs)))
                return True
            else:
                retries -= 1

        print("[upload_user_log] failed: {}".format(response_body))
        return False
...
def validate_log(log) -> (bool, str):
    ...
    if log['log_type'] not in ["log", "errpr"]:
