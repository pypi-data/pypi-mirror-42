...
class WorkerStagePoll(WorkerStage):
    def run(self, state_machine):
        try:
            success, task_ready, re_enroll = state_machine.worker.poll()
        except Exception as e:
...
class WorkerStageFetch(WorkerStage):
    def run(self, state_machine):
        ...
        if success:
            ...
            return state_machine.stage_cleanup
...
class WorkerStagePostRun(WorkerStage):
    def run(self, state_machine):
        try:
            ...
            traceback.print_exc()
            # Swallow exception here and always proceed to cleanup.
...
'''
class WorkerStageCleanup(WorkerStage):
    def run(self, state_machine):
        try:
            success = state_machine.worker.cleanup()
        except Exception as e:
            print("[Stage:CLEANUP] Exception: {}".format(str(e)))
            traceback.print_exc()
            success = False
...
class WorkerStateMachine:
    ...
    stage_cleanup = WorkerStageCleanup()
