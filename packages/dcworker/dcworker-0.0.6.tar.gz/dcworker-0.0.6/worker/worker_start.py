...
from os.path import exists, join, basename, \
    realpath, dirname, isfile, \
    isdir, splitext, getsize
...
class Worker(object):
    ...
    def enroll(self):
        ...
        if not isdir(self.workdir):
            self.worker_logger.log_error(
                "enroll", "invalid workdir: {}".format(self.workdir))
        ...
        response_body = self.runtime.get_stage_configuration(
            WORKER_STAGE_ENROLL,
    ...
    def poll(self):
        ...
        # Server indicates that a task is ready for the worker.
            ...
            keys = ["task_uuid", "signal_finish", "ask_finish",
                    "keep_alive_interval", "config", "access_key_id",
                    "secret_access_key", "session_token", "expiration",
    ...
    def _download_code(self):
        ...
        s3_client = S3Client(region_name=self.temp_token["s3_region"],
                             aws_access_key_id=self.temp_token["access_key_id"],
                             aws_secret_access_key=self.temp_token["secret_access_key"])
        ...
        if USE_COMPILED_CODE:
            ...
            s3_client.s3_download_folder(bucket_name, task_location, code_dir)
            ...
            s3_client.s3_download_folder(bucket_name,
                                         self.task.model_location,
                                         code_dir)
    ...
    def pre_run(self):
        ...
        if not self.runtime.worker_run_in_container():
            ...
            sucess = self._install_user_requirements_direct(requirements_file)
        else:
            sucess = self._install_user_requirements_container(requirements_file)
    ...
    def run(self):
        self.worker_logger.log_info(
            "run",
            "{} task: {}".format(self.task.job_name, self.task.task_uuid))

        # Prepare environment variables.
        env = {
            WORKER_RUN_ENV_TASK_UUID: str(self.task.task_uuid),
    ...
    def signal(self):
        ...
        while True:
            ...
            if alive and graceful:
                ...
                if response_body is None:
                    request_body = {'task_uuid': self.task.task_uuid,
                                    'action': 'progress'}
                ...
                if status_code == 200:
                    ...
                    if status == "continue":
                        ...
                        self.worker_logger.log_info(
                            "signal",
                            "terminate task {}".format(self.task.task_uuid))
                ...
                if graceful:
                    self.worker_logger.log_info(
                        "signal",
                        "task {} finished".format(self.task.task_uuid))
                ...
                if not graceful or self.task.signal_finish:
                    # First check if finish signal is simulated.
    ...
    def _create_tensorboard(self):
        ...
        success, _, response_body, error_message = request_server(
            self.master_endpoint + "/api/v1/worker/task/{}/".format(self.task.task_uuid),
            'put',
            **{"data": request_body,
               "auth": self.auth_token})
        if success:
    ...
    def _get_user_log_file_name(self):
        return "{}.log".format(self.task.task_uuid)
    ...
    def _get_user_error_log_file_name(self):
        return "{}.error.log".format(self.task.task_uuid)
