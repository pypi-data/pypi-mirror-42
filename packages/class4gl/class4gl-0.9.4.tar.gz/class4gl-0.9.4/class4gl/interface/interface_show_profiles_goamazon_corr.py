...
path_forcing = "/data/gent/vo/000/gvo00090/D2D/data/SOUNDINGS/GOAMAZON5/"
path_experiments = "/user/data/gent/gvo000/gvo00090/D2D/data/GOAMAZON/CAMPAIGN_CORR/"

experiments = ['ORIG','CC','EF','EFCC']
...
for iexp,exp in enumerate(experiments):
    c4gldata[iexp].sel_station(90002)
    sel = 0
    if (select[0]) > 0:
        c4gldata[iexp].next_record( (select[0]) - sel)
    profiles_morning            = []
    profiles_morning_raw        = []
    profiles_noon_obs           = []
    profiles_noon_obs_raw       = []
    ...
    profiles_afternoon_obs      = []
    profiles_afternoon_obs_raw  = []
    profiles_afternoon_end_mod.append([])
    profiles_location      = []
    profiles_morning_datetime      = []
    profiles_noon_datetime      = []
    profiles_afternoon_datetime      = []
    for j,i in enumerate(select):
        ...
        profiles_morning_datetime.append(c4gldata[iexp].frames['profiles']['record_yaml_ini'].pars.ldatetime)
        profiles_morning.append( c4gldata[iexp].frames['profiles']['record_yaml_ini'].air_ap)
        profiles_morning_raw.append( c4gldata[iexp].frames['profiles']['record_yaml_ini'].air_balloon[::15])
        ...
        profiles_afternoon_datetime.append(c4gldata[iexp].frames['profiles']['record_yaml_end_obs'].pars.ldatetime)
        profiles_afternoon_obs.append( c4gldata[iexp].frames['profiles']['record_yaml_end_obs'].air_ap)
        profiles_afternoon_obs_raw.append(c4gldata[iexp].frames['profiles']['record_yaml_end_obs'].air_balloon[::15])
        profiles_afternoon_end_mod[iexp].append( c4gldata[iexp].frames['profiles']['record_yaml_end_mod'].air_ap)
    
    
        if j+1 < len(select):
            c4gldata[iexp].next_record(select[j+1] - sel)
    
    
                #axes[varkey].set_title(latex['d'+varkey+'dt']+' '+units_final,fontsize=12.)                                     


numprofiles = len(profiles_afternoon_end_mod[-1])
...
for var in VARS:
    ...
    for i in range(numprofiles):
        ...
        plt.plot(profiles_morning_raw[i][var].values,profiles_morning_raw[i].z.values,'bx')
        
        label = profiles_afternoon_datetime[i].strftime("%H:%m")+'ST'+' obs'
        plt.plot(profiles_afternoon_obs[i][var].values*fac, profiles_afternoon_obs[i].z.values,'m--',label=label)
        plt.plot(profiles_afternoon_obs_raw[i][var].values*fac,profiles_afternoon_obs_raw[i].z.values,'mx')
        linestyles = ['-','--',':','-.']
        for iexp,exp in enumerate(experiments):
            label = profiles_afternoon_datetime[i].strftime("%H:%m")+'ST'+' '+exp
            plt.plot(profiles_afternoon_end_mod[iexp][i][var].values*fac, profiles_afternoon_end_mod[iexp][i].z.values,'r'+linestyles[iexp],label=label)
