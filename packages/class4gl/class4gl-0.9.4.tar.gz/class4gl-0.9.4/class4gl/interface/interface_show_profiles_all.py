...
path_forcing = "/data/gent/vo/000/gvo00090/D2D/data/SOUNDINGS/IOPS/"
path_experiments = "/user/data/gent/gvo000/gvo00090/D2D/data/IOPS/"
for exp in ['BASE_ITER_NOADV']:
...
#select = [1,2,3,4,5,6,7,13,13]


for STNID,location in [[90000,'HUMPPA'],[90001,'BLLAST'],[90002,'GOAMAZON']]:

#location = 'GOAMAZON'
    c4gldata[0].sel_station(STNID)
    ...
    for j,i in enumerate(select):
        ...
        skip = int(np.sum(c4gldata[0].frames['profiles']['record_yaml_ini'].air_balloon.z.values < 3000.)/20.)
        print(skip)
        profiles_morning_raw.append( c4gldata[0].frames['profiles']['record_yaml_ini'].air_balloon[::skip])
        ...
        profiles_afternoon_obs_raw.append(c4gldata[0].frames['profiles']['record_yaml_end_obs'].air_balloon[::skip])
...
for var in VARS:
    ...
    for i in range(numprofiles):
        ...
        plt.plot(profiles_morning_raw[i][var].values*fac,profiles_morning_raw[i].z.values,'bx')
        ...
        plt.plot(profiles_afternoon_obs_raw[i][var].values*fac,profiles_afternoon_obs_raw[i].z.values,'rx')
        ...
        if var == 'theta':
             ...
             ax.set_xlim((0.,21.))
        ax.set_title(profiles_location[i]+' '+ str(profiles_morning_datetime[i].date()),fontsize=6.)
    ...
    fig.tight_layout()
    fig.subplots_adjust(wspace=0.05,right=0.99,hspace=0.25,left=0.04)
    fig.show()
