...
class QuantumMonty:
    __slots__ = ("arr", "flat", "data", "max_id", "truffle_shuffle", "quantum_monty_arr", "quantum_not_monty_arr")
    ...
    def quantum_monty(self):
        return flatten(random_value(self.quantum_monty_arr), self.flat)

    def quantum_not_monty(self):
        return flatten(random_value(self.quantum_not_monty_arr), self.flat)
...
def timer(func: staticmethod, *args, **kwargs):
    ...
    return "Time: " + ", ".join(output)


def distribution(func: staticmethod, *args, **kwargs):
    num_cycles = 10000
...
def distribution_timer(func: staticmethod, *args, dist=True, time=True, **kwargs):
    if time:
        print(timer(func, *args, **kwargs))
    if dist:
