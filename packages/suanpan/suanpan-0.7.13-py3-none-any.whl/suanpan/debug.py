...
def abtest(
    ...
    costTimes = sorted(round(r[0] * 1000, 6) for r in results)
    totalTime = sum(costTimes)
    avgTime = totalTime / number
    minTime = min(costTimes)
    maxTime = max(costTimes)

    chartLabels, chartData = zip(
        *[
            ("{}%".format(int(mp * 100)), [costTimes[math.floor(number * mp) - 1]])
    ...
    term.chart(chartData, labels=chartLabels)
