...
class RedisMQ(object):
    ...
    def sendMessage(self, queue, data, messageID="*", maxlen=1000):
    ...
    def recvPendingMessages(
        self, queue, group="default", consumer="unknown", start="-", end="+", count=1
    ):
        return self.client.xpending_range(
            queue, group, start, end, count, consumername=consumer
        )

    def getPendingMessage(self, queue, id, group="default", consumer="unknown"):
        pendingMessages = self.recvPendingMessages(
            queue, group=group, consumer=consumer, start=id, end="+", count=1
        )
        return next(iter(pendingMessages), None)
