...
from .validators import max_str_len, min_max_str_len, min_str_len, min_number, max_number, min_max_number
...
def _implement_validator(type_, field):
    ...
    valid_func = {
        ...
        ('min_length', 'max_length'): min_max_str_len,
        ('minimum', ): min_number,
        ('maximum', ): max_number,
        ('minimum', 'maximum'): min_max_number
    }
    if type_ == str:
        key = tuple({"min_length", "max_length"} & set(field.metadata))
        validator = valid_func[key] if key in valid_func else None
    elif type_ in (int, float):
        key = tuple({"minimum", "maximum"} & set(field.metadata))
        validator = valid_func[key] if key in valid_func else None
    if validator:
...
def route(summary=None, description=None, consumes=None, produces=None,
          consumes_content_type=None, produces_content_type=None, exclude=None):
    def inner(func):
        ...
        data = {
            ...
            'consumes': consumes,
            'produces': produces,
            'consumes_content_type': consumes_content_type,
            'produces_content_type': produces_content_type,
            'exclude': exclude
        }
        for k, v in data.items():
            if v:
                setattr(route_spec, k, v)
        return func
