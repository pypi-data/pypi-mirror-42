...
def install():
    ...
    subprocess.check_output("rm -rf ~/.canvas/ExpertModeCTStudio && rm -rf ~/.canvas/Modelhouse && curl -x 192.168.204.85:8080 -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/dependencies.zip -O && unzip dependencies.zip -d ~/.canvas/ && rm -rf dependencies.zip && dos2unix -R "+app_server_path+"/conf/*", shell=True)
    ...
    if (server_type == 'tomcat'):
        ...
        if os.path.exists(os.path.expanduser("~/.canvas")):
            if os.path.exists(os.path.expanduser("~/.canvas/ctmodelhouse.war")) and os.path.exists(os.path.expanduser("~/.canvas/expertctstudio.war")):
                ...
                subprocess.check_output("curl -x 192.168.204.85:8080 -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/expertctstudio.war -O && curl -x 192.168.204.85:8080 -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/ctmodelhouse.war -O && mv expertctstudio.war ~/.canvas/expertctstudio.war &&  mv ctmodelhouse.war ~/.canvas/ctmodelhouse.war && chmod +x ~/.canvas/*", shell=True)
        else:
            subprocess.check_output("mkdir ~/.canvas && mkdir ~/.canvas/template && curl -x 192.168.204.85:8080 -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/expertctstudio.war -O && curl -x 192.168.204.85:8080 -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/ctmodelhouse.war -O && mv expertctstudio.war ~/.canvas/expertctstudio.war &&  mv ctmodelhouse.war ~/.canvas/ctmodelhouse.war && chmod +x ~/.canvas/*", shell=True)
...
def db(server_type,db_type,app_server_path):
    ...
    subprocess.check_output("rm -rf ~/.canvas/tmpl && curl -x 192.168.204.85:8080 -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/tmpl.zip -O && unzip tmpl.zip -d ~/.canvas/tmpl/", shell=True)
