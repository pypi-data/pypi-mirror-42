...
class TotalStatistics(object):
    ...
    def message(self):
        ...
        cpass_str = str(cpass) if cpass == 0 else '\033[32m' + str(cpass) + ' passed\033[0m'
        apass_str = str(apass) if apass == 0 else '\033[32m' + str(apass) + ' passed\033[0m'
        cfail_str = str(cfail) if cfail == 0 else '\033[31m' + str(cfail) + ' failed\033[0m'
        afail_str = str(afail) if afail == 0 else '\033[31m' + str(afail) + ' failed\033[0m'
        return ('%d critical %s%s, %s, %s'
                '%d %s%s total, %s, %s'
                % (ctotal, test_or_task, cend, cpass_str, cfail_str,
                   atotal, test_or_task, aend, apass_str, afail_str))
