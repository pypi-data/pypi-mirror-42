...
class _Verify(_BuiltInBase):
    ...
    def should_be_empty(self, *item, msg=None):
        ...
        for el in item:
            if self.get_length(el) == 0:
                raise AssertionError(msg or "%s '%s' should be empty." % ('' if len(item) == 0 else '(index=' + str(item.index(el)) + ')', el)
    ...
    def should_not_be_empty(self, *item, msg=None):
        ...
        for el in item:
            if self.get_length(el) == 0:
                raise AssertionError(msg or "%s '%s' should not be empty." % ('' if len(item) == 0 else '(index=' + str(item.index(el)) + ')', el)
