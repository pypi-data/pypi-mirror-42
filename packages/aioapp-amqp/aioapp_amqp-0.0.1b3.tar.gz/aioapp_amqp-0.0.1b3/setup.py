...
if not PY_VER >= (3, 6):
    raise RuntimeError('aioapp_amqp does not support Python earlier than 3.6')

with open('aioapp_amqp/__init__.py') as ver_file:
...
setup(
    name='aioapp_amqp',
    version=version,
    description="Aioapp adapter for aioamqp",
    long_description=readme + '\n\n' + history,
    author="Konstantin Stepanov",
    url='https://github.com/inplat/aioapp_amqp',
    packages=find_packages(),
    include_package_data=True,
    install_requires=list(filter(lambda a: a, requirements.split('\n'))),
    license="Apache License 2.0",
    zip_safe=False,
    keywords='aioapp_amqp',
    classifiers=[
