...
TODO:
- URL encode to handle whitespaces, special characters, ecc. in stocks
...
class IEXTrading():
    ...
    #This method format lists in url encode format (ex. ['AAPL', 'MSFT'] --> 'AAPL,MSFT')
    ...
    def get_companies_by_sector(self, sectorName):
        urlCall = '/stock/market/collection/sector?collectionName='+sectorName
        return self.make_request(urlCall)

    #Note: need url encode
    def get_companies_by_tag(self, tagName):
        urlCall = '/stock/market/collection/tag?collectionName='+tagName
        return self.make_request(urlCall)

    #https://iextrading.com/developer/docs/#dividends
    def get_stock_dividend(self, range):
        if isinstance(sel.ticker, str):
            if range in availableRanges:
