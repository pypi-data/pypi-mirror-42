...
def send_tokens(skale, sender_wallet, receiver_account, amount, await=True):
    logger.info(f'Sending {amount} SKALE tokens from {sender_wallet["address"]} => {receiver_account}')
...
def send_ether(web3, sender_wallet, receiver_account, amount, await=True):
    logger.info(f'Sending {amount} ETH from {sender_wallet["address"]} => {receiver_account}')
...
def generate_accounts(skale, base_wallet, n_wallets, skale_amount, eth_amount, debug=False):
