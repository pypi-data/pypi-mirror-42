...
def make_index_from_book(book):
  ...
  left = ""
  small_dots = """<span class='title' style="font-size:25px"> .............................. </span>"""
  for index, item in enumerate(book):
    for title, subtitles in item.items():
      left += "<br/><span class='title'>{TEXT}  {dots} {INDEX}</span>".format(TEXT=title, INDEX=index+1, dots=small_dots)
      for subtitle_index, subtitle in enumerate(subtitles):
        left += "<br/> &emsp; <span class='subtitle'>{TEXT}  {dots} {INDEX}</span>".format(TEXT=subtitle, INDEX=str(index+1) + "." + str(subtitle_index), dots=small_dots)

  table = """
  <table style="width:100%">
    <tr>
      <td style="width:90%">{LEFT}</td>
      <td style="width:10%">{RIGHT}</td>
    </tr>
  </table>
