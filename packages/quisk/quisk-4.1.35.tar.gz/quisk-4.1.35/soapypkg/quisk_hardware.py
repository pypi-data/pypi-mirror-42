...
class Hardware(BaseHardware):
  ...
  def open(self):			# Called once to open the Hardware
    ...
    txt = soapy.open_device(self.conf.soapy_device, 1, self.conf.data_poll_usec)
    rate = self.rx_sample_rates[self.vardecim_index]
    soapy.set_parameterD('soapy_sample_rate_rx', float(rate))
    soapy.set_parameterS('soapy_antenna_rx', self.conf.soapy_antenna_rx)
    soapy.set_parameterD('soapy_gain_rx', self.conf.soapy_gain_rx)
    return txt
  ...
  def ChangeFrequency(self, tune, vfo, source='', band='', event=None):
    ...
    if self.VFO != fVFO:
      self.VFO = fVFO
      if soapy:
        soapy.set_parameterD('soapy_freq_rx', fVFO)
    return tune, vfo
  ...
  def VarDecimSet(self, index=None):	# Called when the control is operated; if index==None, called on startup.
    ...
    if soapy:
      soapy.set_parameterD('soapy_sample_rate_rx', float(rate))
      soapy.set_parameterD('soapy_freq_rx', self.VFO)	# driver Lime requires reset of Rx freq on sample rate change
    return rate
