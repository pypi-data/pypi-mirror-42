  ...
  def UpdateConf(self):		# Called second to update the configuration for the selected radio
    ...
    for k, v in list(radio_dict.items()):	# radio_dict may change size during iteration
      ...
      if k == 'power_meter_local_calibrations':	# present in configuration data but not in the config file
        continue
      if k[0:7] == 'Choice_':	# present in configuration data but not in the config file
        continue
      try:
  ...
  def OnChange2(self, ctrl, value):
    ...
    if self.FormatOK(value, fmt4):
      ...
      if self.radio_name == Settings[1]:	# changed for current radio
        ...
        elif name[0:6] == 'soapy_' and soapy:
          if fmt4 == 'text':
            soapy.set_parameterS(name, value)
          else:
            value = float(value)
            soapy.set_parameterD(name, value)
...
class RadioHardwareSoapySDR(RadioHardwareBase):	# The Hardware page in the second-level notebook for the SoapySDR radios
  def __init__(self, parent, radio_name):
    ...
    border = 2
    for name, text, fmt, help_text, values in data_names:
      if name == 'soapy_device':
        device = self.GetValue(name, radio_dict)
        txt, self.edit_soapy_device, btn = self.AddTextEditHelp(1, text, device, help_text, span1=1, span2=4)
    ...
    self.soapy_params = []
    for name, text, fmt, help_text, values in data_names:
      if name in ('hardware_file_name', 'widgets_file_name', 'soapy_device', 'use_soapy'):
        pass
      elif name[0:4] == platform_ignore:
        pass
      elif fmt[0:4] in ('dict', 'list'):
        ...
        no_edit = "choice" in fmt or fmt == 'boolean'
        if name[0:6] == 'soapy_':
          values = radio_dict.get('Choice_' + name, [])
          self.soapy_params.append(name)
        txt, cb, btn = self.AddTextComboHelp(col, text, value, values, help_text, no_edit, border=border)
        cb.handler = self.OnChange
        cb.quisk_data_name = name
        setattr(self, 'ctrl_' + name, cb)
        if col == 1:
          col = 4
          border = 0
        else:
  ...
  def OnButtonChangeSoapyDevice(self, event):
    if not soapy:
      ...
      msg.Destroy()
      return
    is_sample_device = Settings[1] == self.radio_name
    try:
      choices = self.GetSoapyDevices()
    except:
      traceback.print_exc()
      choices = ['No devices were found.']
    dlg = wx.SingleChoiceDialog(None, "", "Quisk found these devices",
        choices, style=wx.DEFAULT_FRAME_STYLE|wx.OK|wx.CANCEL)
    ok = dlg.ShowModal()
    if ok == wx.ID_OK:
      device = dlg.GetStringSelection()
    dlg.Destroy()
    if ok == wx.ID_OK:
      if is_sample_device:
      ...
      if txt[0:8] == 'Capture ':
        ...
        for param in self.soapy_params:
          values = soapy.get_parameter(param, 0)
          radio_dict['Choice_' + param] = values
          #print (param, type(values), values)
          ctrl = getattr(self, 'ctrl_' + param)
