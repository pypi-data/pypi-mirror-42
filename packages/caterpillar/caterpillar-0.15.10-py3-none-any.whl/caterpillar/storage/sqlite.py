...
class SqliteReader(StorageReader):
    ...
    def count_docs_from_frames(self, frame_ids):
        """ Returns the number of documents that match the supplied frame_ids."""
        rows = self._executemany('''
            SELECT document_id
            FROM frame
            WHERE id = ?
        ''', ((f,) for f in frame_ids))
        unique_doc_ids = {doc[0] for doc in rows}
    ...
    def count_documents_with_frames(self, include_fields=None):
        """Returns the number of documents in the index."""
        where_clause, fields = self._fielded_where_clause(include_fields=include_fields, exclude_fields=None)
        return self._execute(
            'select count(distinct document_id) '
            'from frame '
            'inner join unstructured_field field on field.id = frame.field_id ' + where_clause,
            fields
        ).fetchone()[0]
    ...
    def iterate_metadata_values(self):
        """
        Get all metadata field values.

        This method is a generator that yields tuples of (field_name, value)
    ...
    def _iterate_frame_metadata(self, frame_ids, structured_fields=None):
        ...
        if not structured_where_clause:
            structured_where_clause = "where frame.id = ?"
        else:
            structured_where_clause += "and frame.id = ?"
        results = self._executemany(
            """
            select
            ...
            {}
            """.format(structured_where_clause),
            (metadata_fields + [frame_id] for frame_id in sorted(frame_ids))
        )
        return results
    ...
    def _iterate_document_metadata(self, document_ids, structured_fields=None):
        ...
        if not structured_where_clause:
            structured_where_clause = "where document_id = ?"
        else:
            structured_where_clause += "and document_id = ?"
        results = self._executemany(
            """
            select
                field.name, value, document_id
            from document_data
            inner join structured_field field
                on field.id = document_data.field_id
            {}
            """.format(structured_where_clause),
            (metadata_fields + [document_id] for document_id in sorted(document_ids))
        )
        return results

    def _iterate_frame_date_metadata(self, frame_ids, structured_fields, resolution='day'):
        """
        Provisional API for retrieving metadata for a specific set of frames
        specifically from date fields.

        This is provisional while we're still working out how best to handle
        frame-document correspondence and the granularity of measurements.

        ..warning: You must past a list of fields that you wish to query and
        they *MUST* be date fields. This isn't checked and will fail horribly
        if not adhered to.

        TODO: Actually check the fields are date fields, and make structured_fields
        an optional argument that defaults to all date fields.

        This method supports aggregating dates into a desired resolution. The
        options are:
            * Year
            * Month
            * Week
        ...
        """
        value = "date(value"
        if DATE_MODIFIERS.get(resolution):
            value += ", {}".format(DATE_MODIFIERS[resolution])
        value += ")"
        structured_where_clause, metadata_fields = self._fielded_where_clause(
            structured_fields, None, structured=True
        )
        ...
        results = self._executemany(
            ...
            inner join frame
                on dd.document_id = frame.document_id
            {}
            """.format(value, structured_where_clause),
            (metadata_fields + [frame_id] for frame_id in sorted(frame_ids))
        )
        return results

    def _iterate_document_date_metadata(self, document_ids, structured_fields, resolution='day'):
        """
        Provisional API for retrieving metadata for a specific set of documents
        specifically from date fields.
    ...
    def filter_metadata(
        ...
        valid_metadata_operators = set(('<', '>', '<=', '>=', '=', 'not in', 'in', 'between', 'not between'))
        ...
        for metadata_field, operators in metadata.items():
            ...
            for operator, value in operators.items():
                ...
                if operator.endswith('in'):  # The values for an 'in' operator should be an iterable.
    ...
    def _fielded_where_clause(self, include_fields, exclude_fields, structured=False):
        ...
        if include_fields:
            ...
            where_clause = ''
        return where_clause, list(fields)
