...
class IndexReader(object):
    ...
    def get_frame_count(self, include_fields=None):
        """Return the int count of frames stored on this index."""
        return self.__storage.count_frames(include_fields=include_fields)
    ...
    def get_document_count_from_frame_ids(self, frame_ids):
        """ Returns the number of unique documents that match the given ``frame_ids``."""
        return self.__storage.count_docs_from_frames(frame_ids)
    ...
    def get_documents_with_frames_count(self, include_fields=None):
        """
        Returns the number of unique documents that have frames with the supplied fields.

        include_fields: which fields to include as an iterable.

        """
        return self.__storage.count_documents_with_frames(include_fields)
    ...
    def get_metadata_values(self):
        """
        Return a dict of metadata field values:
        ...
        """
        metadata_values = self.__storage.iterate_metadata_values()
    ...
    def _validate_analyse_metadata(self, metadata_search_spec):
        ...
        for field, operators in metadata_search_spec.items():
            ...
            for operator, values in operators.items():
                ...
                if operator == 'in' or operator == 'not in' or operator == 'between' or operator == 'not between':
                    # Value list
