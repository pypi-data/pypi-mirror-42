...
def get_best_match(output, po_lines):
    ...
    #     print('POOOOOOOs', len(filtered_pos_dict))
    #     for k, v in filtered_pos_dict.items():
    #         print(k, v)
    matches = []
    for i, line in enumerate(output["lines"]):
        ...
        if filtered_pos_dict.get(line["po_item"], "") != "":
            ...
            #             print(line['qty'], line['description'])
            ...
            print(f"Matched using the line matcher")
            print(
                f"PO line: {list(filtered_pos_dict.values())[best_match_line_number-1]}"
            )
            print(f'Invoice line: {line["description"]} qty: {line["qty"]}')
            print()
...
def move_to_no_po(api_key, document_id, document, output):
...
def build_full_output(api_key, doc_list, po_lines):
    ...
    for doc in doc_list:
        ...
        if not output["header"]["po_number"]:
            move_to_no_po(api_key, document, output)
            print("No PO. Moved to no_po folder")
            continue
        if output["header"]["multiline"] > 0:
