...
def build_output_email_invoice(document, output):
    try:
        output['stem'] = output['header']['filename']rsplit('.', 1)[0]
    except:
...
def build_output_policy_remittance(document, output):
    try:
        ...
        output['stem'] = output['header']['filename']
    filename_list = [output['header']['remittance_date'], 
...
def get_best_match(output, po_lines):
    ...
    filtered_pos = filtered_pos[['stock_code', 'stock_description', 'qty', 'line_net', 'line_number']]
