...
class HashNode(_BasicNode):
	...
	def toDict(self):
		...
		# Add the hash key
		dRet['__hash__'] = self._node.toDict()
		...
		# If we are validating a DATE, DATETIME, IP or TIME data point
			...
			if self._type == 'ip':
				...
				if self._minimum or self._maximum:
					...
					if self._maximum and self.__compare_ips(value, self._maximum) == 1:
						self.validation_failures.append(('.'.join(level), 'exceeds maximum'))
						return False

					# If the IP is less than the minimum
					if self._minimum and self.__compare_ips(value, self._minimum) == -1:
...
class OptionsNode(_NodeInterface):
	...
	def __init__(self, details):
		...
		for i in range(len(details)):
			...
			if isinstance(details[i], _BasicNode):
