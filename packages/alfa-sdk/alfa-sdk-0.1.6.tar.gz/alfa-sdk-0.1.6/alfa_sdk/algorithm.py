...
from alfa_sdk.common.session import fetch_context
...
class AlgorithmClient(BaseClient):
    ...
    def get_environment_from_context(self):
        if not self.context or "algorithmEnvironmentId" not in self.context:
            return None

        environment_id = self.context.get("algorithmEnvironmentId")
    ...
    def deploy(self, algorithm_id, environment, version, path, **kwargs):
        algorithm = self.get_algorithm(algorithm_id)
        environment = algorithm.get_environment(environment)
        return environment.deploy(version, path, **kwargs)
