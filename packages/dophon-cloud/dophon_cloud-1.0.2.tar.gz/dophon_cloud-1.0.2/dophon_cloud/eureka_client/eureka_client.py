...
def _register(eureka_server, instance_dic):
    ...
    response = urllib2.urlopen(req, json.dumps({"instance": instance_dic}).encode(_DEFAULT_ENCODING),
...
class RegistryClient:
        ...
        with self.__net_lock:
            ...
            while len(untry_servers) > 0:
                ...
                try:
                    ...
                    logger.warning(f"Eureka server [{url}] is down, use next url to try.({e})" )
    ...
    def __get_instance_ip(eureka_server):
        ...
        logger.debug("target eureka host::: %s" % _target_)
        if _target_.find(':') > 0:
            arr = _target_.split(':')
            target_ip = arr[0]
    ...
    def send_heart_beat(self, overridden_status=""):
        try:
            ...
            logger.error(f"error!{e.__dict__}")
            # lost heartbeat info
            try:
                self.register()
            except:
                # try reg self failed,close this server
                self.stop()
...
class DiscoveryClient:
    ...
    def __fetch_delta(self):
        def do_fetch(url):
            ...
            logger.debug("delta got: v.%s::%s" % (delta.versionsDelta, delta.appsHashcode))
    ...
    def __is_hash_match(self):
        ...
        logger.debug("check hash, local[%s], remote[%s]" % (app_hash, self.__delta.appsHashcode))
        return app_hash == self.__delta.appsHashcode

    def __merge_delta(self, delta):
        logger.debug("merge delta...length of application got from delta::%d" % len(delta.applications))
        for application in delta.applications:
            for instance in application.instances:
                logger.debug("instance [%s] has %s" % (instance.instanceId, instance.actionType))
    ...
    def walk_nodes(self, app_name="", service="", prefer_ip=False, prefer_https=False, walker=None):
        ...
        while node is not None:
            try:
                ...
                logger.warning("do service %s in node [%s] error, use next node." % (service, node.instanceId))
    ...
    def do_service(self, app_name="", service="", return_type="string",
        ...
        def walk_using_urllib(url):
            ...
            response = urllib2.urlopen(req, data=data, timeout=timeout, cafile=cafile, capath=capath,
                                       cadefault=cadefault, context=context)
...
def _cleanup_before_exist():
    if len(__cache_registry_clients) > 0:
        logger.debug("cleaning up registry clients")
    ...
    if len(__cache_discovery_clients) > 0:
        logger.debug("cleaning up discovery clients")
