...
def abc_to_alphabetagamma(signals, method="Amplitude invariant"):
    ...
    """
    return _impl.abc_to_alphabetagamma(signals, method)


def alphabetagamma_to_abc(signals, method="Amplitude invariant"):
    ...
    1)  Amplitude invariant:
        ...
        c = -1/2 * a - sqrt(3)/2 * b + c

    2)  Uniform - Clarke's original:
        a = alpha + 1/sqrt(2) * gamma
        b = -1/2 * alpha + sqrt(3)/2 * beta + 1/sqrt(2) * gamma
        c = -1/2 * alpha - sqrt(3)/2 * beta + 1/sqrt(2) * gamma

    3)  Power invariant:
        a = sqrt(2/3) * (alpha + 1/sqrt(2) * gamma)
        b = sqrt(2/3) * (-1/2 * alpha + sqrt(3)/2 * beta + 1/sqrt(2) * gamma)
        c = sqrt(2/3) * (-1/2 * alpha - sqrt(3)/2 * beta + 1/sqrt(2) * gamma)
    ...
    """
    return _impl.alphabetagamma_to_abc(signals, method)
...
def alphabetagamma_to_dq0(signals, theta, alignment='d'):
    ...
    alignment : string
        It has two valid values: 'd' or 'q'. That is the way two chose which axis in rotating reference frame is
