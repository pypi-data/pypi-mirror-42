...
from performance import TODAY, DEBUG, config, lint, checkstyle
...
def send_mail(report):
    ...
    """ % (TODAY, report.success, report.duration, report.pkg, report.device,
           len(report.anrs), len(report.crashes), lints, checkstyles, report.max, report.avg)
    ...
    if os.path.exists(report.checkstyle_file):
        ...
        checkstyle_file["Content-Disposition"] = 'attachment; filename=%s' % os.path.split(report.lint_file)[1]
