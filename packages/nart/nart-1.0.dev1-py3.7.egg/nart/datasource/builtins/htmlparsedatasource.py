...
class HtmlParseDataSource(DataSource):
    ...
    def realtimekeywords(self) -> NartKeywords:
        ...
        rt_keyword_lists = soup.select('.PM_CL_realtimeKeyword_rolling span[class*=ah_k]')
        print(rt_keyword_lists)
        rt_keywords = [NartKeyword(rank=idx, keyword=keyword) for idx, keyword in enumerate(rt_keyword_lists, 1)]
