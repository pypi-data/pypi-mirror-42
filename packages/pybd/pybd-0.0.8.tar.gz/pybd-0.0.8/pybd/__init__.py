...
class PyBD(object):
    ...
    def __init__(self, user=bazean_postgres_username,
    ...
    def get_well_locations_by_ticker_and_state(self, ticker, state):
        ...
        Returns:
            (tuple of ndarray): Return a tuple of numpy arrays in the order `(latidude, longitude, api_number)`
    ...
    def get_production_from_api(self, api):
        ...
        Returns:
            (tuple of ndarray): Return a tuple of numpy arrays in the order `(dates, oil, gas, water)`, 
            where `dates` represents the date the production was reported, and `oil, gas, water` are the production 
            histories in units of bbls for oil and water and mcf for gas. Missing production data indicated with a `None` 
            return from the query is omitted.
