...
class _AbstractGraph:
    ...
    default_pen = QPen(QColor(0, 0, 0), 10)
    ...
    def __init__(self, data, name: str, parent: '_AbstractGraphicView', color: QColor = None):
        ...
        if color:
            ...
            self.default_pen = QPen(color, self.default_pen.widthF())
    ...
    def paint(self, painter: QPainter, echo=None):
        ...
        for x, y in self.data.items():
            if previous_x is not None:
                a = start + QPointF(previous_x * x_interval, -previous_y * y_interval)
                b = start + QPointF(x * x_interval, -y * y_interval)
...
class _HorizontalAxis(_Axis):
    ...
    def paint(self, painter):
        ...
        self._set_tick_interval(self.size() / self.tick_count() if self.tick_count() else 0)
