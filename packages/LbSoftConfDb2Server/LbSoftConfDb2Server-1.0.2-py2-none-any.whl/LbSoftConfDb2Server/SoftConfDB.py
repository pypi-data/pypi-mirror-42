...
def versionKey(v):
    ...
    For example:
    versionKey(1.2.3') (1, '.', 2, '.', 3)
    versionKey('v10r0') ('v', 10, 'r', 0)
    versionKey('1.2-a') (1, '.', 2, '-a')
...
def get_connection(host=None,
    ...
    if envhost:
        host = envhost
    if host is None:
        # TODO: Update this with production
        host = "lb-software-db-test.cern.ch"
...
class SoftConfDB():
    ...
    def getProjectNode(self, project):
        ...
        return Project.match(self.mNeoDB).where(name=project).first()
    ...
    def listReleaseStacks(self):
        ...
        query = "MATCH p=(u:ToRelease)- [:REQUIRES*]->(v:ToRelease) WHERE " \
        ...
        for top_plat in raw_data.keys():
            ...
            if requested_platforms and len(requested_platforms):
        ...
        for _, stack in raw_data.items():
            ...
            for i, filtered_stack in enumerate(filteredstacks):
                ...
                if len(intersec) > 0:
                    ...
                    import collections
                    duplicateProjs = [x for x, y in collections.Counter(
    ...
    def getPV(self, project, version):
        ...
        """
        project = project.upper()
        return ProjectVersion.match(self.mNeoDB).where(
            name="%s_%s" % (str(project).upper(), version)).first()
    ...
    def setPlatformTag(self, project, version, platform, tag):
        ...
        if not pv:
            raise Exception("%s %s not found" % (project, version))
        platform = Platform.match(self.mNeoDB).where(
            name="%s" % (str(platform))).first()
