...
DEPRECATION_ERROR = """
Incompatible SafeWISE library detected.

(Original error: {})
""".strip()

OUTDATED_FIRMWARE_ERROR = """
Your SafeWISE firmware is out of date. Update it with the following command:
  safewisectl firmware-update
Or visit https://wallet.safewise.io/
""".strip()
...
class SafeWISEClient:
    """SafeWISE client, a connection to a SafeWISE device.
    ...
    `safewiselib.transport.get_transport` to find one.
    ...
    See `safewiselib.ui` for details.
    ...
    def call(self, msg):
        ...
        while True:
            if isinstance(resp, messages.PinMatrixRequest):
                ...
                raise exceptions.SafeWISEFailure(resp)
            else:
                return resp
    ...
    def init_device(self):
        ...
        if not isinstance(resp, messages.Features):
            raise exceptions.SafeWISEException("Unexpected initial response")
        else:
            self.features = resp
    ...
    def ping(
        ...
        # We would like ping to work on any valid SafeWISEClient instance, but
...
class ProtocolMixin(object):
    """Fake mixin for old-style software that constructed SafeWISEClient class
    ...
    def __init__(self, *args, **kwargs):
        warnings.warn("SafeWISEClient mixins are not supported anymore")
...
class BaseClient:
    ...
    def __init__(self, *args, **kwargs):
        warnings.warn("SafeWISEClient mixins are not supported anymore")
        self.safewise_client = SafeWISEClient(*args, **kwargs)

    def __getattr__(self, key):
        return getattr(self.safewise_client, key)
