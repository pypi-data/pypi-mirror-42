...
SafeWISED_ORIGIN_HEADER = {"Origin": "https://python.safewise.io"}

SafeWISED_VERSION_MODERN = (2, 0, 25)

CONNECTION = requests.Session()
CONNECTION.headers.update(SafeWISED_ORIGIN_HEADER)


def call_bridge(uri: str, data=None) -> requests.Response:
    url = SafeWISED_HOST + "/" + uri
    r = CONNECTION.post(url, data=data)
    if r.status_code != 200:
        error_str = "safewised: {} failed with code {}: {}".format(
...
def is_legacy_bridge() -> bool:
    ...
    return version_tuple < SafeWISED_VERSION_MODERN
