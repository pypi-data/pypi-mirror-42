...
version_tuple = tuple(map(int, safewiselib.__version__.split(".")))
if not (1, 1) <= version_tuple < (2, 0):
    raise RuntimeError("safewiselib version mismatch (>= 1.1.x is required)")
...
def choose_device(devices):
    ...
    for d in devices:
        try:
            client = SafeWISEClient(d, ui=ClickUI())
        except IOError:
...
def main():
    ...
    client = SafeWISEClient(transport, ui=ClickUI())
    ...
    if not os.path.exists(passw_file):
        ...
        safewise_entropy = safewiselib.misc.get_entropy(client, 32)
        ...
        bip32_path = [10, 0]
        passw_encrypted = safewiselib.misc.encrypt_keyvalue(
    ...
    passw = safewiselib.misc.decrypt_keyvalue(
        client,
        data["bip32_path"],
        data["label"],
