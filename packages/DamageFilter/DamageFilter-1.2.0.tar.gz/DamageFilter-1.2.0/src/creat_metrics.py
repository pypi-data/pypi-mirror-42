...
def run_creat_metrics(args):
    ...
    for chr in bam.references:
        refseq[chr] = refa.fetch(chr)
    ...
    for rdaln in tqdm(bam,total=bam.mapped+bam.unmapped, unit=" reads", desc="Reading Bam(%s)"%args.input):
    #for rdaln in bam:
        ...
        if (rdaln.is_read1 and rdaln.is_reverse) or (rdaln.is_read2 and (not rdaln.is_reverse)):      ### R1- or R2+
            ...
            for t,n in rdaln.cigartuples:
                if t in [5,6]:   ## H, P
                    ...
                    refseq_rd = refseq[chr][ref_block-CONTEXT_SIZE:ref_block+n+CONTEXT_SIZE]            
                    for offset in range(n):
                        context =  refseq_rd[offset:(offset+contextFullLength)] 
                        if "N" in context:
                            continue   
                        readPos = read_block + offset
                        ...
                        if readBase == "N":
                            continue                        
                        tmpd[(readBase,context,"CON")] += 1
                        tmpd[(readBase,context,"FWD")] += 1               
                    ref_block += n
                    read_block += n
                else:
                    raise Exception("Error: cann't deal with cigar")                                                         
    ...
    for k in tmpd:
        r,c,t = k
        if t == "PRO" or t == "CON":
            pre_metrics[r][c][t] += tmpd[k]
        else:
            bait_metrics[r][c][t] += tmpd[k]
    ...
    fo1.close()                   
    fo2.close() 
    print "total time elapse: ",time.time() - startime
