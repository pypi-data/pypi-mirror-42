...
class RpcServer:
    ...
    async def _run(self, datapath, cartopath, dbhost, dbport, dbuser, dbpass, rpchost, rpcport):
        try:
            interface = Interface(self.logger, datapath, cartopath, dbhost, dbport, dbuser, dbpass)
            await interface.async_init()
            services = [interface]
            services = ServerReflection.extend(services)
            self.server = Server(services, loop=asyncio.get_event_loop())
            await self._serve(rpchost, rpcport)
        except:
