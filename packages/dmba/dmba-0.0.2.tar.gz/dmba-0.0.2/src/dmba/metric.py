...
def AIC_score(y_true, y_pred, model, df=None):
    ...
    return n * math.log(sse / n) + 2 * (p + 1) + constant


def BIC_score(y_true, y_pred, model, df=None):
    ...
    Input:
        ...
        df (optional): degrees of freedom of model
    """
    aic = AIC_score(y_true, y_pred, model, df=df)
    p = len(model.coef_) + 1 if df is None else df
    n = len(y_pred)
