...
import os
import hashlib
from creagraphenebase import bip38
...
from creaapi.exceptions import NoAccessApi
...
class Wallet(object):
    """ The wallet is meant to maintain access to private keys for
        ...
        * **Wallet Database**: Here, crea loads the keys from the
          locally stored wallet SQLite database (see ``storage.py``).
          To use this mode, simply call :class:`crea.crea.Crea` without the
          ``keys`` parameter
        * **Providing Keys**: Here, you can provide the keys for
          your accounts manually. All you need to do is add the wif
          keys for the accounts you want to use as a simple array
          using the ``keys`` parameter to :class:`crea.crea.Crea`.
        * **Force keys**: This more is for advanced users and
        ...
        .. code-block:: python
           ...
           crea.wallet.create("supersecret-passphrase")

        This will raise :class:`crea.exceptions.WalletExists` if you already have a wallet installed.
        ...
        **after** unlocking the wallet with the correct passphrase:
        ...
        .. code-block:: python
           ...
           crea = Crea()
           crea.wallet.unlock("supersecret-passphrase")
           crea.wallet.addPrivateKey("5xxxxxxxxxxxxxxxxxxxx")
    ...
    def setToken(self, loadtoken):
        """ This method is strictly only for in memory token that are
            passed to Wallet/Crea with the ``token`` argument
        """
        log.debug(
    ...
    def tryUnlockFromEnv(self):
        ...
        """
        password_storage = self.crea.config["password_storage"]
        if password_storage == "environment" and "UNLOCK" in os.environ:
            ...
            pwd = keyring.get_password("crea", "wallet")
            self.unlock(pwd)
        else:
    ...
    def _get_pub_from_wif(self, wif):
        ...
        """
        # it could be either graphenebase or crea so we can't check
        # the type directly
    ...
    def getAccountsFromPublicKey(self, pub):
        ...
        if self.crea.rpc.get_use_appbase():
            names = self.crea.rpc.get_key_references({'keys': [pub]}, api="account_by_key")["accounts"]
        else:
            names = self.crea.rpc.get_key_references([pub], api="account_by_key")
        for name in names:
            for i in name:
                yield i
    ...
    def getAllAccounts(self, pub):
        ...
        for name in self.getAccountsFromPublicKey(pub):
            try:
                account = Account(name, crea_instance=self.crea)
    ...
    def getKeyType(self, account, pub):
        """ Get key type
            ...
            :param crea.account.Account/dict account: Account data
    ...
    def getPublicKeys(self):
        ...
        """
        if self.keyStorage:
            return self.keyStorage.getPublicKeys(prefix=self.crea.prefix)
        else:
