...
from creaapi.exceptions import ApiNotSupported
    ...
    def refresh(self):
        ...
        if self.only_ops or self.only_virtual_ops:
            if self.crea.rpc.get_use_appbase():
                try:
                    ops = self.crea.rpc.get_ops_in_block({"block_num": self.identifier, 'only_virtual': self.only_virtual_ops}, api="account_history")["ops"]
                except ApiNotSupported:
                    ops = self.crea.rpc.get_ops_in_block(self.identifier, self.only_virtual_ops, api="condenser")
            else:
                ops = self.crea.rpc.get_ops_in_block(self.identifier, self.only_virtual_ops)
            if bool(ops):
            ...
            if self.crea.rpc.get_use_appbase():
                try:
                    block = self.crea.rpc.get_block({"block_num": self.identifier}, api="block")
                    if block and "block" in block:
                        block = block["block"]
                except ApiNotSupported:
                    block = self.crea.rpc.get_block(self.identifier, api="condenser")
            else:
                block = self.crea.rpc.get_block(self.identifier)
        if not block:
            raise BlockDoesNotExistsException("output: %s of identifier %s" % (str(block), str(self.identifier)))
        block = self._parse_json_data(block)
        super(Block, self).__init__(block, lazy=self.lazy, full=self.full, crea_instance=self.crea)
...
class BlockHeader(BlockchainObject):
    ...
    def refresh(self):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            block = self.crea.rpc.get_block_header({"block_num": self.identifier}, api="block")
            if "header" in block:
                block = block["header"]
        else:
            block = self.crea.rpc.get_block_header(self.identifier)
        if not block:
