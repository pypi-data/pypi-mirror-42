...
class Query(dict):
    """ Query to be used for all discussion queries
        ...
        .. testcode::
            ...
            from crea.discussions import Query
            query = Query(limit=10, tag="creait")
...
class Discussions(object):
    ...
    """
    def __init__(self, lazy=False, crea_instance=None):
    ...
    def get_discussions(self, discussion_type, discussion_query, limit=1000):
        """ Get Discussions
            ...
            .. testcode::
                ...
                from crea.discussions import Query, Discussions
                query = Query(limit=51, tag="creait")
        ...
        while (query_count < limit and found_more_than_start_entry):
            ...
            if discussion_type == "trending":
                dd = Discussions_by_trending(discussion_query, crea_instance=self.crea, lazy=self.lazy)
                ...
                dd = Discussions_by_author_before_date(author=discussion_query["author"],
                                                       ...
                                                       crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "payout":
                dd = Comment_discussions_by_payout(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "post_payout":
                dd = Post_discussions_by_payout(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "created":
                dd = Discussions_by_created(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "active":
                dd = Discussions_by_active(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "cashout":
                dd = Discussions_by_cashout(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "votes":
                dd = Discussions_by_votes(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "children":
                dd = Discussions_by_children(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "hot":
                dd = Discussions_by_hot(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "feed":
                dd = Discussions_by_feed(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "blog":
                dd = Discussions_by_blog(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "comments":
                dd = Discussions_by_comments(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "promoted":
                dd = Discussions_by_promoted(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "replies":
                dd = Replies_by_last_update(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            elif discussion_type == "tags":
                dd = Trending_tags(discussion_query, crea_instance=self.crea, lazy=self.lazy)
            else:
...
class Discussions_by_trending(list):
    """ Get Discussions by trending
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_trending
            q = Query(limit=10, tag="crea")
            for h in Discussions_by_trending(q):
                print(h)

    """
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        self.crea = crea_instance or shared_crea_instance()
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_discussions_by_trending(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_discussions_by_trending(discussion_query)
        super(Discussions_by_trending, self).__init__(
            [
                Comment(x, lazy=lazy, crea_instance=self.crea)
                for x in posts
            ]
        )
...
class Discussions_by_author_before_date(list):
    """ Get Discussions by author before date
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_author_before_date
            for h in Discussions_by_author_before_date(limit=10, author="gtg"):
                print(h)

    """
    def __init__(self, author="", start_permlink="", before_date="1970-01-01T00:00:00", limit=100, lazy=False, crea_instance=None):
        self.crea = crea_instance or shared_crea_instance()
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            discussion_query = {"author": author, "start_permlink": start_permlink, "before_date": before_date, "limit": limit}
            posts = self.crea.rpc.get_discussions_by_author_before_date(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_discussions_by_author_before_date(author, start_permlink, before_date, limit)
...
class Comment_discussions_by_payout(list):
    """ Get comment_discussions_by_payout
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Comment_discussions_by_payout
            q = Query(limit=10)
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_comment_discussions_by_payout(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_comment_discussions_by_payout(discussion_query)
...
class Post_discussions_by_payout(list):
    """ Get post_discussions_by_payout
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Post_discussions_by_payout
            q = Query(limit=10)
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_post_discussions_by_payout(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_post_discussions_by_payout(discussion_query)
...
class Discussions_by_created(list):
    """ Get discussions_by_created
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_created
            q = Query(limit=10)
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_discussions_by_created(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_discussions_by_created(discussion_query)
...
class Discussions_by_active(list):
    """ get_discussions_by_active
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_active
            q = Query(limit=10)
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_discussions_by_active(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_discussions_by_active(discussion_query)
...
class Discussions_by_cashout(list):
    """ Get discussions_by_cashout. This query seems to be broken at the moment.
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_cashout
            q = Query(limit=10)
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_discussions_by_cashout(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_discussions_by_cashout(discussion_query)
...
class Discussions_by_votes(list):
    """ Get discussions_by_votes
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_votes
            q = Query(limit=10)
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_discussions_by_votes(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_discussions_by_votes(discussion_query)
...
class Discussions_by_children(list):
    """ Get discussions by children
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_children
            q = Query(limit=10)
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_discussions_by_children(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_discussions_by_children(discussion_query)
...
class Discussions_by_hot(list):
    """ Get discussions by hot
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_hot
            q = Query(limit=10, tag="crea")
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_discussions_by_hot(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_discussions_by_hot(discussion_query)
...
class Discussions_by_feed(list):
    """ Get discussions by feed
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_feed
            q = Query(limit=10, tag="crea")
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_discussions_by_feed(discussion_query, api="tags")['discussions']
        else:
            # limit = discussion_query["limit"]
            # account = discussion_query["tag"]
            # entryId = 0
            # posts = self.crea.rpc.get_feed(account, entryId, limit, api='follow')["comment"]
            posts = self.crea.rpc.get_discussions_by_feed(discussion_query)
...
class Discussions_by_blog(list):
    """ Get discussions by blog
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_blog
            q = Query(limit=10)
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_discussions_by_blog(discussion_query, api="tags")['discussions']
        else:
            # limit = discussion_query["limit"]
            # account = discussion_query["tag"]
            # entryId = 0
            # posts = self.crea.rpc.get_feed(account, entryId, limit, api='follow')
            posts = self.crea.rpc.get_discussions_by_blog(discussion_query)
...
class Discussions_by_comments(list):
    """ Get discussions by comments
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_comments
            q = Query(limit=10, start_author="creait", start_permlink="firstpost")
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_discussions_by_comments(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_discussions_by_comments(discussion_query)
...
class Discussions_by_promoted(list):
    """ Get discussions by promoted
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Discussions_by_promoted
            q = Query(limit=10, tag="crea")
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_discussions_by_promoted(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_discussions_by_promoted(discussion_query)
...
class Replies_by_last_update(list):
    """ Returns a list of replies by last update
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Replies_by_last_update
            q = Query(limit=10, start_parent_author="creait", start_permlink="firstpost")
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            posts = self.crea.rpc.get_replies_by_last_update(discussion_query, api="tags")['discussions']
        else:
            posts = self.crea.rpc.get_replies_by_last_update(discussion_query["start_parent_author"], discussion_query["start_permlink"], discussion_query["limit"])
...
class Trending_tags(list):
    """ Returns the list of trending tags.
        ...
        .. testcode::
            ...
            from crea.discussions import Query, Trending_tags
    ...
    def __init__(self, discussion_query, lazy=False, crea_instance=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
            tags = self.crea.rpc.get_trending_tags(discussion_query, api="tags")['tags']
        else:
            tags = self.crea.rpc.get_trending_tags(discussion_query["start_tag"], discussion_query["limit"], api="tags")
