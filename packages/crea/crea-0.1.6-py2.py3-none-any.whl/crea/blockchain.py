...
class Blockchain(object):
    ...
    def __init__(
        ...
        self.block_interval = self.crea.get_block_interval()
    ...
    def get_transaction(self, transaction_id):
        ...
        if self.crea.rpc.get_use_appbase():
            ret = self.crea.rpc.get_transaction({'id': transaction_id}, api="account_history")
        else:
            ret = self.crea.rpc.get_transaction(transaction_id, api="database")
        return ret
    ...
    def get_transaction_hex(self, transaction):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            ret = self.crea.rpc.get_transaction_hex({'trx': transaction}, api="database")["hex"]
        else:
            ret = self.crea.rpc.get_transaction_hex(transaction, api="database")
        return ret
    ...
    def get_current_block_num(self):
        ...
        """
        props = self.crea.get_dynamic_global_properties(False)
        if props is None:
    ...
    def get_estimated_block_num(self, date, estimateForwards=False, accurate=True):
        ...
        if estimateForwards:
            block_offset = 10
            first_block = Block(block_offset, crea_instance=self.crea)
        ...
        if accurate:
            ...
            while block_time_diff.total_seconds() > self.block_interval or block_time_diff.total_seconds() < -self.block_interval:
                block = Block(block_number, crea_instance=self.crea)
    ...
    def blocks(self, start=None, stop=None, max_batch_size=None, threading=False, thread_num=8, only_ops=False, only_virtual_ops=False):
        """ Yields blocks starting from ``start``.
            ...
            .. note:: If you want instant confirmation, you need to instantiate
                      class:`crea.blockchain.Blockchain` with
        ...
        if threading:
            crea_instance = [self.crea]
            nodelist = self.crea.rpc.nodes.export_working_nodes()
            for i in range(thread_num - 1):
                crea_instance.append(stm.Crea(node=nodelist,
                                                num_retries=self.crea.rpc.num_retries,
                                                num_retries_call=self.crea.rpc.num_retries_call,
                                                timeout=self.crea.rpc.timeout))
            ...
            if threading and not head_block_reached:
                ...
                for blocknum in range(start, head_block + 1, thread_num):
                    ...
                    if FUTURES_MODULE is not None:
                        futures = []
                    block_num_list = []
                    # freeze = self.crea.rpc.nodes.freeze_current_node
                    num_retries = self.crea.rpc.nodes.num_retries
                    # self.crea.rpc.nodes.freeze_current_node = True
                    self.crea.rpc.nodes.num_retries = thread_num
                    error_cnt = self.crea.rpc.nodes.node.error_cnt
                    while i < thread_num and blocknum + i <= head_block:
                        block_num_list.append(blocknum + i)
                        results = []
                        if FUTURES_MODULE is not None:
                            futures.append(pool.submit(Block, blocknum + i, only_ops=only_ops, only_virtual_ops=only_virtual_ops, crea_instance=crea_instance[i]))
                        else:
                            pool.enqueue(Block, blocknum + i, only_ops=only_ops, only_virtual_ops=only_virtual_ops, crea_instance=crea_instance[i])
                        i += 1
                    if FUTURES_MODULE is not None:
                        try:
                        ...
                        pool.abort()
                    self.crea.rpc.nodes.num_retries = num_retries
                    # self.crea.rpc.nodes.freeze_current_node = freeze
                    new_error_cnt = self.crea.rpc.nodes.node.error_cnt
                    self.crea.rpc.nodes.node.error_cnt = error_cnt
                    if new_error_cnt > error_cnt:
                        self.crea.rpc.nodes.node.error_cnt += 1
                    #    self.crea.rpc.next()
                    ...
                    while len(missing_block_num) > 0:
                        for blocknum in missing_block_num:
                            try:
                                block = Block(blocknum, only_ops=only_ops, only_virtual_ops=only_virtual_ops, crea_instance=self.crea)
                ...
                for blocknumblock in range(start, head_block + 1, batches):
                    ...
                    for blocknum in range(blocknumblock, blocknumblock + batches - 1):
                        if only_virtual_ops:
                            if self.crea.rpc.get_use_appbase():
                                # self.crea.rpc.get_ops_in_block({"block_num": blocknum, 'only_virtual': only_virtual_ops}, api="account_history", add_to_queue=True)
                                self.crea.rpc.get_ops_in_block(blocknum, only_virtual_ops, add_to_queue=True)
                            else:
                                self.crea.rpc.get_ops_in_block(blocknum, only_virtual_ops, add_to_queue=True)
                        else:
                            if self.crea.rpc.get_use_appbase():
                                self.crea.rpc.get_block({"block_num": blocknum}, api="block", add_to_queue=True)
                            else:
                                self.crea.rpc.get_block(blocknum, add_to_queue=True)
                    ...
                    if latest_block <= head_block:
                        if only_virtual_ops:
                            if self.crea.rpc.get_use_appbase():
                                # self.crea.rpc.get_ops_in_block({"block_num": blocknum, 'only_virtual': only_virtual_ops}, api="account_history", add_to_queue=False)
                                block_batch = self.crea.rpc.get_ops_in_block(blocknum, only_virtual_ops, add_to_queue=False)
                            else:
                                block_batch = self.crea.rpc.get_ops_in_block(blocknum, only_virtual_ops, add_to_queue=False)
                        else:
                            if self.crea.rpc.get_use_appbase():
                                block_batch = self.crea.rpc.get_block({"block_num": latest_block}, api="block", add_to_queue=False)
                            else:
                                block_batch = self.crea.rpc.get_block(latest_block, add_to_queue=False)
                        ...
                        for block in block_batch:
                            ...
                            if self.crea.rpc.get_use_appbase():
                                if only_virtual_ops:
                                    ...
                                    block = block["block"]
                            block = Block(block, only_ops=only_ops, only_virtual_ops=only_virtual_ops, crea_instance=self.crea)
    ...
    def wait_for_and_get_block(self, block_number, blocks_waiting_for=None, only_ops=False, only_virtual_ops=False, block_number_check_cnt=-1, last_current_block_num=None):
        ...
        while (block is None or block.block_num is None or int(block.block_num) != block_number) and (block_number_check_cnt < 0 or cnt < block_number_check_cnt):
            try:
                block = Block(block_number, only_ops=only_ops, only_virtual_ops=only_virtual_ops, crea_instance=self.crea)
                cnt += 1
    ...
    def ops_statistics(self, start, stop=None, add_to_ops_stat=None, with_virtual_ops=True, verbose=False):
        ...
        if add_to_ops_stat is None:
            ...
            ops_stat = creabase.operationids.operations.copy()
            for key in ops_stat:
                ops_stat[key] = 0
        else:
        ...
        """ Yield specific operations (e.g. comments) only
            ...
            .. code-block:: js

                {
                    ...
                    'to': 'thundercurator',
                    'amount': '0.080 CBD',
                    'memo': 'https://creary.net/lofi/@johngreenfield/lofi-joji-yeah-right',
    ...
    def get_all_accounts(self, start='', stop='', steps=1e3, limit=-1, **kwargs):
        ...
        if not self.crea.is_connected():
            raise OfflineHasNoRPCException("No RPC available in offline mode!")
        if self.crea.rpc.get_use_appbase() and start == "":
            lastname = None
        else:
            lastname = start
        ...
        while True:
            if self.crea.rpc.get_use_appbase():
                ret = self.crea.rpc.list_accounts({'start': lastname, 'limit': steps, 'order': 'by_name'}, api="database")["accounts"]
            else:
                ret = self.crea.rpc.lookup_accounts(lastname, steps)
            for account in ret:
    ...
    def get_account_count(self):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            ret = self.crea.rpc.get_account_count(api="condenser")
        else:
            ret = self.crea.rpc.get_account_count()
        return ret
    ...
    def get_account_reputations(self, start='', stop='', steps=1e3, limit=-1, **kwargs):
        ...
        while True:
            if self.crea.rpc.get_use_appbase():
                ret = self.crea.rpc.get_account_reputations({'account_lower_bound': lastname, 'limit': steps}, api="follow")["reputations"]
            else:
                ret = self.crea.rpc.get_account_reputations(lastname, steps, api="follow")
            for account in ret:
                if isinstance(account, dict):
    ...
    def get_similar_account_names(self, name, limit=5):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            account = self.crea.rpc.list_accounts({'start': name, 'limit': limit}, api="database")
            if bool(account):
                return account["accounts"]
        else:
            return self.crea.rpc.lookup_accounts(name, limit)
    ...
    def find_rc_accounts(self, name):
        ...
        if isinstance(name, list):
            account = self.crea.rpc.find_rc_accounts({'accounts': name}, api="rc")
            if bool(account):
    ...
    def list_change_recovery_account_requests(
        ...
        if not self.crea.is_connected():
            return None
        self.crea.rpc.set_next_node_on_empty_reply(False)
        requests = self.crea.rpc.list_change_recovery_account_requests(
    ...
    def find_change_recovery_account_requests(self, accounts):
        ...
        if isinstance(accounts, str):
            accounts = [accounts]
        requests = self.crea.rpc.find_change_recovery_account_requests(
