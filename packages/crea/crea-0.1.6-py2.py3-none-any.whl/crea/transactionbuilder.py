...
class TransactionBuilder(dict):
    """ This class simplifies the creation of transactions by adding
        ...
        :param Crea crea_instance: If not set, shared_crea_instance() is used
        ...
        .. testcode::
           ...
           stm = Crea(nobroadcast=True, keys={'active': wif})
           tx = TransactionBuilder(crea_instance=stm)
           transfer = {"from": "test", "to": "test1", "amount": "1 CREA", "memo": ""}
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.set_expiration(kwargs.get("expiration", self.crea.expiration))
    ...
    def list_operations(self):
        ...
        if self.crea.is_connected() and self.crea.rpc.get_use_appbase():
            ...
            appbase = False
        return [Operation(o, appbase=appbase, prefix=self.crea.prefix) for o in self.ops]
    ...
    def json(self, with_prefix=False):
        ...
        json_dict = dict(self)
        if with_prefix:
            json_dict["prefix"] = self.crea.prefix
        return json_dict
    ...
    def appendSigner(self, account, permission):
        ...
        if self.crea.wallet.locked():
            raise WalletLocked()
        if self.crea.use_sc2 and self.crea.creaconnect is not None:
            self.crea.creaconnect.set_username(account["name"], permission)
            return
        ...
        def fetchkeys(account, perm, level=0):
            ...
            for authority in account[perm]["key_auths"]:
                try:
                    wif = self.crea.wallet.getPrivateKeyForPublicKey(
                        authority[0])
                    if wif:
            ...
            if sum([x[1] for x in r]) < required_treshold:
                ...
                for authority in account[perm]["account_auths"]:
                    auth_account = Account(
                        authority[0], crea_instance=self.crea)
        ...
        if account["name"] not in self.signing_accounts:
            ...
            if isinstance(account, PublicKey):
                self.wifs.add(
                    self.crea.wallet.getPrivateKeyForPublicKey(
                        str(account)
                    )
                )
            else:
    ...
    def appendWif(self, wif):
        ...
        if wif:
            try:
                PrivateKey(wif, prefix=self.crea.prefix)
                self.wifs.add(wif)
            except:
    ...
    def constructTx(self, ref_block_num=None, ref_block_prefix=None):
        ...
        for op in self.ops:
            ...
            ops.extend([Operation(op, appbase=appbase, prefix=self.crea.prefix)])

        # We no wrap everything into an actual transaction
        expiration = formatTimeFromNow(
            self.expiration or self.crea.expiration
        )
    ...
    def sign(self, reconstruct_tx=True):
        ...
        if "operations" not in self or not self["operations"]:
            return
        if self.crea.use_sc2:
            return
        # We need to set the default prefix, otherwise pubkeys are
        # presented wrongly!
        if self.crea.rpc is not None:
            operations.default_prefix = (
                self.crea.chain_params["prefix"])
        elif "blockchain" in self:
            operations.default_prefix = self["blockchain"]["prefix"]

        try:
            signedtx = Signed_Transaction(**self.json(with_prefix=True))
            signedtx.add_custom_chains(self.crea.custom_chains)
        except:
        ...
        signedtx.sign(self.wifs, chain=self.crea.chain_params)
    ...
    def verify_authority(self):
        ...
        try:
            ...
            if self.crea.rpc.get_use_appbase():
                ...
                args = self.json()
            ret = self.crea.rpc.verify_authority(args, api="database")
            if not ret:
    ...
    def get_potential_signatures(self):
        ...
        if self.crea.rpc.get_use_appbase():
            ...
            args = self.json()
        ret = self.crea.rpc.get_potential_signatures(args, api="database")
        if 'keys' in ret:
            ret = ret["keys"]
        return ret
    ...
    def get_transaction_hex(self):
        ...
        if self.crea.rpc.get_use_appbase():
            ...
            args = self.json()
        ret = self.crea.rpc.get_transaction_hex(args, api="database")
        if 'hex' in ret:
            ret = ret["hex"]
        return ret
    ...
    def get_required_signatures(self, available_keys=list()):
        ...
        if self.crea.rpc.get_use_appbase():
            ...
            ret = self.crea.rpc.get_required_signatures(args, api="database")
        else:
            ret = self.crea.rpc.get_required_signatures(self.json(), available_keys, api="database")
    ...
    def broadcast(self, max_block_age=-1):
        ...
        try:
            self.crea.rpc.set_next_node_on_empty_reply(False)
            if self.crea.use_sc2:
                ret = self.crea.creaconnect.broadcast(self["operations"])
            elif self.crea.blocking:
                ret = self.crea.rpc.broadcast_transaction_synchronous(
                    args, api=broadcast_api)
                if "trx" in ret:
                    ret.update(**ret.get("trx"))
            else:
                self.crea.rpc.broadcast_transaction(
                    args, api=broadcast_api)
        except Exception as e:
    ...
    def addSigningInformation(self, account, permission, reconstruct_tx=False):
        ...
        if not self._is_constructed() or (self._is_constructed() and reconstruct_tx):
            self.constructTx()
        self["blockchain"] = self.crea.chain_params
