...
from creaapi.exceptions import ApiNotSupported, MissingRequiredActiveAuthority
...
from creagraphenebase.account import PrivateKey, PublicKey, PasswordKey
from creagraphenebase.py23 import bytes_types, integer_types, string_types, text_type
from crea.constants import CREA_VOTE_REGENERATION_SECONDS, CREA_1_PERCENT, CREA_100_PERCENT, CREA_VOTING_MANA_REGENERATION_SECONDS
...
class Account(BlockchainObject):
    """ This class allows to easily access Account data
        ...
        :rtype: dictionary
        :raises crea.exceptions.AccountDoesNotExistsException: if account
                does not exist
    ...
    def refresh(self):
        ...
        if not self.crea.is_connected():
            return
        self.crea.rpc.set_next_node_on_empty_reply(self.crea.rpc.get_use_appbase())
        if self.crea.rpc.get_use_appbase():
                account = self.crea.rpc.find_accounts({'accounts': [self.identifier]}, api="database")
        else:
            if self.full:
                account = self.crea.rpc.get_accounts(
                    [self.identifier], api="database")
            else:
                account = self.crea.rpc.lookup_account_names(
                    [self.identifier], api="database")
        if self.crea.rpc.get_use_appbase() and "accounts" in account:
        ...
        # self.crea.refresh_data()

        super(Account, self).__init__(account, id_item="name", lazy=self.lazy, full=self.full, crea_instance=self.crea)
    ...
    def _parse_json_data(self, account):
        ...
        for p in amounts:
            if p in account and isinstance(account.get(p), (string_types, list, dict)):
                account[p] = Amount(account[p], crea_instance=self.crea)
        return account
    ...
    def get_rc(self):
        ...
        b = Blockchain(crea_instance=self.crea)
    ...
    def get_rc_manabar(self):
        ...
        current_mana = int(last_mana + diff_in_seconds * max_mana / CREA_VOTING_MANA_REGENERATION_SECONDS)
        ...
        if max_mana > 0:
            ...
            current_pct = 0
        max_rc_creation_adjustment = Amount(rc_param["max_rc_creation_adjustment"], crea_instance=self.crea)
    ...
    def get_similar_account_names(self, limit=5):
        """ Returns ``limit`` account names similar to the current account
            ...
            This is a wrapper around :func:`crea.blockchain.Blockchain.get_similar_account_names()`
    ...
    @property
    def sp(self):
        """ Returns the accounts Crea Power
        """
        return self.get_crea_power()
    ...
    def print_info(self, force_refresh=False, return_str=False, use_table=False, **kwargs):
        ...
        try:
            ...
            rc = self.get_rc()
            rc_calc = RC(crea_instance=self.crea)
        except:
            rc_mana = None
            rc_calc = None
        ...
        if use_table:
            ...
            t.add_row(["Vote Value", "%.2f $" % (self.get_voting_value_CBD())])
            t.add_row(["Last vote", "%s ago" % last_vote_time_str])
            t.add_row(["Full in ", "%s" % (self.get_recharge_time_str())])
            t.add_row(["Crea Power", "%.2f %s" % (self.get_crea_power(), self.crea.crea_symbol)])
            ...
            ret += " VP = %.2f $\n" % (self.get_voting_value_CBD())
            ret += "full in %s \n" % (self.get_recharge_time_str())
            ret += "--- Balance ---\n"
            ret += "%.2f SP, " % (self.get_crea_power())
    ...
    def get_reputation(self):
        """ Returns the account reputation in the (creait) normalized form
        """
        if not self.crea.is_connected():
            return None
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            rep = self.crea.rpc.get_account_reputations({'account_lower_bound': self["name"], 'limit': 1}, api="follow")['reputations']
            if len(rep) > 0:
    ...
    def get_manabar(self):
        ...
        if max_mana == 0:
            props = self.crea.get_chain_properties()
            required_fee_crea = Amount(props["account_creation_fee"], crea_instance=self.crea)
            max_mana = int(self.crea.sp_to_vests(required_fee_crea))
    ...
    def get_voting_power(self, with_regeneration=True):
        ...
        if "voting_manabar" in self:
            ...
            if with_regeneration:
                ...
                regenerated_vp = diff_in_seconds * CREA_100_PERCENT / CREA_VOTE_REGENERATION_SECONDS / 100
            else:
                regenerated_vp = 0
    ...
    def get_crea_power(self, onlyOwnSP=False):
        """ Returns the account crea power
        """
        return self.crea.vests_to_sp(self.get_vests(only_own_vests=onlyOwnSP))

    def get_voting_value_CBD(self, voting_weight=100, voting_power=None, crea_power=None, not_broadcasted_vote=True):
        """ Returns the account voting value in CBD
        """
        if voting_power is None:
            voting_power = self.get_voting_power()
        if crea_power is None:
            sp = self.get_crea_power()
        else:
            sp = crea_power

        VoteValue = self.crea.sp_to_sbd(sp, voting_power=voting_power * 100, vote_pct=voting_weight * 100, not_broadcasted_vote=not_broadcasted_vote)
        return VoteValue

    def get_vote_pct_for_CBD(self, sbd, voting_power=None, crea_power=None, not_broadcasted_vote=True):
        """ Returns the voting percentage needed to have a vote worth a given number of CBD.

            If the returned number is bigger than 10000 or smaller than -10000,
            the given CBD value is too high for that account

            :param sbd: The amount of CBD in vote value
        ...
        if voting_power is None:
            voting_power = self.get_voting_power()
        if crea_power is None:
            crea_power = self.get_crea_power()

        if isinstance(sbd, Amount):
            sbd = Amount(sbd, crea_instance=self.crea)
        elif isinstance(sbd, string_types):
            sbd = Amount(sbd, crea_instance=self.crea)
        else:
            sbd = Amount(sbd, self.crea.sbd_symbol, crea_instance=self.crea)
        if sbd['symbol'] != self.crea.sbd_symbol:
            raise AssertionError('Should input CBD, not any other asset!')

        vote_pct = self.crea.rshares_to_vote_pct(self.crea.sbd_to_rshares(sbd, not_broadcasted_vote=not_broadcasted_vote), voting_power=voting_power * 100, crea_power=crea_power)
        return vote_pct
    ...
    def get_recharge_timedelta(self, voting_power_goal=100, starting_voting_power=None):
        ...
        if missing_vp < 0:
            return 0
        recharge_seconds = missing_vp * 100 * CREA_VOTING_MANA_REGENERATION_SECONDS / CREA_100_PERCENT
    ...
    def get_manabar_recharge_timedelta(self, manabar, recharge_pct_goal=100):
        ...
        if missing_rc_pct < 0:
            return 0
        recharge_seconds = missing_rc_pct * 100 * CREA_VOTING_MANA_REGENERATION_SECONDS / CREA_100_PERCENT
        return timedelta(seconds=recharge_seconds)
    ...
    def get_feed(self, start_entry_id=0, limit=100, raw_data=False, short_entries=False, account=None):
        """ Returns a list of items in an account’s feed
            ...
            .. code-block:: python
                ...
                >>> from crea import Crea
                >>> stm = Crea("https://cread.minnowsupportproject.org")
                >>> account = Account("creait", crea_instance=stm)
        ...
        if not self.crea.is_connected():
            return None
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if raw_data and short_entries and self.crea.rpc.get_use_appbase():
            return [
                c for c in self.crea.rpc.get_feed_entries({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')["feed"]
            ]
        elif raw_data and short_entries and not self.crea.rpc.get_use_appbase():
            return [
                c for c in self.crea.rpc.get_feed_entries(account, start_entry_id, limit, api='follow')
            ]
        elif raw_data and self.crea.rpc.get_use_appbase():
            return [
                c for c in self.crea.rpc.get_feed({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')["feed"]
            ]
        elif raw_data and not self.crea.rpc.get_use_appbase():
            return [
                c for c in self.crea.rpc.get_feed(account, start_entry_id, limit, api='follow')
            ]
        elif not raw_data and self.crea.rpc.get_use_appbase():
            from .comment import Comment
            return [
                Comment(c['comment'], crea_instance=self.crea) for c in self.crea.rpc.get_feed({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')["feed"]
            ]
        else:
            from .comment import Comment
            return [
                Comment(c['comment'], crea_instance=self.crea) for c in self.crea.rpc.get_feed(account, start_entry_id, limit, api='follow')
            ]
    ...
    def get_blog_entries(self, start_entry_id=0, limit=100, raw_data=True,
        ...
        """ Returns the list of blog entries for an account
            ...
            .. code-block:: python
                ...
                creait - firstpost - creait - 1970-01-01T00:00:00
    ...
    def get_blog(self, start_entry_id=0, limit=100, raw_data=False, short_entries=False, account=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if raw_data and short_entries and self.crea.rpc.get_use_appbase():
            ret = self.crea.rpc.get_blog_entries({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')
            ...
            return [
                c for c in self.crea.rpc.get_blog_entries(account, start_entry_id, limit, api='follow')
            ]
        elif raw_data and self.crea.rpc.get_use_appbase():
            ret = self.crea.rpc.get_blog({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')
            if isinstance(ret, dict) and "blog" in ret:
                ret = ret["blog"]            
            return [
                c for c in ret
            ...
            return [
                c for c in self.crea.rpc.get_blog(account, start_entry_id, limit, api='follow')
            ]
        elif not raw_data and self.crea.rpc.get_use_appbase():
            from .comment import Comment
            ret = self.crea.rpc.get_blog({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')
            if isinstance(ret, dict) and "blog" in ret:
                ret = ret["blog"]
            return [
                Comment(c["comment"], crea_instance=self.crea) for c in ret
            ]                
        else:
            from .comment import Comment
            return [
                Comment(c["comment"], crea_instance=self.crea) for c in self.crea.rpc.get_blog(account, start_entry_id, limit, api='follow')
            ]
    ...
    def get_blog_authors(self, account=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            return self.crea.rpc.get_blog_authors({'blog_account': account}, api='follow')['blog_authors']
        else:
            return self.crea.rpc.get_blog_authors(account, api='follow')
    ...
    def get_follow_count(self, account=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            return self.crea.rpc.get_follow_count({'account': account}, api='follow')
        else:
            return self.crea.rpc.get_follow_count(account, api='follow')
    ...
    def get_followers(self, raw_name_list=True, limit=100):
        ...
        if raw_name_list:
            ...
            return Accounts(name_list, crea_instance=self.crea)
        ...
        while limit_reached:
            ...
            if self.crea.rpc.get_use_appbase():
                ...
                if direction == "follower":
                    followers = self.crea.rpc.get_followers(query, api='follow')
                    if isinstance(followers, dict) and 'followers' in followers:
                        followers = followers['followers']
                elif direction == "following":
                    followers = self.crea.rpc.get_following(query, api='follow')
                    if isinstance(followers, dict) and 'following' in followers:
                        followers = followers['following']         
            else:
                if direction == "follower":
                    followers = self.crea.rpc.get_followers(self.name, last_user, what, limit, api='follow')
                elif direction == "following":
                    followers = self.crea.rpc.get_following(self.name, last_user, what, limit, api='follow')
            if cnt == 0:
    ...
    def reward_balances(self):
        amount_list = ["reward_crea_balance", "reward_sbd_balance", "reward_vesting_balance"]
        rewards_amount = []
        for amount in amount_list:
            if amount in self:
        ...
        """ Returns all account balances as dictionary
            ...
            Sample output:
                ...
                .. code-block:: js
                    ...
                    {
                        'available': [102.985 CREA, 0.008 CBD, 146273.695970 VESTS],
                        'savings': [0.000 CREA, 0.000 CBD],
                        'rewards': [0.000 CREA, 0.000 CBD, 0.000000 VESTS],
                        'total': [102.985 CREA, 0.008 CBD, 146273.695970 VESTS]
                    }
    ...
    def get_balance(self, balances, symbol):
        """ Obtain the balance of a specific Asset. This call returns instances of
            :class:`crea.amount.Amount`. Available balance types:
            ...
            * "total"
            ...
            :param symbol: Can be "CBD", "CREA" or "VESTS
            :type symbol: str, dict

            .. code-block:: python

                >>> from crea.account import Account
                >>> account = Account("crea.app")
                >>> account.get_balance("rewards", "CBD")
                0.000 CBD
        ...
        for b in balances:
            if b["symbol"] == symbol:
                return b
        return Amount(0, symbol, crea_instance=self.crea)
    ...
    def interest(self):
        ...
        interest_rate = self.crea.get_dynamic_global_properties()[
    ...
    def get_account_bandwidth(self, bandwidth_type=1, account=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            # return self.crea.rpc.get_account_bandwidth({'account': account, 'type': 'post'}, api="witness")
            return self.crea.rpc.get_account_bandwidth(account, bandwidth_type)
        else:
            return self.crea.rpc.get_account_bandwidth(account, bandwidth_type)
    ...
    def get_bandwidth(self):
        ...
        """
        account = self["name"]
        global_properties = self.crea.get_dynamic_global_properties()
        try:
            reserve_ratio = self.crea.get_reserve_ratio()
        except:
            return {"used": 0,
                    "allocated": 0}
        ...
        total_vesting_shares = Amount(global_properties["total_vesting_shares"], crea_instance=self.crea).amount
        ...
        if not not self.crea.is_connected() and self.crea.rpc.get_use_appbase():
    ...
    def get_owner_history(self, account=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            return self.crea.rpc.find_owner_histories({'owner': account}, api="database")['owner_auths']
        else:
            return self.crea.rpc.get_owner_history(account)

    def get_conversion_requests(self, account=None):
        """ Returns a list of CBD conversion request
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            return self.crea.rpc.find_sbd_conversion_requests({'account': account}, api="database")['requests']
        else:
            return self.crea.rpc.get_conversion_requests(account)
    ...
    def get_vesting_delegations(self, start_account="", limit=100, account=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            delegations = self.crea.rpc.list_vesting_delegations(
            ...
            return self.crea.rpc.get_vesting_delegations(account, start_account, limit)
    ...
    def get_withdraw_routes(self, account=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            return self.crea.rpc.find_withdraw_vesting_routes({'account': account, 'order': 'by_withdraw_route'}, api="database")['routes']
        else:
            return self.crea.rpc.get_withdraw_routes(account, 'all')
    ...
    def get_savings_withdrawals(self, direction="from", account=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            return self.crea.rpc.find_savings_withdrawals({'account': account}, api="database")['withdrawals']
        elif direction == "from":
            return self.crea.rpc.get_savings_withdraw_from(account)
        elif direction == "to":
            return self.crea.rpc.get_savings_withdraw_to(account)
    ...
    def get_recovery_request(self, account=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            return self.crea.rpc.find_account_recovery_requests({'account': account}, api="database")['requests']
        else:
            return self.crea.rpc.get_recovery_request(account)
    ...
    def get_escrow(self, escrow_id=0, account=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            return self.crea.rpc.find_escrows({'from': account}, api="database")['escrows']
        else:
            return self.crea.rpc.get_escrow(account, escrow_id)
    ...
    def verify_account_authority(self, keys, account=None):
        ...
        try:
            if self.crea.rpc.get_use_appbase():
                return self.crea.rpc.verify_account_authority({'account': account, 'signers': keys}, api="database")
            else:
                return self.crea.rpc.verify_account_authority(account, keys)
    ...
    def get_tags_used_by_author(self, account=None):
        """ Returns a list of tags used by an author.
            ...
            .. code-block:: python
                ...
                >>> from crea import Crea
                >>> stm = Crea("https://cread.minnowsupportproject.org")
                >>> account = Account("crea.app", crea_instance=stm)
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            return self.crea.rpc.get_tags_used_by_author({'author': account}, api="tags")['tags']
        else:
            return self.crea.rpc.get_tags_used_by_author(account, api="tags")
    ...
    def get_expiring_vesting_delegations(self, after=None, limit=1000, account=None):
        ...
        if self.crea.rpc.get_use_appbase():
            return self.crea.rpc.find_vesting_delegation_expirations({'account': account}, api="database")['delegations']
        else:
            return self.crea.rpc.get_expiring_vesting_delegations(account, formatTimeString(after), limit)
    ...
    def get_account_votes(self, account=None):
        ...
        self.crea.rpc.set_next_node_on_empty_reply(False)
        if self.crea.rpc.get_use_appbase():
            vote_list = self.crea.rpc.get_account_votes(account, api="condenser")
        else:
            vote_list = self.crea.rpc.get_account_votes(account)
        if isinstance(vote_list, dict) and "error" in vote_list:
            ...
            finished = False
            while not finished:
                ret = self.crea.rpc.list_votes({"start": [account, start_author, start_permlink], "limit": 1000, "order": "by_voter_comment"}, api="database")["votes"]
                if start_author != "":
                    if len(ret) == 0:
                        finished = True                     
                    ret = ret[1:]
    ...
    def get_vote(self, comment):
        ...
        c = Comment(comment, crea_instance=self.crea)
    ...
    def _get_account_history(self, account=None, start=-1, limit=0):
        ...
        if self.crea.rpc.get_use_appbase():
            try:
                ret = self.crea.rpc.get_account_history({'account': account["name"], 'start': start, 'limit': limit}, api="account_history")['history']
            except ApiNotSupported:
                ret = self.crea.rpc.get_account_history(account["name"], start, limit, api="condenser")
        else:
            ret = self.crea.rpc.get_account_history(account["name"], start, limit, api="database")
            if len(ret) == 0 and limit == 0:
                ret = self.crea.rpc.get_account_history(account["name"], start, limit + 1, api="database")
        return ret
    ...
    def get_curation_reward(self, days=7):
        ...
        reward_vests = Amount(0, self.crea.vests_symbol, crea_instance=self.crea)
        for reward in self.history_reverse(stop=stop, use_block_num=False, only_ops=["curation_reward"]):
            reward_vests += Amount(reward['reward'], crea_instance=self.crea)
        return self.crea.vests_to_sp(reward_vests.amount)
        ...
        """ Update an account's profile in json_metadata
            ...
            .. code-block:: js

                {
                    ...
                    'website': 'https://github.com/holgern/crea'
                }
    ...
    def update_account_metadata(self, metadata, account=None, **kwargs):
        ...
        return self.crea.finalizeOp(op, account, "active", **kwargs)
    ...
    def approvewitness(self, witness, account=None, approve=True, **kwargs):
        ...
        #     witness = Witness(witness, crea_instance=self)
    ...
    def update_memo_key(self, key, account=None, **kwargs):
        ...
        PublicKey(key, prefix=self.crea.prefix)
    ...
    def update_account_keys(self, new_password, account=None, **kwargs):
        ...
        for role in ['owner', 'active', 'posting', 'memo']:
            ...
            key_auths[role] = format(pk.get_public_key(), self.crea.prefix)
        ...
        return self.crea.finalizeOp(op, account, "owner", **kwargs)
    ...
    def transfer(self, to, amount, asset, memo="", account=None, **kwargs):
        """ Transfer an asset to another account.
            ...
            .. code-block:: python
                ...
                from crea import Crea
                active_wif = "5xxxx"
                stm = Crea(keys=[active_wif])
                acc = Account("test", crea_instance=stm)
                acc.transfer("test1", 1, "CREA", "test")
        ...
        if account is None:
            ...
            account = Account(account, crea_instance=self.crea)
        amount = Amount(amount, asset, crea_instance=self.crea)
        to = Account(to, crea_instance=self.crea)
        if memo and memo[0] == "#":
    ...
    def transfer_to_vesting(self, amount, to=None, account=None, **kwargs):
        ...
        if to is None:
            ...
            to = Account(to, crea_instance=self.crea)
        amount = self._check_amount(amount, self.crea.crea_symbol)
    ...
    def convert(self, amount, account=None, request_id=None):
        """ Convert CreaDollars to Crea (takes 3.5 days to settle)

            :param float amount: amount of CBD to convert
            :param str account: (optional) the source account for the transfer
                if not ``default_account``
        ...
        if account is None:
            ...
            account = Account(account, crea_instance=self.crea)
        amount = self._check_amount(amount, self.crea.sbd_symbol)
        if request_id:
    ...
    def transfer_to_savings(self, amount, asset, memo, to=None, account=None, **kwargs):
        ...
        """
        if asset not in [self.crea.crea_symbol, self.crea.sbd_symbol]:
    ...
    def _check_amount(self, amount, symbol):
        if isinstance(amount, (float, integer_types)):
            amount = Amount(amount, symbol, crea_instance=self.crea)
        elif isinstance(amount, string_types) and amount.replace('.', '', 1).replace(',', '', 1).isdigit():
            amount = Amount(float(amount), symbol, crea_instance=self.crea)
        else:
    ...
    def claim_reward_balance(self,
        ...
        `reward_crea`, `reward_sbd` or `reward_vests`.

        :param str reward_crea: Amount of CREA you would like to claim.
        :param str reward_sbd: Amount of CBD you would like to claim.
        ...
        reward_crea = self._check_amount(reward_crea, self.crea.crea_symbol)
        reward_sbd = self._check_amount(reward_sbd, self.crea.sbd_symbol)
        reward_vests = self._check_amount(reward_vests, self.crea.vests_symbol)

        if reward_crea.amount == 0 and reward_sbd.amount == 0 and reward_vests.amount == 0:
            if len(account.balances["rewards"]) == 3:
                reward_crea = account.balances["rewards"][0]
    ...
    def delegate_vesting_shares(self, to_account, vesting_shares,
        ...
        if account is None:
            ...
            account = Account(account, crea_instance=self.crea)
        to_account = Account(to_account, crea_instance=self.crea)
        if to_account is None:
            raise ValueError("You need to provide a to_account")
        vesting_shares = self._check_amount(vesting_shares, self.crea.vests_symbol)
    ...
    """
        if account is None:
            ...
            account = Account(account, crea_instance=self.crea)
        amount = self._check_amount(amount, self.crea.vests_symbol)
    ...
    def set_withdraw_vesting_route(self,
        ...
        op = operations.Set_withdraw_vesting_route(
            **{
                ...
                "to_account": to,
                "percent": int(percentage * CREA_1_PERCENT),
                "auto_vest": auto_vest
            })
    ...
    def allow(
        ...
        if permission not in account:
            account = Account(account, crea_instance=self.crea, lazy=False, full=True)
            account.clear_cache()
            account.refresh()
        if permission not in account:
            account = Account(account, crea_instance=self.crea)
        ...
        try:
            pubkey = PublicKey(foreign, prefix=self.crea.prefix)
            ...
            try:
                foreign_account = Account(foreign, crea_instance=self.crea)
        ...
        if threshold:
            ...
            self.crea._test_weights_treshold(authority)
    ...
    def feed_history(self, limit=None, start_author=None, start_permlink=None,
        ...
        while True:
            ...
            query = Query(start_author=start_author,
                          ...
                          tag=account['name'])
            results = Discussions_by_feed(query, crea_instance=self.crea)
    ...
    def blog_history(self, limit=None, start=-1, reblogs=True, account=None):
        """ Stream the blog entries done by an account in reverse time order.
            ...
            :param bool reblogs: (optional) if set `True` (default)
                reblogs / recreas are included. If set `False`,
                reblogs/recreas are omitted.
...
class AccountsObject(list):
    ...
    def print_summarize_table(self, tag_type="Follower", return_str=False, **kwargs):
        ...
        for f in self:
            ...
            eff_sp.append(f.get_crea_power())
...
class Accounts(AccountsObject):
    """ Obtain a list of accounts
        ...
        :param Crea crea_instance: Crea() instance to use when
            accessing a RPCcreator = Account(creator, crea_instance=self)
    """
    def __init__(self, name_list, batch_limit=100, lazy=False, full=True, crea_instance=None):
        ...
        while name_cnt < len(name_list):
            self.crea.rpc.set_next_node_on_empty_reply(False)
            if self.crea.rpc.get_use_appbase():
                accounts += self.crea.rpc.find_accounts({'accounts': name_list[name_cnt:batch_limit + name_cnt]}, api="database")["accounts"]
            else:
                accounts += self.crea.rpc.get_accounts(name_list[name_cnt:batch_limit + name_cnt])
            name_cnt += batch_limit

        super(Accounts, self).__init__(
            [
                Account(x, lazy=lazy, full=full, crea_instance=self.crea)
                for x in accounts
            ]
        )
