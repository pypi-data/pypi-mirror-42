...
from crea.utils import formatTimeString, formatTimedelta, remove_from_dict, reputation_to_score, addTzInfo, parse_time
...
from crea.constants import CREA_VOTE_REGENERATION_SECONDS, CREA_1_PERCENT, CREA_100_PERCENT
...
class AccountSnapshot(list):
    """ This class allows to easily access Account history
        ...
        :param Crea crea_instance: Crea
               instance
    """
    def __init__(self, account, account_history=[], crea_instance=None):
        self.crea = crea_instance or shared_crea_instance()
        self.account = Account(account, crea_instance=self.crea)
        self.reset()
    ...
    def reset(self):
        ...
        """
        self.own_vests = [Amount(0, self.crea.vests_symbol, crea_instance=self.crea)]
        self.own_crea = [Amount(0, self.crea.crea_symbol, crea_instance=self.crea)]
        self.own_sbd = [Amount(0, self.crea.sbd_symbol, crea_instance=self.crea)]
    ...
    def get_data(self, timestamp=None, index=0):
        ...
        crea = self.own_crea[index]
        sbd = self.own_sbd[index]
        sum_in = sum([din[key].amount for key in din])
        sum_out = sum([dout[key].amount for key in dout])
        sp_in = self.crea.vests_to_sp(sum_in, timestamp=ts)
        sp_out = self.crea.vests_to_sp(sum_out, timestamp=ts)
        sp_own = self.crea.vests_to_sp(own, timestamp=ts)
        sp_eff = sp_own + sp_in - sp_out
        return {"timestamp": ts, "vests": own, "delegated_vests_in": din, "delegated_vests_out": dout,
                "sp_own": sp_own, "sp_eff": sp_eff, "crea": crea, "sbd": sbd, "index": index}
    ...
    def update_rewards(self, timestamp, curation_reward, author_vests, author_crea, author_sbd):
        self.reward_timestamps.append(timestamp)
        self.curation_rewards.append(curation_reward)
        self.author_rewards.append({"vests": author_vests, "crea": author_crea, "sbd": author_sbd})
    ...
    def update(self, timestamp, own, delegated_in=None, delegated_out=None, crea=0, sbd=0):
        ...
        self.own_crea.append(self.own_crea[-1])
        ...
        self.own_crea.append(self.own_crea[-1] + crea)
    ...
    def parse_op(self, op, only_ops=[], enable_rewards=False, enable_out_votes=False, enable_in_votes=False):
        ...
        if op['type'] == "account_create":
            fee_crea = Amount(op['fee'], crea_instance=self.crea).amount
            fee_vests = self.crea.sp_to_vests(Amount(op['fee'], crea_instance=self.crea).amount, timestamp=ts)
            # print(fee_vests)
            if op['new_account_name'] == self.account["name"]:
                self.update(ts, fee_vests, 0, 0)
                return
            if op['creator'] == self.account["name"]:
                self.update(ts, 0, 0, 0, fee_crea * (-1), 0)
                return
            ...
            fee_vests = self.crea.sp_to_vests(Amount(op['fee'], crea_instance=self.crea).amount, timestamp=ts)
            if op['new_account_name'] == self.account["name"]:
                if Amount(op['delegation'], crea_instance=self.crea).amount > 0:
                    delegation = {'account': op['creator'], 'amount':
                                  Amount(op['delegation'], crea_instance=self.crea)}
                else:
                    delegation = None
            ...
            if op['creator'] == self.account["name"]:
                delegation = {'account': op['new_account_name'], 'amount':
                              Amount(op['delegation'], crea_instance=self.crea)}
                self.update(ts, 0, 0, delegation, fee_crea * (-1), 0)
                return

        elif op['type'] == "delegate_vesting_shares":
            vests = Amount(op['vesting_shares'], crea_instance=self.crea)
            # print(op)
            ...
            amount = Amount(op['amount'], crea_instance=self.crea)
            # print(op)
            if op['from'] == self.account["name"]:
                if amount.symbol == self.crea.crea_symbol:
                    self.update(ts, 0, 0, 0, amount * (-1), 0)
                elif amount.symbol == self.crea.sbd_symbol:
            ...
            current_pays = Amount(op["current_pays"], crea_instance=self.crea)
            open_pays = Amount(op["open_pays"], crea_instance=self.crea)
            if op["current_owner"] == self.account["name"]:
                if current_pays.symbol == self.crea.crea_symbol:
                    self.update(ts, 0, 0, 0, current_pays * (-1), open_pays)
                elif current_pays.symbol == self.crea.sbd_symbol:
            ...
            crea = Amount(op['amount'], crea_instance=self.crea)
            vests = self.crea.sp_to_vests(crea.amount, timestamp=ts)
            if op['from'] == self.account["name"]:
                self.update(ts, vests, 0, 0, crea * (-1), 0)
            else:
            ...
            # print(op)
            vests = Amount(op['withdrawn'], crea_instance=self.crea)
            self.update(ts, vests * (-1), 0, 0)
            return

        elif op['type'] == "return_vesting_delegation":
            delegation = {'account': None, 'amount':
                          Amount(op['vesting_shares'], crea_instance=self.crea)}
            self.update(ts, 0, 0, delegation)
            return

        elif op['type'] == "claim_reward_balance":
            vests = Amount(op['reward_vests'], crea_instance=self.crea)
            crea = Amount(op['reward_crea'], crea_instance=self.crea)
            sbd = Amount(op['reward_sbd'], crea_instance=self.crea)
            self.update(ts, vests, 0, 0, crea, sbd)
            return

        elif op['type'] == "curation_reward":
            if "curation_reward" in only_ops or enable_rewards:
                vests = Amount(op['reward'], crea_instance=self.crea)
            ...
            if "author_reward" in only_ops or enable_rewards:
                # print(op)
                vests = Amount(op['vesting_payout'], crea_instance=self.crea)
                crea = Amount(op['crea_payout'], crea_instance=self.crea)
                sbd = Amount(op['sbd_payout'], crea_instance=self.crea)
            if "author_reward" in only_ops:
                self.update(ts, vests, 0, 0, crea, sbd)
            if enable_rewards:
                self.update_rewards(ts, 0, vests, crea, sbd)
            return
            ...
            amount_in = Amount(op["amount_in"], crea_instance=self.crea)
            amount_out = Amount(op["amount_out"], crea_instance=self.crea)
            if op["owner"] == self.account["name"]:
                self.update(ts, 0, 0, 0, amount_out, amount_in * (-1))
            return

        elif op['type'] == "interest":
            interest = Amount(op["interest"], crea_instance=self.crea)
    ...
    def build_vp_arrays(self):
        ...
        self.vp = [CREA_100_PERCENT]
        for (ts, weight) in zip(self.out_vote_timestamp, self.out_vote_weight):
            self.vp.append(self.vp[-1])

            if self.vp[-1] < CREA_100_PERCENT:
                regenerated_vp = ((ts - self.vp_timestamp[-1]).total_seconds()) * CREA_100_PERCENT / CREA_VOTE_REGENERATION_SECONDS
                self.vp[-1] += int(regenerated_vp)

            if self.vp[-1] > CREA_100_PERCENT:
                self.vp[-1] = CREA_100_PERCENT
            self.vp[-1] -= self.crea._calc_resulting_vote(self.vp[-1], weight)
            if self.vp[-1] < 0:
                self.vp[-1] = 0
    ...
    def build_curation_arrays(self, end_date=None, sum_days=7):
        ...
        for (ts, vests) in zip(self.reward_timestamps, self.curation_rewards):
            if vests == 0:
                continue
            sp = self.crea.vests_to_sp(vests, timestamp=ts)
