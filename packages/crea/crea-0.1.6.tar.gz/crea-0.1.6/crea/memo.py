...
import random
from creabase import memo as BtsMemo
...
class Memo(object):
    """ Deals with Memos that are attached to a transfer
        ...
        .. code-block:: python
            ...
            m = Memo("creaeu", "wallet.xeroc")
            m.crea.wallet.unlock("secret")
        ...
        In Crea, memos are AES-256 encrypted with a shared secret between sender and
        ...
        The high level memo class makes use of the crea wallet to obtain keys
    ...
    def __init__(
        ...
        if to_account:
            self.to_account = Account(to_account, crea_instance=self.crea)
        if from_account:
            self.from_account = Account(from_account, crea_instance=self.crea)

    def unlock_wallet(self, *args, **kwargs):
        """ Unlock the library internal wallet
        """
        self.crea.wallet.unlock(*args, **kwargs)
        return self
