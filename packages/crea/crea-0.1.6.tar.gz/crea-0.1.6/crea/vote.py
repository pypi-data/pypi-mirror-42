...
from creagraphenebase.py23 import integer_types, string_types, text_type
...
from creaapi.exceptions import UnkownKey
...
class Vote(BlockchainObject):
    """ Read data about a Vote in the chain
        ...
        .. code-block:: python
           ...
           >>> v = Vote("@gtg/crea-pressure-4-need-for-speed|gandalf", crea_instance=stm)
    ...
    def refresh(self):
        ...
        try:
            ...
            if self.crea.rpc.get_use_appbase():
                votes = self.crea.rpc.get_active_votes({'author': author, 'permlink': permlink}, api="tags")['votes']
            else:
                votes = self.crea.rpc.get_active_votes(author, permlink, api="database_api")
        except UnkownKey:
        ...
        super(Vote, self).__init__(vote, id_item="authorpermvoter", lazy=self.lazy, full=self.full, crea_instance=self.crea)
    ...
    @property
    def sbd(self):
        return self.crea.rshares_to_sbd(int(self.get("rshares", 0)))
...
class VotesObject(list):
    ...
    def printAsTable(self, voter=None, votee=None, start=None, stop=None, start_percent=None, stop_percent=None, sort_key="time", reverse=True, allow_refresh=True, return_str=False, **kwargs):
        utc = pytz.timezone('UTC')
        table_header = ["Voter", "Votee", "CBD", "Time", "Rshares", "Percent", "Weight"]
...
class ActiveVotes(VotesObject):
    """ Obtain a list of votes for a post
        ...
        :param Crea crea_instance: Crea() instance to use when accesing a RPC
    """
    def __init__(self, authorperm, lazy=False, full=False, crea_instance=None):
        self.crea = crea_instance or shared_crea_instance()
        votes = None
        if not self.crea.is_connected():
            return None
        ...
        if isinstance(authorperm, Comment):
            ...
            elif self.crea.rpc.get_use_appbase():
                self.crea.rpc.set_next_node_on_empty_reply(True)
                votes = self.crea.rpc.get_active_votes({'author': authorperm["author"],
                                                         'permlink': authorperm["permlink"]},
                                                        api="tags")['votes']
            else:
                votes = self.crea.rpc.get_active_votes(authorperm["author"], authorperm["permlink"])
            authorperm = authorperm["authorperm"]
        elif isinstance(authorperm, string_types):
            [author, permlink] = resolve_authorperm(authorperm)
            if self.crea.rpc.get_use_appbase():
                self.crea.rpc.set_next_node_on_empty_reply(True)
                votes = self.crea.rpc.get_active_votes({'author': author,
                                                         'permlink': permlink},
                                                        api="tags")['votes']
            else:
                votes = self.crea.rpc.get_active_votes(author, permlink)
        ...
        super(ActiveVotes, self).__init__(
            [
                Vote(x, authorperm=authorperm, lazy=lazy, full=full, crea_instance=self.crea)
                for x in votes
            ]
        )
...
class AccountVotes(VotesObject):
    """ Obtain a list of votes for an account
        ...
        :param Crea crea_instance: Crea() instance to use when accesing a RPC
    """
    def __init__(self, account, start=None, stop=None, lazy=False, full=False, crea_instance=None):
        self.crea = crea_instance or shared_crea_instance()
        start = addTzInfo(start)
        stop = addTzInfo(stop)
        ...
        for x in votes:
            ...
            if (start is None or d_time >= start) and (stop is None or d_time <= stop):
                vote_list.append(Vote(x, authorperm=account["name"], lazy=lazy, full=full, crea_instance=self.crea))
