...
from creagraphenebase.signedtransactions import Signed_Transaction as GrapheneSigned_Transaction
...
class Signed_Transaction(GrapheneSigned_Transaction):
    """ Create a signed transaction and offer method to create the
        ...
        :param num refNum: parameter ref_block_num (see :func:`creabase.transactions.getBlockParams`)
        :param num refPrefix: parameter ref_block_prefix (see :func:`creabase.transactions.getBlockParams`)
    ...
    def sign(self, wifkeys, chain=u"CREA"):
        return super(Signed_Transaction, self).sign(wifkeys, chain)

    def verify(self, pubkeys=[], chain=u"CREA", recover_parameter=False):
