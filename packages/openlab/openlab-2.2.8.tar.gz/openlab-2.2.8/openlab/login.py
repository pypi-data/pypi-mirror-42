...
environments = {'prod': "https://live.openlab.app", 'dev' : "https://dev.openlab.iris.no", 'build':"https://build.openlab.iris.no", 'local':"http://localhost:8888"}
...
def token_handler(token, proxies, auth_data):
    """
...
def get_credentials(location = 'keyring', environment = 'prod'):
    ...
    if location == 'keyring':
        c['user'] = get_keyring_username(environment)
        c['key'] = get_keyring_password(c['user'], environment)
        c['license_id'] = get_keyring_license(c['user'], environment)
    elif location == 'account':
        try:
            exec(open(current_directory+"/account.py").read())
            c['user'] = account.username
            c['key'] = account.apikey
            c['license_id'] = account.licenseguid
        except:
...
def create_token(**kwargs):
    ...
    auth_data = get_credentials(location, environment)
    ...
    if 'proxies' in kwargs:
        ...
        proxies = kwargs.get('proxies')
    # give 2nd priority to kwargs in auth_data
    elif auth_data['proxies']:
    ...
    try:
        token = oauth.fetch_token(token_url = auth_data['url'] + "/connect/token",
            key = auth_data['key'], username = auth_data['user'], client_id=auth_data['client_id'], include_client_id=True)
    ...
    if token is not None:
        #save the token
        token_saver(token)
        return token_handler(token, proxies, auth_data) #overwrite the oauth so that we can refresh on the first time
    else:
