...
<md:IDPSSODescriptor WantAuthnRequestsSigned="false" protocolSupportEnumeration="urn:oasis:names:tc:SAML:2.0:protocol">
    ...
    <ds:KeyInfo xmlns:ds="http://www.w3.org/2000/09/xmldsig#">
        <ds:X509Data>
        <ds:X509Certificate>
Y86OFv3OgMSmyCtleTa38PslKlILVrq7sDpsGXTyTYLZ</ds:X509Certificate>
        </ds:X509Data>
    </ds:KeyInfo>
    </md:KeyDescriptor>
    ...
    #
    # Google Group Functions
    #
    def create_google_group(self, email):
        """Create a Google Group and add some members."""
        group_name = email.replace('@broadinstitute.org', '')
        print('  * Creating Group: %s...' % (email))
        group = self.google.directory().create_group(email, group_name)
        print('  * Created Group: %s' % (email))

        # add owner(s) to group
        self.add_google_group_owners(email)

        # add member(s) to group
        self.add_google_group_members(email)

        return group

    def get_google_group(self, email):
        """Return a Google Group from Google."""
        try:
            group = self.google.directory().get_group(email)
            return group
        except Exception:
            return None

    def update_google_group_members(self):
        """Update the members of the Google Group for this account."""

    def update_google_group_owners(self):
        """Update the owners of the Google Group for this account."""
    ...
    def check_group_managed_policies(self, group_name):
        ...
        if group_name in ['Admins']:
            if self.admin_policy not in policies:
                if self.verbose:
                    print('    + Adding admin policy to group: %s' % (
                        group_name,
                    ))
                # attach admin policy
        ...
        if group_name in ['Admins', 'BillingAdmins']:
            if self.billing_policy not in policies:
                if self.verbose:
                    print('    + Adding billing policy to role: %s' % (
                        group_name,
                    ))
        ...
        if group_name in ['BillingAdmins']:
            if self.admin_policy in policies:
                if self.verbose:
                    print('    - Removing admin policy from role: %s' % (
                        group_name,
                    ))
                # detach admin policy
    ...
    def check_role_managed_policies(self, role_name):
        ...
        if self.admin_policy not in policies:
            if self.verbose:
                print('    * Adding admin policy to role: %s' % (
                    role_name,
                ))
            # attach admin policy
    ...
    def update_saml_provider(self):
        ...
        if not saml_check:
            ...
            if 'SAMLProviderArn' in result:
                ...
                print('    * Updated SAML provider: %s' % (arn))
