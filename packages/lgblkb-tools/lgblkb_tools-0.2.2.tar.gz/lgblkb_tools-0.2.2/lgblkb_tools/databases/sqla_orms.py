...
from .statuses import back_status
...
class SridTest(Base,IntegerID_Base):
	__tablename__='cadastres_srid_test'
...
class MainRequest(Base,IntegerID_Base):
	...
	back_status=Column(Text,default=back_status.Unprocessed)
...
class ImageDateChecker(Base,IntegerID_Base):
...
class Sentinel2_Info(TextID_Base,Base):
	...
	status=Column(Text,server_default=back_status.Unprocessed)
	priority=Column(Integer,default=5)
...
def main():
	...
	# old_statuses['RunningAtmosphericCorrection']=back_status.Running.AtmosphericCorrection
	# old_statuses['FinishedAtmosphericCorrection']=back_status.Finished.AtmosphericCorrection
	# old_statuses['QueuedForAtmosphericCorrection']=back_status.QueuedFor.AtmosphericCorrection
	# old_statuses['NotForProcessing']=back_status.NotForProcessing
	#
