...
class Query_Item(sqla_orms.Reconnectable):
	_protected_keys=['_Query_Item'+x for x in ['__item','__engine_creator','__obj_class','__primary_keys']]
	...
	def __repr__(self):
		return f"Query_Item: {gsup.reprer(self.__item)}"
...
class Manager(object):
	...
	def __init__(self,db_configs: dict,create_if_not_exists=False,**connection_kwargs):
		...
		self.__engine=None
		# self.__session=None
		self.__query_item_creator=partial(Query_Item,engine_creator=self.__engine_creator)
		self.__box=Box()
	...
	# def make_session(self,autoflush=True,autocommit=False,expire_on_commit=True,info=None,**kwargs):
	...
	def session_context(self):
		return partial(session_context,self.engine)
	...
	def query_get(self,entity: typing.Type[sqla_orms.KeysBase],*primary_keys):
		...
		with self.session_context() as session:
			return Query_Item(session.query(entity).get(primary_keys),self.__engine_creator,entity.primary_keys)
	
	def get_wrapped(self,obj):
		return Query_Item(obj,self.__engine_creator,obj.primary_keys)
