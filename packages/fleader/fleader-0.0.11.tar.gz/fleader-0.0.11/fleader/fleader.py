...
class fleader():
    #===============请求类=========================================
    @staticmethod#请求器
    def get(url,data='',cookie='',ip='',timeout=60,headers='',encode='',redect=True,timetry=3,files='',session='',prce='',ex='',tail='',rw='',cache=False,cachedur=2,bins=False,getcks=False,cookies='',autoua=True):
        import requests
    ...
    @staticmethod         
    def rw(path, line=True, aw='a',sp="",readline=False):
        fleader.init()#修改工作空间
        import os
    ...
    @staticmethod
    def cache(suffix='.txt',path='cache',ext=[],dur=2,fc=False,fin='',aw='w'):#类似functools.lru_cache()#只能对字符串变量进行文件缓存
        def deco(func):
            def wrapper(*arg, **kw):
                ...
                if fc:
                    ...
                    fname=''
                state,res=fleader.rwcache(fhash,suffix=suffix,dur=dur,path=path,fname=fname)
                if not state:
                    res=func(*arg, **kw)
