...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_12_score(self):
        ...
        test_image = load_img('datasets/dogs-vs-cats/cat.2345.jpg', target_size=(150, 150))
        test_image_array = img_to_array(test_image)

        payload_values = np.expand_dims(test_image_array, axis=0).tolist()
        scoring_payload = {'values': np.expand_dims(test_image_array, axis=0).tolist()}

        for i in range(0, 20):
            scores = self.wml_client.deployments.score(scoring_url=scoring_endpoint, payload=scoring_payload)
            self.assertIsNotNone(scores)

        print("Waiting 20 seconds for performance calculation...")
        time.sleep(20)
    ...
    def test_16_feedback_logging(self):
        ...
        feedback_payload = np.expand_dims(test_image_array, axis=0).tolist()
