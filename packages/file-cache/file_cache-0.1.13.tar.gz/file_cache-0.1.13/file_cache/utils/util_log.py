...
import functools
import time
def timed(logger=logger, level='debug', format='%s: %s ms', paras=True):
    ...
    def decorator(fn):
        ...
        def inner(*args, **kwargs):
            ...
            if paras:
                log("%s begin with(%s paras) :%r, %r" % (fn.__name__, arg_count, args_mini, kwargs_mini))
            else:
            ...
            log('%s cost %7.2f sec:(%s paras)(%r, %r), return:%s, end '
