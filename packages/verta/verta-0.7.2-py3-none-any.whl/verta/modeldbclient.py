...
from . import _utils
from .exceptions import StateError, NotFoundError
...
class Project:
    ...
    def _get(auth, socket, proj_name=None, *, _proj_id=None):
        ...
        if response.ok:
            return response.json().get('project_by_user', [])[0]  # TODO: handle collaboration
        else:
    ...
    def _create(auth, socket, proj_name, desc=None, tags=None):
        msg = _ProjectService.CreateProject(name=proj_name, description=desc, tags=tags)
        data = _utils.jsonify(msg)
    ...
    def find(self, where, ret_all_info=False, *, _proj_id=None, _expt_id=None):
        ...
        for predicate in where:
            ...
            predicates.append(_ExperimentRunService.KeyValueQuery(key=key, value=_utils.to_msg(value),
                                                                  operator=operator))
        msg = _ExperimentRunService.FindExperimentRuns(project_id=_proj_id, experiment_id=_expt_id,
...
class ExperimentRun:
    ...
    def get_hyperparameter(self, name):
        ...
        hyperparameters = {hyperparameter['key']: hyperparameter['value']
    ...
    def get_hyperparameters(self):
        ...
        return {hyperparameter['key']: hyperparameter['value']
                for hyperparameter in response.json().get('hyperparameters', [])}
    ...
    def get_dataset(self, name):
        ...
        datasets = {dataset['key']: dataset['path'] for dataset in response.json().get('datasets', [])}
    ...
    def get_observations(self, name):
        ...
        return [observation['attribute']['value']
