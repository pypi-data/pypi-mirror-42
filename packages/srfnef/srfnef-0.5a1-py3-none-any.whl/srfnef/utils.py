...
def _dataclass_to_md(dat, indent = 0):
    ...
    for key, value in dat.as_dict(recurse = False).items():
...
def _imshow(dat):
    ...
    if not isinstance(dat, Image):
        raise ValueError
    plt.figure(figsize = (15, 8))
    plt.subplot(121)
    plt.imshow(dat.central_slices[2])
...
def write_markdown(path, args, output):
    ...
    out_string += '---\n\n'
    out_string += '## Output: \n\n'
    out_string += _dataclass_to_md(output)
