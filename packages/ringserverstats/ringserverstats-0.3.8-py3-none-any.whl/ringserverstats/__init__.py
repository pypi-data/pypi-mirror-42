        ...
        for log in file:
            ...
            if log.startswith('START CLIENT'):
                ...
                if location != None:
                    ...
                    logger.warning("No location available at %s:%d : %s"%(filename, linecount, log))
    ...
    for f in files:
        ...
        if dbhost != None :
            ...
            try:
                i=0
                j=10000
                while j < len(influxdb_json_data):
                    logger.info("%d/%d"%(j,len(influxdb_json_data)))
                    influxdb_send_data(dbhost, dbport, dbname, verifyssl, dbuser, password, influxdb_json_data[i:j])
                    i=j
                    j+=10000
            except Exception as e:
