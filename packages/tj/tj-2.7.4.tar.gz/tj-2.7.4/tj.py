...
Updated on: 20th February 05:15 PM
''' % ( __version__)
...
def get_files_in_folder(path, show_progress=False):
    ...
    show_progress-> It is an optional argument set to False by default.
        When show_progress is not False, the progress will be displayed.
        When show_progress can be anything, either True, or a string. If it is a
        string, then that string will be printed alongside the progress.
    ...
    if show_progress!=False:
        ...
        if not is_present:raise ModuleNotFoundError(
