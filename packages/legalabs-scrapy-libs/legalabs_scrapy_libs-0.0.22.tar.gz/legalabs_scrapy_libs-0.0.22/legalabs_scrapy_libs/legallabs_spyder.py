...
class LegalLabsSpider(scrapy.Spider):
    ...
    def init(self):
        ...
        self.crawler.stats.set_value('custom/date_start', self.date_start.isoformat())
        self.crawler.stats.set_value('custom/date_end', self.date_end.isoformat())
        self.crawler.stats.set_value('custom/save_mode', self.save_mode)
    ...
    def closed(self, reason):
        ...
        if type(start_time) == datetime and type(finish_time) == datetime:
            duration = int((finish_time - start_time).total_seconds())
            minutes = duration // 60
            hours = duration // 3600
            days = duration // (24 * 3600)
            duration = ' '.join(x for x in [
                '' if days == 0 else '{} day(s)'.format(days),
                '' if hours == 0 else '{} hours(s)'.format(hours % 24),
                '' if minutes == 0 else '{} minutes(s)'.format(minutes % 60),
                '{} second(s)'.format(duration % 60)
            ] if len(x) > 0)
        else:
            duration = '?'

        self.crawler.stats.set_value('custom/running_duration', duration)
