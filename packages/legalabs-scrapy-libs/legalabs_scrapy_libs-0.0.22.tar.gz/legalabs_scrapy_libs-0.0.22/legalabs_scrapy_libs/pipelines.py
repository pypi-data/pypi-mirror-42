...
class ElasticSearchPipeline(object):
    ...
    crawler = None
    items_buffer = []
    bulk_insert_errors_key = 'custom/es_pipeline/bulk_insert_errors'
    bulk_insert_count_key = 'custom/es_pipeline/bulk_insert_count'
    ...
    def from_crawler(cls, crawler):
        ...
        ext.crawler = crawler
        ext.crawler.stats.set_value(cls.bulk_insert_errors_key, 0)
        return ext
    ...
    def send_items(self):
        try:
            ...
            old_value = self.crawler.stats.get_value(self.bulk_insert_count_key) or 0
            self.crawler.stats.set_value(
                self.bulk_insert_count_key,
                old_value + len(self.items_buffer)
            )
        except:
            old_value = self.crawler.stats.get_value(self.bulk_insert_errors_key) or 0
            self.crawler.stats.set_value(
                self.bulk_insert_errors_key,
                old_value + len(self.items_buffer)
            )
...
class JurisprudenciaOABPipeline(object):
    ...
    es = None
    items_buffer = []
    bulk_insert_errors_key = 'custom/oab_pipeline/bulk_insert_errors'
    bulk_insert_count_key = 'custom/oab_pipeline/bulk_insert_count'
    ...
    def from_crawler(cls, crawler):
        ...
        ext.crawler = crawler
        ext.crawler.stats.set_value(cls.bulk_insert_errors_key, 0)
        ext.save_mode = crawler.settings.get('SAVE_MODE')
        return ext
