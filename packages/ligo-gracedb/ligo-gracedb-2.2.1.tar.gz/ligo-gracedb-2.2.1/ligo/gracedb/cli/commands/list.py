...
class ListCommand(RegisteredCommandBase):
    ...
    description = textwrap.dedent("""\
        List files, log entries, labels, EMObservations, or VOEvents associated
        with an event or superevent, or list tags attached to a log entry
    """)
...
class ListEmobservationsCommand(ListChildBase):
    name = "emobservations"
    description = "List EMObservations associated with an event or superevent"
...
class ListLogsCommand(ListChildBase):
    name = "logs"
    description = "List logs associated with an event or superevent"


class ListSignoffsCommand(ListChildBase):
    name = "signoffs"
    description = textwrap.dedent("""\
        List signoffs attached to a superevent. Event signoff retrieval
