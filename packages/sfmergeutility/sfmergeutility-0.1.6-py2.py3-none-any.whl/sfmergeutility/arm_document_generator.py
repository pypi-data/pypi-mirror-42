...
class ArmDocumentGenerator(object):
    ...
    def process_resource_refs(properties, schema_version):
        ...
        for prop in properties.keys():
            ...
            elif isinstance(properties[prop], OrderedDict):
                properties[prop] = ArmDocumentGenerator.process_resource_refs(properties[prop], schema_version)
                for prop_iter in properties[prop].keys():
                    val = str(properties[prop][prop_iter]).lower()
                    m = re.search("reference\(\'secrets\/\w*\/values\/\w*\'\)", val)
                    if m:
                        string_arr = val.split('/')
                        properties[prop][prop_iter] = "[resourceId('{0}','{1}','{2}')]".format(ArmDocumentGenerator.get_sbz_resource_type(Constants.SecretValue, schema_version), string_arr[1], string_arr[3].split('\'')[0])
        return properties
