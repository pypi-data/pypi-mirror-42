...
/*     set seeds for random number generator to negative value */
    ...
    inputcom_1.ipseed = -abs(inputcom_1.ipseed);
...
/* abor */
    ...
    for (i1 = inputcom_1.nharm + 1; i1 <= 5; ++i1) {
	if (inputcom_1.lout[i1 + 13] != 0) {
	    i__ = printerr_(&c_n9, "harmonic output not calculated", (ftnlen)
		    30);
	}
    }
