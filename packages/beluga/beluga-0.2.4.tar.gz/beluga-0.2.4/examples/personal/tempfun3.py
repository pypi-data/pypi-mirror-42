...
class Shooting(BaseAlgorithm):
    ...
    @staticmethod
    def _propagate_gammas(derivative_function, quadrature_function, gamma_set, paramGuess, sol, prop, pool, n_quads):
        tspan = []
        y0g = []
        q0g = []
        for ii in range(len(gamma_set)):
            _y0g, _q0g, _u0g = gamma_set[ii](gamma_set[ii].t[0])
            tspan.append(gamma_set[ii].t)
            y0g.append(_y0g)
            q0g.append(_q0g)

        pickled_derivative_function = pickle.dumps(derivative_function)
        pickled_quadrature_function = pickle.dumps(quadrature_function)
        def preload(args):
    ...
    def solve(self, solinit, **kwargs):
        ...
        yfstm = np.zeros((len(stm0) + n_odes))
        ...
        if self.algorithm in scipy_algorithms:
            ...
            def _constraint_function(X, deriv_func, quad_func, n_odes, n_quads, n_dynparams, n_nondynparams, n_arcs, aux):
                ...
                for ii in range(n_arcs):
                    ...
                    if n_quads > 0:
                        g[ii].q[0] = _q
                g = self._propagate_gammas(deriv_func, quad_func, g, _params, sol, prop, pool, n_quads)
                return self.bc_func_ms(g, _params, _nonparams, aux)

            def _constraint_function_wrapper(X):
                return _constraint_function(X, self.derivative_function, self.quadrature_function, n_odes, n_quads, n_dynparams, n_nondynparams, self.num_arcs, sol.aux)

            # Set up the jacobian of the constraint function
            def _jacobian_function(X, deriv_func, quad_func, n_odes, n_quads, n_dynparams, n_nondynparams, n_arcs, aux):
                g = copy.copy(gamma_set)
                stm0 = np.hstack((np.eye(n_odes), np.zeros((n_odes, n_dynparams)))).reshape(n_odes * (n_odes + n_dynparams))
                _y, _q, _params, _nonparams = self._unwrap_y0(X, n_odes, n_quads, n_dynparams, n_arcs)
                ...
                for ii in range(len(g)):
                    _y0g, _q0g, _u0g = g[ii](g[ii].t[0])
                    _yfg, _qfg, _ufg = g[ii](g[ii].t[-1])
                    y0stm = np.zeros((len(stm0) + n_odes))
                    yfstm = np.zeros((len(stm0) + n_odes))
                    y0stm[:n_odes] = _y0g
                    y0stm[n_odes:] = stm0[:]
                    yfstm[:n_odes] = _yfg
                    yfstm[n_odes:] = stm0[:]
                    g2.append(Trajectory(np.hstack((t0,tf)), np.vstack((y0stm,yfstm)), np.vstack((_q0g, _qfg)), np.vstack((_u0g, _ufg))))

                g2 = self._propagate_gammas(deriv_func, quad_func, g2, _params, sol, prop, pool, n_quads)
                ...
                for ii in range(len(g2)):
                    ...
                    q_set = g2[ii].q
                    u_set = g2[ii].u
                    g2[ii] = Trajectory(t_set, y_set, q_set, u_set)
                    phi_temp = np.reshape(temp[:, n_odes:], (len(g2[ii].t), n_odes, n_odes + n_dynparams))
                    phi_full_list.append(np.copy(phi_temp))

                J = self._bc_jac_multi(g2, phi_full_list, _params, _nonparams, sol.aux, quad_func, self.bc_func_ms, StepSize=1e-6)
                return J

            def _jacobian_function_wrapper(X):
                return _jacobian_function(X, self.stm_ode_func, self.quadrature_function, n_odes, n_quads, n_dynparams, n_nondynparams, self.num_arcs, sol.aux)
            ...
            for ii in range(self.num_arcs):
                ...
                if n_quads > 0:
                    gamma_set[ii].q[0] = q
            gamma_set = self._propagate_gammas(self.derivative_function, self.quadrature_function, gamma_set, parameter_guess, sol, prop, pool, n_quads)
            n_iter = opt.nit
            ...
            while not converged and n_iter <= self.max_iterations and err < self.max_error:
                ...
                gamma_set, phi_full_list = self._propagate_gammas_and_stt(self.stm_ode_func, self.quadrature_function, gamma_set, y0stm, stm0, parameter_guess, sol, prop, pool, n_odes, n_quads, n_dynparams)
