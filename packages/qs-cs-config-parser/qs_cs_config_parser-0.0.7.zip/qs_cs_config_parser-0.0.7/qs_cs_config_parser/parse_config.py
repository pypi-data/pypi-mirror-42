...
class parse_commands():
    ...
    def replace_placeholders_additional_nics(self, file_name, file_type, reservation_description):
        ...
        for command in commands:
            ...
            self.logger.info('B0 command is {0}'.format(command))

            if ('<<__D') in command:

                self.logger.info('B1 command is {0}'.format(command))
                # reference to device with subnet
                current_device_name = command.split('<<__D__')[1].split('__D__>>')[0]

                self.logger.info('B2 device name is {0}'.format(current_device_name))
                device_full_name = self._find_gateway_entity(current_device_name, reservation_description)
