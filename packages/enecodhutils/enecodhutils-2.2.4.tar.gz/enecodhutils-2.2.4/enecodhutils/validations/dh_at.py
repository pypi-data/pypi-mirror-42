...
@retry(stop_max_attempt_number=20, wait_fixed=500, retry_on_exception=dh_mssql.retry_if_error)
...
def get_previous_info():
    with pyodbc.connect('DRIVER={};SERVER={};DATABASE={};UID={};PWD={}'.format(
            ...
            settings.SQL_PASSWORD)) as db_connection_data:
        columns_info = pd.read_sql(
            "select * from forecastaanvraag_resultaat_nl_validated where "
            "datetime between '2019-02-05 23:00' and '2019-02-12 22:45';", con=db_connection_data)
    return columns_info
