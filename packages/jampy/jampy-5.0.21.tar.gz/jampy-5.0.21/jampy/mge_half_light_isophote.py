...
CALLING SEQUENCE:
    reff, reff_maj, eps_e, lum_tot = \
        mge_half_light_isophote(surf, sigma, q_obs, distance=distance)

INPUT PARAMETERS:
    SURF: Peak surface brightness of each Gaussian in Lsun/pc**2
    SIGMA: Gaussian dispersion in arcsec
    q_obs: Observed axial ratio of each Gaussian
...
OUTPUTS:
    ...
    REFF_MAJ: Major axis (largest size) of the isophote containing
        1/2 of the total MGE light.
    EPS_E: Ellipticity of the isophote containing 1/2 of the total MGE
        light, computed from the inertia tensor within the isophote.
    LUM_TOT: Analytic total luminosity in solar luminosities if the
...
def mge_half_light_radius(surf, sigma, q_obs, distance=1e-5):
    ...
    lum_r = np.sum(lum*(1 - np.exp(-0.5*(rad[:, None]/sigma)**2)), axis=1)
