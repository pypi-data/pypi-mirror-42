...
INPUT PARAMETERS:
  ...
  INC: inclination in degrees (90 being edge-on).
...
KEYWORDS:
  ...
  FLUX_OBS: Optional mean surface brigfhtness of each bin for plotting.
  ...
  NRAD: Number of logarithmically spaced radial positions for which the
      ...
      scales as NRAD*NANG.
  NSTEPS: Steps for the integration in the interval [0, 1] (default 50).
  ...
  VMAX: Maximum absolute value of the velocity to plot.

OUTPUT PARAMETERS:
  VELMODEL: Vector of length P with the model predictions for the mean
      velocity of each bin.
  KAPPA: Best fitting kappa parameter. If kappa is given in input, this value
  ...
  FLUX: Vector of length P with the PSF-convolved MGE surface brightness of
...
from jampy.jam_axi_rms import jam_axi_rms
...
def _surf_vlos(x1, y1, inc_deg,
    ...
    sh = np.sinh(np.linspace(-3, 3, 100))
    z1 = ms/sh[-1]*sh  # sinh sampling of z1 in [-ms, ms] with pivot ms/3
...
def _vel(x, y, inc_deg,
    ...
    for j, (xj, yj) in enumerate(zip(xPol, yPol)):
        wmPol[j] = _surf_vlos(xj, yj, inc_deg,
                              surf_lum, sigma_lum, qobs_lum,
                              surf_pot, sigma_pot, qobs_pot,
                              beta, kappa, gamma, component, nsteps)
...
def jam_axi_vel(surf_lum, sigma_lum, qobs_lum, surf_pot, sigma_pot, qobs_pot,
    ...
    velModel, kappa, chi2, flux = jam_axi_vel(
        surf_lum, sigma_lum, qobs_lum, surf_pot, sigma_pot, qobs_pot,
        inc, mbh, distance, xbin, ybin, beta=None, component='z',
        evel=None, flux_obs=None, gamma=None, goodbins=None, kappa=None,
        nang=10, normpsf=1., nrad=20, nsteps=65, pixang=0., pixsize=0.,
        plot=True, quiet=False, rbh=0.01, sigmapsf=0., step=0.,
        vel=None, vmax=None)
    ...
    assert round(np.sum(normpsf)*100) == 100, "PSF not normalized"
