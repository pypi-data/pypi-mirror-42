...
class ShapleyTest(unittest.TestCase):
    ...
    def test_run(self):
        ...
        results = s.run()
        expected = pd.DataFrame([['a', 0.3916666666666667],
                                 ['c', 0.3016666666666666],
                                 ['b', 0.2066666666666667]],
    ...
    def test_filter(self):
        ...
        results = s.run()
        expected = pd.DataFrame([['_0', 0.3916666666666667],  # there is a reason
                                 ['c', 0.3016666666666666],
                                 ['b', 0.2066666666666667]],
                                columns=['campaign', 'Shapley_val'])
        assert_frame_equal(results, expected, check_exact=False)
    ...
    def test_score_conversions(self):
        ...
        results = s.run()
        expected = pd.DataFrame([['a', 39.16666666666667],  # this is 100 x larger because i'm not using conversion rates, but raw conversion.
                                 ['c', 30.16666666666666],
                                 ['b', 20.66666666666667]],
                                columns=['campaign', 'Shapley_val'])
        assert_frame_equal(results, expected, check_exact=False)
