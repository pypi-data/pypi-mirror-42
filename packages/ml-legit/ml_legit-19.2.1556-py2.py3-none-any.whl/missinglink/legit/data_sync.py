...
from .path_utils import get_batch_of_files_from_paths, safe_make_dirs, DestPathEnum, enumerate_paths_with_info, \
    expend_and_validate_dir, get_moniker, remove_moniker, is_subdir
...
class DataSync(object):
    ...
    @classmethod
    def __get_file_to_upload(cls, get_changeset):
        ...
        for data in change_set:
            ...
            data = '{name} {size} {sha} {mtime} {ctime}\n'.format(**data)
    ...
    def _process_index(self, data_path, object_name, isolation_token):
        ...
        def iter_files():
            for line in files_to_upload:
                current_name, current_size, current_sha, current_mtime, current_ctime = line.strip().split(' ')

                path = os.path.join(data_path, current_name)
    ...
    def __get_index_and_metadata_info(self, files_info_iter):
        total_data_files = 0
        total_size_data_files = 0

        metadata_schema = {}
        index_schema = {}

        msg_pack_metadata_file = tempfile.TemporaryFile('wb+')
        msg_pack_index_file = tempfile.TemporaryFile('wb+')

        for entry_type, rel_file_name, data in files_info_iter:
            if entry_type == self.EntryType.Metadata:
                msg_pack_metadata_file.write(msgpack.packb((rel_file_name, data), use_bin_type=True))
                AvroWriter.get_schema_from_item(metadata_schema, data)
                continue

            total_size_data_files += data['size']
            total_data_files += 1

            file_params = self.__append_index_file(data)
            AvroWriter.get_schema_from_item(index_schema, file_params)
            msg_pack_index_file.write(msgpack.packb((rel_file_name, file_params), use_bin_type=True))

        msg_pack_index_file.seek(0)
        msg_pack_metadata_file.seek(0)

        return (msg_pack_index_file, total_data_files, total_size_data_files, index_schema), (msg_pack_metadata_file, metadata_schema)
    ...
    def upload_index_and_metadata(self, data_path, isolation_token=None, data_path_root=None):
        ...
        files_info_iter = self.__create_combined_index_and_metadata(full_data_path, status_line, full_data_path_root=full_data_path_root)

        with closing(status_line):
            index_info, metadata_info = self.__get_index_and_metadata_info(files_info_iter)

        msg_pack_metadata_file, metadata_schema = metadata_info
        if metadata_schema:
