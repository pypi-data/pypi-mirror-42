...
def __repo_dynamic_map_if_needed(ctx, config, volume_id, data_volume_config, data_path, require_path, **kwargs):
    session = ctx.obj.session
    repo_root = default_data_volume_path(volume_id)

    try:
        bind_repo = MlRepo(config, repo_root, data_volume_config, session=session, require_path=require_path, **kwargs)
        if data_path is not None:
            bind_repo.data_path = data_path
    except NotGitRepository:
        data_volume_config = map_volume(ctx, volume_id, data_path)
        bind_repo = MlRepo(
            config, repo_root, data_volume_config, read_only=True, require_path=False, session=session)

    return bind_repo


def repo_dynamic(ctx, data_volume_config_or_volume_id, data_path=None, require_path=True, **kwargs):
    ...
    if data_volume_config is None:
        repo_root = default_data_volume_path(volume_id)
        data_volume_config = __data_volume_config_from_path(config, repo_root)

    return __repo_dynamic_map_if_needed(ctx, config, volume_id, data_volume_config, data_path, require_path, **kwargs)


def with_repo_dynamic(ctx, data_volume_config_or_volume_id, data_path=None, **kwargs):
    return closing(repo_dynamic(ctx, data_volume_config_or_volume_id, data_path=data_path, **kwargs))


def create_repo(config, repo_root, data_volume_config=None, read_only=False, require_map=True, **kwargs):
    if data_volume_config is None:
        data_volume_config = __data_volume_config_from_path(config, repo_root)

    return MlRepo(config, repo_root, data_volume_config, read_only=read_only, require_path=require_map, **kwargs)
