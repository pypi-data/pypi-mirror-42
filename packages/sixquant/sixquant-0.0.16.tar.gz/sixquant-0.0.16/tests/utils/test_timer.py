...
class TestMethods(unittest.TestCase):
    ...
    def test_recycle_timer(self):
        ...
        # 异步模式
        self.counter = 0
        timer = RecycleTimer(interval=0.1, target=self.nothing, async_mode=True, align_to_minute=True)
        timer.start(1)  # 启动了一个独立线程
        timer.stop()

        self.counter = 0
        timer = RecycleTimer(interval=0.1, target=self.nothing, async_mode=True)
        timer.start(1)  # 启动了一个独立线程
