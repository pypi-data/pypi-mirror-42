...
class LensingImage(fit_data.FitData):
    ...
    def __init__(self, data, mask, sub_grid_size=2, image_psf_shape=None, mapping_matrix_psf_shape=None,
                 positions=None):
        """
        The lens datas_ is the collection of datas (regular, noise_map-maps, PSF), a masks, grid_stacks, convolvers and other \
        utilities that are used for modeling and fitting an datas_ of a strong lens.

        Whilst the datas_ datas is initially loaded in 2D, for the lens datas_ the masked-datas_ (and noise_map-maps) \
        ...
        sub_grid_size : int
            The size of the sub-grid used for each lens SubGrid. E.g. a value of 2 grid_stacks each datas_-pixel on a 2x2 \
            sub-grid.
        ...
        """
        super().__init__(data=data, mask=mask, sub_grid_size=sub_grid_size, image_psf_shape=image_psf_shape)
