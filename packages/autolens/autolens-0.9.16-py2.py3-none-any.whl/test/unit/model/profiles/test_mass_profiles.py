...
class TestEinsteinRadii(object):
    ...
    def test__same_as_above__elliptical_profiles(self):
        ...
        sersic = mp.EllipticalSersic(centre=(0.0, 0.0), intensity=1.0, mass_to_light_ratio=1.0, axis_ratio=0.4)
        assert sersic.radius_of_average_critical_curve_in_circle == pytest.approx(0.9598751, 1e-4)
        assert sersic.einstein_radius == pytest.approx(0.9598751, 1e-4)

        sersic = mp.EllipticalSersic(centre=(0.0, 0.0), intensity=1.0, mass_to_light_ratio=1.0,
                                                sersic_index=2.0, effective_radius=1.0, axis_ratio=0.4)
        assert sersic.radius_of_average_critical_curve_in_circle == pytest.approx(1.38900, 1e-4)
