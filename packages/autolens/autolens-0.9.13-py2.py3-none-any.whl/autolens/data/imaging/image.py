...
def output_imaging_to_fits(image, image_path, psf_path, noise_map_path=None, background_noise_map_path=None,
                           ...
                           overwrite=False):
    array_util.numpy_array_to_fits(array=image, file_path=image_path, overwrite=overwrite)
    array_util.numpy_array_to_fits(array=image.psf, file_path=psf_path, overwrite=overwrite)

    if image.noise_map is not None and noise_map_path is not None:
        array_util.numpy_array_to_fits(array=image.noise_map, file_path=noise_map_path, overwrite=overwrite)

    if image.background_noise_map is not None and background_noise_map_path is not None:
        array_util.numpy_array_to_fits(array=image.background_noise_map, file_path=background_noise_map_path,
                                       overwrite=overwrite)

    if image.poisson_noise_map is not None and poisson_noise_map_path is not None:
        array_util.numpy_array_to_fits(array=image.poisson_noise_map, file_path=poisson_noise_map_path,
                                       overwrite=overwrite)

    if image.exposure_time_map is not None and exposure_time_map_path is not None:
        array_util.numpy_array_to_fits(array=image.exposure_time_map, file_path=exposure_time_map_path,
                                       overwrite=overwrite)

    if image.background_sky_map is not None and background_sky_map_path is not None:
        array_util.numpy_array_to_fits(array=image.background_sky_map, file_path=background_sky_map_path,
                                       overwrite=overwrite)