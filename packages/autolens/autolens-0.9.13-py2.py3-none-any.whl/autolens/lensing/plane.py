...
class Plane(object):
    ...
    def __init__(self, galaxies, grids, border=None, compute_deflections=True, cosmology=None):
        ...
        grids : masks.GridStack
            The grid_stacks of (x,y) arc-second coordinates of this plane.
    ...
    def mapper(self):
        ...
        if len(galaxies_with_pixelization) == 1:
            ...
            return pixelization.mapper_from_grid_stack_and_border(self.grids[0], self.border)
    ...
    def image_plane_images_for_simulation(self):
        if not self.has_padded_grids:
            raise exc.RayTracingException(
                'To retrieve an datas_ plane datas_ for the simulation, the grid_stacks in the tracer_normal'
...
def sub_to_image_grid(func):
    """
    Wrap the function in a function that, if the grid is a sub-grid (grid_stacks.SubGrid), rebins the computed values to
