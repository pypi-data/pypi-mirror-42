...
class SLACS(object):
    ...
    def setup_lensing_plane_ltm2r(self, values, center_skip, ltm_skip):
    ...
    def density_vs_radii_sie(self, values, radius_kpc, number_bins=60):
        ...
        for r in range(number_bins):
            ...
            sie_density = self.critical_density_arcsec * (self.lens_galaxy.sie.mass_within_circle(radii[r+1]) -
                                                            self.lens_galaxy.sie.mass_within_circle(radii[r])) / annuli_area

            self.sie_density_plot.append(sie_density)

            self.radii_plot.append( ((radii[r+1] + radii[r]) / 2.0) * self.kpc_per_arcsec)

    def density_vs_radii_ltm2(self, values, radius_kpc, center_skip, ltm_skip, number_bins=60):
        ...
        for r in range(number_bins):
            ...
            envelope_density = self.critical_density_arcsec * (self.lens_galaxy.disk.mass_within_circle(radii[r+1]) -
    ...
    def plot_density(self, image_name='', labels=None, xaxis_is_physical=True, yaxis_is_physical=True):
        ...
        plt.savefig(path + 'density/RJLens2.png', bbox_inches='tight')
        plt.show()

    def masses_of_all_samples(self, radius_kpc, values, center_skip, ltm_skip):

        self.setup_lensing_plane_ltm2r(values, center_skip, ltm_skip)
        ...
        return total_mass, stellar_mass, bulge_mass, disk_mass, dark_mass