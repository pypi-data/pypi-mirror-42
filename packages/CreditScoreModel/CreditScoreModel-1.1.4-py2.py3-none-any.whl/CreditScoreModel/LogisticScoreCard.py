...
class logistic_score_card(object):
    ...
    def plot_roc(self,lr, test_x, test_y):
    ...
    def plot_ks(self,lr, test_x, test_y):
        predictions = lr.predict_proba(test_x)  # 每一类的概率
    ...
    def plot_auc_ks(self, train_x, test_x, train_y, test_y):
        start = time.time()
        ...
        plt.subplot(221)
        self.plot_roc(lr,train_x, train_y)
        plt.subplot(222)
        self.plot_roc(lr,test_x, test_y)
        plt.subplot(223)
        self.plot_ks(lr,train_x, train_y)
        plt.subplot(224)
        self.plot_ks(lr,test_x, test_y)
    ...
    def get_logistic_socre_card(self,data, cut_points_result,increase_score=50, base_score=600):
        logging.info('制作评分卡进行中。。。')

        col_result = [i[0] for i in cut_points_result]
    ...
    def fit(self, data):
        ...
        self.score_card=self.get_logistic_socre_card(data,cut_points_result)
        logging.info('任务结束')

    def predict_score_proba(self, data,col_result_cut_points,score_card,increase_score=50, base_score=600):
        logging.info('预测用户分数中。。。')
        b = -increase_score / np.log(2)
        a = base_score - score_card['lr_intercept'][0] * b
        data = data[[i[0] for i in col_result_cut_points]]
        data_cut_points = self.get_cut_result(data, col_result_cut_points).fillna('null')
