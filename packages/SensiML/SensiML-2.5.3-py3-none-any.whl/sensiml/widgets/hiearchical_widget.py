...
class HiearchicalWidget(BaseWidget):
    ...
    def generate_description(self, b):
        ...
        for key in self.kb_description['parent']:
            description = {clean_name(key):{'uuid':self.kb_description['parent'][key], 'results':{}, 'source':'motion'}}
    ...
    def add_submodel(self, b):
        index = int(b.icon)
        kp = self._dsk.get_knowledgepack(self.kb_items[SUBMODEL_INDEX].children[1].value)
        kp_item = []
    ...
    def get_model_list(self):
        ...
        [Box([Label(value='Sub Model')], layout=category_item_layout)] + self.flatten(self.kb_dict['sub'])
    ...
    def update(self):
        ...
        if self.kb_dict['parent']:
            for child in self.kb_dict['parent'][0][1:]:
                child.children[0].options=[k for k,v in self.kb_description['sub'].items()]

    def create_widget(self):

        self.platform_widget = Dropdown(options = self.get_platform_names())
        self.target_os_widget = Dropdown()
        self.app_output_options_widget = SelectMultiple(options=['ble','led','serial'])
        ...
        self.kb_items = [
            ...
            Box([Label(value='Select Hardware Platform', layout=label_value_layout), widgets.interactive(self.select_platform, platform_name=self.platform_widget)] ,  layout=form_item_layout),
            Box([Label(value='Select Target OS', layout=label_value_layout), self.target_os_widget], layout=form_item_layout),
            Box([Label(value='Select Output', layout=label_value_layout), self.app_output_options_widget], layout=form_item_layout),
            Box([Label(value='Download Type', layout=label_value_layout), self.download_type_widget], layout=form_item_layout),
            Box([Label(value='IMU Sample Rate', layout=label_value_layout), self.imu_sample_widget], layout=form_item_layout),
            Box([Label(value='Debug', layout=label_value_layout), self.debug_widget], layout=form_item_layout),
            Box([Label(value='Test Data', layout=label_value_layout), self.test_data_widget], layout=form_item_layout),
