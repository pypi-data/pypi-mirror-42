...
def addr_iton(addr, v6 = False):
    ...
    if addr < 0x100000000 and v6 is False:
        return str(struct.pack('>L', addr).decode('ascii'))
    else:
        w = []
        ...
        w[3] = addr & 0xffffffff
        return str(struct.pack('>4L', *w).decode('ascii'))
...
def eth_aton(mac):
    return str(struct.pack("6B", *map(lambda c: int(c, 16), str(mac).split(':'))).decode('ascii'))
...
def bit2netmask(bitmask, ipv6 = False):
    if bitmask > 32 or ipv6:
        ...
        tmp1 = str(struct.pack('>L', *[(~((2 ** (32 - bitmask)) - 1)) & 0xFFFFFFFF]).decode('ascii'))
...
class IpSock(SockBase):
    ...
    def sendto(self, gram, peerhost, flags = 0):
        ...
        if (amncore.compat.is_freebsd() == True or amncore.compat.is_netbsd() == True or amncore.compat.is_macosx() == True):
            ...
            tmp_pakt = str(struct.pack("@h", *tmp_unpakt).decode('ascii'))
            
            tmp_unpakt2 = struct.unpack(">H", bytes(gram[6:8]))
            tmp_pakt2 = str(struct.pack("@h", *tmp_unpakt2).decode('ascii'))
...
class LocalComm(SocketComm):
    ...
    def proxy(self, sock, type, host, port):
        if type.lower() == 'sapni':
            ...
            first_route_item = shost + str(struct.pack("B", 0).decode('ascii')) + str(sport) + str(struct.pack("bb", 0, 0).decode('ascii'))
            route_data = str(struct.pack(">L", len(first_route_item)).decode('ascii')) + first_route_item
            route_data += (host + str(struct.pack("B", 0).decode('ascii')) + str(port) + str(struct.pack("bb", 0, 0).decode('ascii')))
            
            ni_packet = packet_type + str(struct.pack("8b", 0, route_info_ver, ni_ver, num_entries, talk_mode, 0, 0, num_rest_nodes).decode('ascii'))
            
            ni_packet += (str(struct.pack(">L", len(route_data) - 4).decode('ascii')) + route_data)
            
            ni_packet = (str(struct.pack(">L", len(ni_packet)).decode('ascii')) + ni_packet)
            ...
            setup = str(struct.pack(">BBH", 4, 1, int(port)).decode('ascii')) + addr_aton(host) + paynspray.rando.rand_text_alpha(random.randrange(8) + 1) + "\x00"
            size = sock.put(setup)
            if size != len(setup):
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Failed to send the entire packet to the proxy server")
            ...
            auth_methods = str(struct.pack("BBB", 5, 1, 0).decode('ascii'))
            ...
            if is_ipv4(host):
                ...
                setup = str(struct.pack("4B", 5, 1, 0, 1).decode('ascii')) + addr + str(struct.pack(">H", int(port)).decode('ascii'))
            elif supports_ipv6() == True and is_ipv6(host):
                addr = socket.inet_pton(socket.AF_INET6, host)
                setup = str(struct.pack("4B", 5, 1, 0, 4).decode('ascii')) + addr + str(struct.pack(">H", int(port)).decode('ascii'))
            else:
                setup = str(struct.pack("4B", 5, 1, 0, 3).decode('ascii')) + str(struct.pack("B", len(host)).decode('ascii')) + host + str(struct.pack(">H", int(port)).decode('ascii'))
