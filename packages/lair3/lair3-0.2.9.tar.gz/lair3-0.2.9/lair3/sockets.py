...
def resolv_nbo(host):
    ...
    if res is not None:
        if res[0] == socket.AF_INET:
            try:
                return str(socket.inet_pton(socket.AF_INET, res[4][0]).decode('latin-1'))
            except UnicodeEncodeError:
                return socket.inet_pton(socket.AF_INET, res[4][0])
        elif res[0] == socket.AF_INET6:
            try:
                return str(socket.inet_pton(socket.AF_INET6, res[4][0]).decode('latin-1'))
            except UnicodeEncodeError:
...
def addr_ntoa(addr):
    ...
    if len(addr) == 4:
        try:
            retval = '.'.join(str(oct) for oct in struct.unpack('4B', bytes(addr)))
        except TypeError:
            retval = '.'.join(str(oct) for oct in struct.unpack('4B', bytes(addr, 'latin-1')))
        return retval
        
    if len(addr) == 16:
        try:
            retval = ':'.join(('%x' % int(i)) for i in struct.unpack('>8H', bytes(addr)))
        except TypeError:
            retval = ':'.join(('%x' % int(i)) for i in struct.unpack('>8H', bytes(addr, 'latin-1')))
...
def addr_iton(addr, v6 = False):
    ...
    if addr < 0x100000000 and v6 is False:
        return str(struct.pack('>L', addr).decode('latin-1'))
    else:
        w = []
        ...
        w[3] = addr & 0xffffffff
        return str(struct.pack('>4L', *w).decode('latin-1'))
    
def addr_ntoi(addr):
    """Converts a NBO address to an integer"""
    try:
        bits = struct.unpack(">{}L".format(len(addr) / 4), bytes(addr))
    except TypeError:
        bits = struct.unpack(">{}L".format(len(addr) / 4), bytes(addr, 'latin-1'))
...
def eth_aton(mac):
    return str(struct.pack("6B", *map(lambda c: int(c, 16), str(mac).split(':'))).decode('latin-1'))

def eth_ntoa(binary):
    try: 
        return ':'.join(map(lambda x: "%.2x" % x, struct.unpack("6B", bytes(binary)))).upper()
    except TypeError:
        return ':'.join(map(lambda x: "%.2x" % x, struct.unpack("6B", bytes(binary, 'latin-1')))).upper()
...
def bit2netmask(bitmask, ipv6 = False):
    if bitmask > 32 or ipv6:
        ...
        tmp1 = str(struct.pack('>L', *[(~((2 ** (32 - bitmask)) - 1)) & 0xFFFFFFFF]).decode('latin-1'))
        try:
            octets = struct.unpack('4B', bytes(tmp1))
        except TypeError:
            octets = struct.unpack('4B', bytes(tmp1, 'latin-1'))
...
class IpSock(SockBase):
    ...
    def sendto(self, gram, peerhost, flags = 0):
        ...
        if (amncore.compat.is_freebsd() == True or amncore.compat.is_netbsd() == True or amncore.compat.is_macosx() == True):
            # byteswap
            try:
                tmp_unpakt = struct.unpack(">H", bytes(gram[2:4]))
            except TypeError:
                tmp_unpakt = struct.unpack(">H", bytes(gram[2:4], 'latin-1'))
            tmp_pakt = str(struct.pack("@h", *tmp_unpakt).decode('latin-1'))
            
            try:
                tmp_unpakt2 = struct.unpack(">H", bytes(gram[6:8]))
            except TypeError:
                tmp_unpakt2 = struct.unpack(">H", bytes(gram[6:8], 'latin-1'))
            tmp_pakt2 = str(struct.pack("@h", *tmp_unpakt2).decode('latin-1'))
...
class LocalComm(SocketComm):
        ...
        if ((param.localport is not None) and (param.localport != 0)) or (param.localhost is not None):
            try:
                ...
                if e.errno == errno.EADDRNOTAVAIL or e.errno == errno.EADDRINUSE:
                    sock.close()
                    raise amncore.xcepts.BindError(param.localhost, param.localport)
        ...
        if param.server == True:
            ...
            if param.peerhost is not None:
                ...
                try:
                    ...
                    sock.close()
                    raise amncore.xcepts.ConnectionTimeout(ip, port)
                except socket.error as e:
                    if (e.errno == errno.EHOSTUNREACH or e.errno == errno.ENETDOWN or e.errno == errno.ENETUNREACH or e.errno == errno.ENETRESET or e.errno == errno.EHOSTDOWN or e.errno == errno.EACCES or e.errno == errno.EINVAL or e.errno == errno.ENOPROTOOPT):
                        sock.close()
                        raise amncore.xcepts.HostUnreachable(ip, port)
                    elif (e.errno == errno.EADDRNOTAVAIL or e.errno == errno.EADDRINUSE):
                        sock.close()
                        raise amncore.xcepts.InvalidDestination(ip, port)
                        ...
                        sock.close()
                        raise amncore.xcepts.ConnectionRefused(ip, int(port))
                finally:
    ...
    def proxy(self, sock, type, host, port):
        if type.lower() == 'sapni':
            ...
            first_route_item = shost + str(struct.pack("B", 0).decode('latin-1')) + str(sport) + str(struct.pack("bb", 0, 0).decode('latin-1'))
            route_data = str(struct.pack(">L", len(first_route_item)).decode('latin-1')) + first_route_item
            route_data += (host + str(struct.pack("B", 0).decode('latin-1')) + str(port) + str(struct.pack("bb", 0, 0).decode('latin-1')))
            
            ni_packet = packet_type + str(struct.pack("8b", 0, route_info_ver, ni_ver, num_entries, talk_mode, 0, 0, num_rest_nodes).decode('latin-1'))
            
            ni_packet += (str(struct.pack(">L", len(route_data) - 4).decode('latin-1')) + route_data)
            
            ni_packet = (str(struct.pack(">L", len(ni_packet)).decode('latin-1')) + ni_packet)
            
            size = sock.put(ni_packet)
            
            if size != len(ni_packet):
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Failed to send the entire packet to the proxy server")
                
            try:
                ret_len = struct.unpack(">L", bytes(sock.get_once(4, 30)))[0]
                if ret_len and ret_len != 0:
                    ret = sock.get_once(ret_len, 30)
            except TypeError:
                ret_len = struct.unpack(">L", bytes(sock.get_once(4, 30), 'latin-1'))[0]
                if ret_len and ret_len != 0:
                    ret = sock.get_once(ret_len, 30)
            except IOError:
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Failed to receive a response from the proxy server")
            except struct.error:
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Failed to parse a response from the proxy server")
                
            if ret and len(ret) < 4:
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Failed to receive a complete response from the proxy server")
                
            if re.search(r'NI_RTERR', ret) is not None:
                if re.search(r'timed out', ret) is not None:
                    raise amncore.xcepts.ConnectionProxyError(host, port, type, "Connection to remote host {} timed out".format(host))
                elif re.search(r'refused', ret) is not None:
                    raise amncore.xcepts.ConnectionProxyError(host, port, type, "Connection to remote port {} refused".format(port))
                elif re.search(r'denied', ret) is not None:
                    raise amncore.xcepts.ConnectionProxyError(host, port, type, "Connection to {}:{} blocked by ACL".format(host, port))
                else:
                    raise amncore.xcepts.ConnectionProxyError(host, port, type, "Connection to {}:{} failed (unknown error)".format(host, port))
            ...
            if resp.code != 200:
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Proxy server responded with non-OK code ({} {})".format(resp.code, resp.message))
        elif type.lower() == 'socks4':
            setup = str(struct.pack(">BBH", 4, 1, int(port)).decode('latin-1')) + addr_aton(host) + paynspray.rando.rand_text_alpha(random.randrange(8) + 1) + "\x00"
            size = sock.put(setup)
            if size != len(setup):
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Failed to send the entire packet to the proxy server")
            ...
            if ret[1:2] != '\x5a':
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Proxy server responded with error code {}".format(struct.unpack("B", str(bytes(ret[0:1], 'latin-1')))[0]))
        elif type.lower() == 'socks5':
            auth_methods = str(struct.pack("BBB", 5, 1, 0).decode('latin-1'))
            ...
            if ret is not None and ret[1:2] == "\xff":
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "The proxy server requires authentication")
            elif ret is None:
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Failed to receive a response from the proxy server")
            
            if is_ipv4(host):
                addr = socket.inet_pton(socket.AF_INET, host)
                setup = str(struct.pack("4B", 5, 1, 0, 1).decode('latin-1')) + addr + str(struct.pack(">H", int(port)).decode('latin-1'))
            elif supports_ipv6() == True and is_ipv6(host):
                addr = socket.inet_pton(socket.AF_INET6, host)
                setup = str(struct.pack("4B", 5, 1, 0, 4).decode('latin-1')) + addr + str(struct.pack(">H", int(port)).decode('latin-1'))
            else:
                setup = str(struct.pack("4B", 5, 1, 0, 3).decode('latin-1')) + str(struct.pack("B", len(host)).decode('latin-1')) + host + str(struct.pack(">H", int(port)).decode('latin-1'))
            ...
            if response[1:2] != "\x00":
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Proxy server responded with error code {}".format(struct.unpack("B", str(bytes(ret[1:2], 'latin-1')))[0]))
        else:
