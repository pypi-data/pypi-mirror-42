...
class LocalComm(SocketComm):
    ...
    def proxy(self, sock, type, host, port):
        if type.lower() == 'sapni':
            ...
            if ret[1:2] != '\x5a':
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Proxy server responded with error code {}".format(struct.unpack("B", bytes(ret[0:1]))[0]))
            ...
            if response[1:2] != "\x00":
                raise amncore.xcepts.ConnectionProxyError(host, port, type, "Proxy server responded with error code {}".format(struct.unpack("B", bytes(ret[1:2]))[0]))
        else:
