...
class Server(object):
    ...
    def get_entries_to_update(self):
        ...
        for dn in sorted(self.new_entries):
            ...
            new_entry = self.new_entries[dn]

            # sort mail emails
            for e in [entry, new_entry]:
                if 'mail' in e and e['mail']:
                    e['mail'] = sorted(e['mail'])
    ...
    def add_entries(self, add):
        ...
        if not add:
            return
        print('  Found %s entries to add.' % (len(add)))
    ...
    def delete_entries(self, delete):
        ...
        if not delete:
            return
        print('  Found %s entries to delete.' % (len(delete)))
    ...
    def update_entries(self, update):
        ...
        print('  Found %s entries to update' % (len(update)))
        for item in sorted(update, key=lambda x: x['dn']):
            ...
            for key in sorted(keys):
                ...
                # TEMPORARILY skip some keys
                if key in [
                    # 'info',         # google+ url
                    # 'manager',      # servicenow manager field is bad in test
                    # 'secretary',    # asistant dn
                    # 'telexNumber',  # asistant phone
                ]:
                    continue
                ...
                # order of emails in "mail" doesn't matter
                # if key == 'mail':
                #     o = sorted(o)
                #     n = sorted(n)
