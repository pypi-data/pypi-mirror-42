...
class Google(Server):
    ...
    def get_person_dn(self, person):
        ...
        if person.terminated and person.hold:
            ...
            base_ou = 'ou=Suspended'
        else:
            base_ou = 'ou=People'

        # get ou
        ou = '%s,%s' % (base_ou, self.ldap.base_dn)

        # get trusted tester
        if person.trusted_tester and not person.terminated:
            ou = 'ou=%s,%s' % (person.trusted_tester, ou)
        return 'uid=%s,%s' % (person.username, ou)
    ...
    def get_person(self, person, supervisor_dn):
        ...
        primaryEmail = '%s@broadintitute.org' % (username)
        ...
        # get primary and secondary domain from ldap objet
        ...
        if primary:
            ...
            # get email
            email = '%s@%s' % (str(email_username), str(primary))
            if email not in emails:
                emails.append(email)

            # get primaryEmail
            primaryEmail = '%s@%s' % (str(username), str(primary))
            if primaryEmail not in emails:
                emails.append(primaryEmail)

            # get primary nickname emails
            for nickname in nicknames:
                primary_nickname = '%s@%s' % (str(nickname), str(primary))
                if primary_nickname not in emails:
                    emails.append(primary_nickname)

        # add secondary domain email
        if secondary:

            # get secondary email
            secondary_email = '%s@%s' % (str(username), str(secondary))
            if secondary_email not in emails:
                emails.append(secondary_email)

            # get secondary nickname emails
            for nickname in nicknames:
                secondary_nickname = '%s@%s' % (str(nickname), str(secondary))
                if secondary_nickname not in emails:
                    emails.append(secondary_nickname)

        # convert object to ldap record
        ldap_person = person.to_google(supervisor_dn)

        # add in carLicenses (g suite license)
        ldap_person['carLicense'] = ['Google-Apps-Lite']

        # employees+ get enterprise
        if person.type in ['Core Member', 'Employee', 'Institute Member']:
            ldap_person['carLicense'] = ['Google-Apps-Enterprise']
    ...
    def prepare_accounts(self):
        ...
        for account in self.update.accounts_records:
            ...
            # get secondary email
            secondary = self.ldap.domains.get('secondary')
            if email and secondary:
                # create secondary email by replacing primary domain with secondary
                secondary_email = email.replace(primary, secondary)
                # if that didn't work, do the opposite for accounts that use the secondary domain
                if secondary_email == email and secondary_email:
                    secondary_email = email.replace(secondary, primary)
                if secondary_email:
    ...
    def prepare_contacts(self):
        ...
        for oid in self.update.shared_contacts:
            ...
            # set cn and dn
            cn = '%s %s' % (contact['first_name'], contact['last_name'])
            ou = 'ou=Contacts,ou=Shared Contacts,%s' % (self.ldap.base_dn)
            dn = 'cn=%s,%s' % (cn, ou)
            ...
            if contact.get('phone'):
                record['telephoneNumber'] = [contact['phone']]
    ...
    def prepare_people(self):
        ...
        for pid in self.update.people_records:
            ...
            if person.start_date > start:
                continue

            # skip people are are past 90 days termination
            if not person.hold and (person.end_date and person.end_date < end):
                continue
    ...
    def prepare_resources(self):
        ...
        for resource in self.update.resources:
            ...
            room = building_room.split('-')[1].strip()
            name = str(resource['resourceName'].split(' (')[0].strip())

            # create record
            record = {
                'cn': [resource['resourceName']],
                'description': [resource['resourceDescription']],
                'email': [resource['resourceEmail']],
                'employeeNumber': [resource['resourceId']],
                'givenName': [name],
                # 'l': ['Cambridge'],
                'objectClass': [
                    'inetOrgPerson',
