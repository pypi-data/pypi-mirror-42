...
def export_markers(depth_markers, markers, N_files, las_files):

    """
    Ex.: export_markers(depth_markers, markers, N_files, las_files)
    ...
    Depths_markers = np.asarray([depth_markers])
...
def freq_corr(DEPTH, gain, f=20, length=0.512, dt=0.0004):
    """
    Ex.: seism_freq = pytie.freq_corr(DEPTH, gain, f, length, dt)
    ...
    freq_high = f*1.6
    freq_low = f*0.6
    idx_high = np.where(DEPTH >= 500)
    idx_low = np.where(DEPTH >= 1500)
    
    t = np.arange(-length/2, length/2, dt)
    wavelet = (1.-2.*(np.pi**2)*(f**2)*(t**2))*np.exp(-(np.pi**2)*(f**2)*(t**2))
    wavelet1 = (1.-2.*(np.pi**2)*(freq_high**2)*(t**2))*np.exp(-(np.pi**2)*(freq_high**2)*(t**2))
    wavelet2 = (1.-2.*(np.pi**2)*(freq_low**2)*(t**2))*np.exp(-(np.pi**2)*(freq_low**2)*(t**2))
    
    seismogram1 = np.convolve(gain[:idx_high[0][0]], wavelet1, mode='same')
    seismogram2 = np.convolve(gain[idx_high[0][0]:idx_low[0][0]], wavelet, mode='same')
    seismogram3 = np.convolve(gain[idx_low[0][0]:], wavelet2, mode='same')
    seism_freq = np.concatenate((seismogram1, seismogram2, seismogram3), axis=0)
