...
def take_coordinates(data, result, color, graph1=None, equiv=None):
    ...
    if result:
        ...
        gv.render('dot', format='plain',
                  filepath=os.path.dirname(os.path.realpath(__file__)) + os.sep + 'graph.gv')
    else:
        data1 = tc.numerate_nodes_in_edge(data)
...
def parse_documents_coordinates(data, result):
    ...
    '''
    x = open(os.path.dirname(os.path.realpath(__file__)) + os.sep + 'graph.gv.plain', 'r')
    txt = x.read()
    x.close()
    ...
    if result:
        ...
        doc.save_ods(final_matrix, proyect_path + os.sep + 'files' + os.sep + 'graph_viz_coord.ods')
    else:
        data1 = tc.numerate_nodes_in_edge(data)
    ...
    return proyect_path + os.sep + 'files' + os.sep + 'graph_viz_coord.ods'
