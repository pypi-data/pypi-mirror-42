...
class CryptoICA(BaseCryptoService):
    ...
    def sign(self, session, alias, fingerprint, password=None, hash_alg=None, sign_alg=None):
        ...
        self.server.cmd(session, '00A4000C023F00', reader[0]['reader'], 1)
        response_all = self.server.cmd(session, '00A4010C020604', reader[0]['reader'])
        ...
        if password is not None:
            ...
            if (reader[0]['pin'] is not None) and (reader[0]['pin'] == encoded_password):
                ...
                response_all = self.server.cmd(session, password_command, reader[0]['reader'])
        ...
        if response_data is None:
            ...
            while keep_trying:
                # h MSE:Set sha-256
                self.server.cmd(session, '002241AA04890214%s' % sha_id, reader[0]['reader'])
                # h MSE:Set DST
                ...
                if response_data is None:
                    ...
                    response_all = self.server.cmd(session, "002A9E9A00", reader[0]['reader'])
                    response_data = self.check_response(response_all, reader[0]['reader'],
    ...
    def _read_certificates(self, session):
        ...
        for reader in self.storage.certificates.keys():
            processThread = threading.Thread(target=self._process_new_card, args=(reader, self.storage, self.server))
            processThread.start()
            list_of_threads.append(processThread)
        for each_process in list_of_threads:
            each_process.join()

#            self._process_new_card(reader)
        pass
    ...
    def _process_new_card(self, reader, storage, server):
        ...
        while new_item:
            ...
            if self.check_response(response_all, reader, "00B2%02X0400 error" % certificate_id, error=0) \
                    is not None:
                break
            ...
            while (len(raw_response) > 2) and ((file_id is None) or (cert_len == 0)):
                ...
                if raw_response[0] == 0x12:  # this is the length of the certificate we expect
                    cert_len = ((raw_response[2] * 256 + raw_response[3]) * 256 + raw_response[4]) * 256 + raw_response[
                        5]
            ...
            if len(response_all) > 0 and response_all[0][-4:] == '9000':
                ...
                if cert_len <= 0:  # it should end up in cert_len == 0
                    ...
                    if ext and ext.value and ext.value.ca:
                        ...
                        storage.card_cas[reader].append({
                            'name': subject,
                            'issuer': issuer,
                        ...
                        if issued_at > latest_cert_time:
                            storage.certificates[reader]['cert'] = \
        ...
        if end_subject == "" or end_issuer == "":
            ...
            while not root:
                next_found = False
                for ca_cert in storage.card_cas[reader]:
            ...
            if len(chain) < 1:
                ...
                storage.certificates[reader]['chain'] = chain
