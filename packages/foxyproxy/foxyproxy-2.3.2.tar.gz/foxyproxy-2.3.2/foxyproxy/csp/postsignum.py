...
class CryptoPostSignum(BaseCryptoService):
    ...
    def sign(self, session, alias, fingerprint, password=None, hash_alg=None, sign_alg=None):
        ...
        if password is not None:
            ...
            if (reader[0]['pin'] is not None) and (reader[0]['pin'] == encoded_password):
                ...
                # select FILE
                self.server.cmd(session, '00A408000614009001200200', reader[0]['reader'])
        ...
        if not pin_ok:
            ...
            self.server.cmd(session, '0022F303', reader[0]['reader'])
            # set security context
            response_all = self.server.cmd(session, '0022F1B80383010E', reader[0]['reader'])
            ...
            if response_all[0][-4:] == "9000":
                ...
                self.server.cmd(session, '102A8086%s00' % padding1, reader[0]['reader'])
                # second half of the data
                response_all = self.server.cmd(session, '102A8086%s00' % padding2, reader[0]['reader'])
    ...
    def _read_certificates(self, session):
        ...
        for reader in self.storage.certificates.keys():
            ...
            ##########################
            error = False
            response_all = self.server.cmd(session, '00A4040010A0000000770103000610000000000002', reader, 0)
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True
            response_all = self.server.cmd(session, '00A408000410001003', reader, 0)
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True
            response_all = self.server.cmd(session, '00B0000010', reader, 0)  # some kind of card ID
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True

            # select certificate store
            response_all = self.server.cmd(session, '00A408000614009001200200', reader, 0)
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True
            ...
            self.server.cmd(session, '00A40004023F0000', reader, 0)
