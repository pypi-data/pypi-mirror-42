...
class SmartOpenFileObjTest(unittest.TestCase):
    ...
    def test_write_str(self):
        ...
        with smart_open.smart_open(buffer, 'w') as sf:
            sf.write(self.text)
            self.assertEqual(buffer.getvalue(), self.text)
    ...
    @patch_string_io
    def test_read_bytes_from_str(self):
        """Can we read bytes from a text stream?"""
        buffer = six.StringIO(self.text)
        with smart_open.smart_open(buffer, 'rb') as sf:
            data = sf.read()
        self.assertEqual(data.decode(), self.text)

    @patch_string_io
    def test_write_bytes_to_str(self):
        """Can we write bytes to a text stream?"""
        buffer = six.StringIO()
        with smart_open.smart_open(buffer, 'wb') as sf:
            sf.write(self.text.encode())
            self.assertEqual(buffer.getvalue(), self.text)
