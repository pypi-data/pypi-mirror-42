...
def render_template(filepath, app, version):
    template = open(filepath, 'r').read()

    rendered_template = Template(template).render(app=app, version=version)
...
def main():
    ...
    app = arg_opts['app']
    version = arg_opts['version']
    for dirpath, _, filenames in os.walk(template_path):
        for f in filenames:
            filepath = os.path.join(dirpath, f)
            render_template(filepath, app, version)
