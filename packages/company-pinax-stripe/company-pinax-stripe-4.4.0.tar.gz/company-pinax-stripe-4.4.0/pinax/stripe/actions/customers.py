...
def _create_without_account(company, card=None, plan=settings.PINAX_STRIPE_DEFAULT_PLAN, charge_immediately=True, quantity=None):
    cus = models.Customer.objects.filter(company=company).first()
    if cus is not None:
        try:
    ...
    trial_end = hooks.hookset.trial_period(company, plan)
    stripe_customer = stripe.Customer.create(
        email=company.main_email_contact,
        source=card,
        plan=plan,
        quantity=quantity,
        trial_end=trial_end
...
def create(company, card=None, plan=settings.PINAX_STRIPE_DEFAULT_PLAN, charge_immediately=True, quantity=None, stripe_account=None):
    """
    ...
    if stripe_account is None:
        return _create_without_account(company, card=card, plan=plan, charge_immediately=charge_immediately, quantity=quantity)
    return _create_with_account(company, stripe_account, card=card, plan=plan, charge_immediately=charge_immediately, quantity=quantity)
...
def get_customer_for_user(user, stripe_account=None):
    ...
    """
    if stripe_account is None:
        return models.Customer.objects.filter(company=user.company).first() #CHANGED
