...
class SpellKernel(object):
    ...
    def __init__(self, run_id, conda_env, local_root, remote_root, ssh_host, ssh_port, api_opts, kernel_spec, username,
                 owner=None, keys=None, verbose=False):
        self.run_id = run_id
    ...
    def track_run_status(self):
        try:
            r_client = runs_client.RunsClient(owner=self.owner or self.username, **self.api_opts)
    ...
    def _build_ssh_config(self):
        ssh_config = {
            ...
            "port": self.ssh_port,
            "username": "{}#{}".format(self.owner or self.username, self.run_id),
