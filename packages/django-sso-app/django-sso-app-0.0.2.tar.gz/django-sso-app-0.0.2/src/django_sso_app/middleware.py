...
class SsoMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        if request.user.is_authenticated:
            ...
            if user_profile.sso_id == sso_id:
                if request.user.is_active:
                    ...
                    if rev_changed:
                        logger.info("Rev changed from {} to {} for {}, updating ...".format(user_profile.sso_rev,
                                                          decoded_jwt["rev"],
                                                          consumer_username))
                        ...
                        logger.info("{} updated with latest data from SSO".format(consumer_username))
        ...
        user = auth.authenticate(consumer_username=consumer_username, encoded_jwt=encoded_jwt, decoded_jwt=decoded_jwt)

        # Redirect user to Term Of Service.
        is_to_subscribe = getattr(user, "is_to_subscribe", False)
        logger.info('IS {} TO SUBSCRIBE? {}'.format(user, is_to_subscribe))
        if user and is_to_subscribe:
            ...
            logger.info("User {username} must agree to the Terms of Service, "
                        "redirecting to {url} ...".format(username=user.username,
                                                          url=_url))
        ...
        if user and user.is_active:
            ...
            logger.info("User {username} was authenticated successfully!".format(username=user.username))
