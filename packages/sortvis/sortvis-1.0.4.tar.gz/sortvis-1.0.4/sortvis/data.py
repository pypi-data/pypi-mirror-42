...
class SortData:
    ...
    MAX_IM_SIZE = 500
    def __init__(self, Length, time_interval=1, sort_title="Figure", is_resampling=False, is_sparse=False, record=False, color_theme="blue", fps=25):
        ...
        self.SetColorTheme(color_theme)
        self.Getfigure()
        self.InitTime()
    ...
    def SetColorTheme(self, color_theme):
    ...
    def Getfigure(self):
        ...
        for i in range(self.length):
            val = self.data[i]
            figure[-1-val*_bar_width:, i*_bar_width:i*_bar_width+_bar_width] = self.GetColor(val, self.length, self.color_theme)
    ...
    def _set_figure(self, idx, val):
        ...
        self.figure[ min_row: , min_col:max_col] = self.GetColor(val, self.length, self.color_theme)
    ...
    def Visualize(self, mark1=None, mark2=None):
        ...
        self.GetTime()
        cv2.putText(img, self.sort_title+" Time:%02.2fs"%self.time, (20,20), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, self.BLACK, 1)
...
if __name__ == "__main__":
    sv = SortData(64, is_sparse=True)
    sv.Visualize()

    for i in range(64):
        for j in range(i,64):
            if sv.data[i]>sv.data[j]:
                sv.Swap(i,j)
                sv.Visualize( (j,i) )
