...
class effective_sql:
    ...
    def __init__(self, kwargs=None):
        ...
        self.connect_timeout = kwargs['connect_timeout'] if 'connect_timeout' in kwargs else 120
    ...
    def _conn(self):
        logging.info('create %s connection .' % self.db)
    ...
    def _atomic_action(self, sql, params, detail):
        ...
        while retry_cnt < self.execute_retries:
            try:
                ...
                logging.debug('[%s/%s:(%s->%s)/%s]' % detail + ' %s time success .' % retry_cnt)
                self.conn.commit()
                error = None
                break
            except Exception, e:
                ...
                retry_cnt += 1
                error = e
                logging.error('[%s/%s:(%s->%s)/%s]' % detail + ' %s time fail for %s.' % (retry_cnt, e.message))
    ...
    def _batch_action(self, sql, params, batch, pojo, optimized=True):
        ...
        for start, end in idxes:
            detail = (submits, total_submit, start, end, total_records)
            if not optimized:
                # # 单线程
