...
def read(socket, n=4096):
    ...
    if pi._requires.six.PY3 and not isinstance(socket, NpipeSocket):
        select.select([socket], [], [])

    try:
        if hasattr(socket, 'recv'):
            return socket.recv(n)
        if pi._requires.six.PY3 and isinstance(socket, getattr(pysocket, 'SocketIO')):
...
def consume_socket_output(frames, demux=False):
    ...
    if demux is False:
        ...
        return pi._requires.six.binary_type().join(frames)
