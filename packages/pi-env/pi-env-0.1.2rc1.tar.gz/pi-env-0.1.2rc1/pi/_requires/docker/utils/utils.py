...
def convert_filters(filters):
    ...
    for k, v in pi._requires.six.iteritems(filters):
        ...
        if not isinstance(v, list):
            v = [v, ]
        result[k] = [
            str(item) if not isinstance(item, pi._requires.six.string_types) else item
            for item in v
        ]
...
def format_extra_hosts(extra_hosts, task=False):
    ...
    if task:
        return [
            '{} {}'.format(v, k) for k, v in sorted(pi._requires.six.iteritems(extra_hosts))
        ]

    return [
        '{}:{}'.format(k, v) for k, v in sorted(pi._requires.six.iteritems(extra_hosts))
    ]
