...
class CancellableStream(object):
    ...
    def __next__(self):
        ...
        except pi._requires.urllib3.exceptions.ProtocolError:
            raise StopIteration
        except socket.error:
            raise StopIteration
    ...
    def close(self):
        ...
        if not self._response.raw.closed:
            ...
            if hasattr(pi._requires.urllib3.contrib, 'pyopenssl') and isinstance(
                    sock, pi._requires.urllib3.contrib.pyopenssl.WrappedSocket):
                sock = sock.socket
