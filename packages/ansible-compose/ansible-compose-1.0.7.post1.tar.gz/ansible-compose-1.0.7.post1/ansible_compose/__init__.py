...
def playbook():
    ...
    for num, compose_src in enumerate(console_script.composes):
        ...
        # consolidate a list of services
    ...
    if console_script.dsn.username:
        argv.append(f'--user={self.dsn.username}')

    #argv += ['-e', f'compose={os.getcwd()}/ansible-compose.yml']
    #argv += ['-e', 'ansible_python_interpreter=/usr/bin/python3']
...
def compose(command='up'):
    ...
    if console_script.host == 'localhost':
        ...
        args = [
            'ssh',
            '@'.join([dsn.username or os.getenv('USER'), host]),
