...
@click.option(
    "--increment",
    is_flag=True,
    help="Enables auto increment of version number (must comply with semantic versioning).",
)
@click.pass_obj
def deploy(obj, path, *, increment=False, commit, **kwargs):
    """Deploy a new algorithm release.
    
    The algorithm must be structured in the correct way and packaged as a .zip file.
    The package must also contain the alfa.yml configuration file.
    ...
    client = obj["client"]
    conf = AlfaConfigHelper.load(path, is_package=True)
    ...
    res = environment.deploy(
        version, path, increment=increment, description=description, release_notes=notes
    )
