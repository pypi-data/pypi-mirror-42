...
class DeepOCClassifier:
    ...
    def __train_dll(self, total_epoch, epochs_per_test, batch_size, inf_loss_decreasing):
        headers = self.__features
        model = self.__build_estimator(headers)
        total_rounds = total_epoch // epochs_per_test
        mile_stone = 100000
        n = 0
        while n < total_rounds:
            ...
            if inf_loss_decreasing and n == total_rounds - 1 and mile_stone > previous_loss:
                total_rounds += 100
