...
# try:
#     from tools.gencode import GenDsigitalCode
# except Exception as e:
#     from ..tools.gencode import GenDsigitalCode
...
class CodeRecord(Model):
    ...
    def isvalid(mobile='',code='',tag=''):
        valid = False
        try:
            record = CodeRecord.objects.get(mobile=mobile,tag=tag,code=code)
            time = record.created
    ...
    def fastCheeck(request):
        try:
            ...
            tag = params.get('type','')
            result = CodeRecord.isvalid(mobile=mobile,code=code,tag=tag)
            if result[0]:
