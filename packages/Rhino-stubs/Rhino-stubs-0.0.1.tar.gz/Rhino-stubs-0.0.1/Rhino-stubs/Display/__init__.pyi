from typing import Tuple, Set, Iterable, List

class VisualAnalysisMode:
    @property
    def RhinoEdgeAnalysisModeId () -> Guid: ...
    @property
    def RhinoCurvatureGraphAnalysisModeId () -> Guid: ...
    @property
    def RhinoZebraStripeAnalysisModeId () -> Guid: ...
    @property
    def RhinoEmapAnalysisModeId () -> Guid: ...
    @property
    def RhinoCurvatureColorAnalyisModeId () -> Guid: ...
    @property
    def RhinoDraftAngleAnalysisModeId () -> Guid: ...
    @property
    def RhinoThicknessAnalysisModeId () -> Guid: ...
    def Register (customAnalysisModeType : Type) -> VisualAnalysisMode: ...
    def Find (id : Guid) -> VisualAnalysisMode: ...
    def Find (t : Type) -> VisualAnalysisMode: ...
    @property
    def Name (self) -> str: ...
    @property
    def Style (self) -> AnalysisStyle: ...
    @property
    def Id (self) -> Guid: ...
    def EnableUserInterface (self, on : bool): ...
    def ObjectSupportsAnalysisMode (self, obj : RhinoObject) -> bool: ...
    @property
    def ShowIsoCurves (self) -> bool: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class PointStyle:
    Square = 0
    Simple = 0
    ControlPoint = 1
    ActivePoint = 2
    X = 3
    RoundSimple = 4
    Circle = 4
    RoundControlPoint = 5
    RoundActivePoint = 6
    Triangle = 7
    Heart = 8
    Chevron = 9
    Clover = 10
    Tag = 11
    Asterisk = 12
    Pin = 13
    ArrowTail = 14
    ArrowTip = 15
class BackgroundStyle:
    SolidColor = 0
    WallpaperImage = 1
    Gradient = 2
    Environment = 3
class Color4f:
    def __init__(self, color : Color): ...
    def __init__(self, color : Color4f): ...
    def __init__(self, red : Single, green : Single, blue : Single, alpha : Single): ...
    @property
    def Empty () -> Color4f: ...
    @property
    def Black () -> Color4f: ...
    @property
    def White () -> Color4f: ...
    def FromArgb (a : Single, r : Single, g : Single, b : Single) -> Color4f: ...
    def FromArgb (a : Single, color : Color4f) -> Color4f: ...
    @property
    def R (self) -> Single: ...
    @property
    def G (self) -> Single: ...
    @property
    def B (self) -> Single: ...
    @property
    def A (self) -> Single: ...
    @property
    def L (self) -> Single: ...
    def op_Equality (a : Color4f, b : Color4f) -> bool: ...
    def op_Inequality (a : Color4f, b : Color4f) -> bool: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def BlendTo (self, t : Single, col : Color4f) -> Color4f: ...
    def ApplyGamma (col : Color4f, gamma : Single) -> Color4f: ...
    def AsSystemColor (self) -> Color: ...
    def ToString (self) -> str: ...
    def GetType (self) -> Type: ...
class CustomDisplay:
    def __init__(self, enable : bool): ...
    @property
    def Enabled (self) -> bool: ...
    @Enabled.setter
    def Enabled (self, value : bool): ...
    @property
    def IsDisposed (self) -> bool: ...
    @property
    def ClippingBox (self) -> BoundingBox: ...
    def Clear (self): ...
    def AddPoint (self, point : Point3d): ...
    def AddPoint (self, point : Point3d, color : Color): ...
    def AddPoint (self, point : Point3d, color : Color, style : PointStyle, radius : int): ...
    def AddPoints (self, points : Iterable(Point3d)): ...
    def AddPoints (self, points : Iterable(Point3d), color : Color): ...
    def AddPoints (self, points : Iterable(Point3d), color : Color, style : PointStyle, radius : int): ...
    def AddLine (self, line : Line): ...
    def AddLine (self, line : Line, color : Color): ...
    def AddLine (self, line : Line, color : Color, thickness : int): ...
    def AddVector (self, anchor : Point3d, span : Vector3d): ...
    def AddVector (self, anchor : Point3d, span : Vector3d, color : Color): ...
    def AddVector (self, anchor : Point3d, span : Vector3d, color : Color, drawAnchor : bool): ...
    def AddArc (self, arc : Arc): ...
    def AddArc (self, arc : Arc, color : Color): ...
    def AddArc (self, arc : Arc, color : Color, thickness : int): ...
    def AddCircle (self, circle : Circle): ...
    def AddCircle (self, circle : Circle, color : Color): ...
    def AddCircle (self, circle : Circle, color : Color, thickness : int): ...
    def AddCurve (self, curve : Curve): ...
    def AddCurve (self, curve : Curve, color : Color): ...
    def AddCurve (self, curve : Curve, color : Color, thickness : int): ...
    def AddPolygon (self, polygon : Iterable(Point3d), fillColor : Color, edgeColor : Color, drawFill : bool, drawEdge : bool): ...
    def AddText (self, text : str, plane : Plane, size : float): ...
    def AddText (self, text : str, plane : Plane, size : float, color : Color): ...
    def AddText (self, text : Text3d, color : Color): ...
    def Dispose (self): ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class ColorHSL:
    def __init__(self, rgb : Color): ...
    def __init__(self, hue : float, saturation : float, luminance : float): ...
    def __init__(self, alpha : float, hue : float, saturation : float, luminance : float): ...
    def CreateFromCMYK (cmyk : ColorCMYK) -> ColorHSL: ...
    def CreateFromXYZ (xyz : ColorXYZ) -> ColorHSL: ...
    def CreateFromLAB (lab : ColorLAB) -> ColorHSL: ...
    def CreateFromLCH (lch : ColorLCH) -> ColorHSL: ...
    def CreateFromHSV (hsv : ColorHSV) -> ColorHSL: ...
    def op_Implicit (hsl : ColorHSL) -> Color: ...
    @property
    def H (self) -> float: ...
    @H.setter
    def H (self, value : float): ...
    @property
    def S (self) -> float: ...
    @S.setter
    def S (self, value : float): ...
    @property
    def L (self) -> float: ...
    @L.setter
    def L (self, value : float): ...
    @property
    def A (self) -> float: ...
    @A.setter
    def A (self, value : float): ...
    def ToArgbColor (self) -> Color: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def ToString (self) -> str: ...
    def GetType (self) -> Type: ...
class ColorCMYK:
    def __init__(self, rgb : Color): ...
    def __init__(self, cyan : float, magenta : float, yellow : float): ...
    def __init__(self, cyan : float, magenta : float, yellow : float, key : float): ...
    def __init__(self, alpha : float, cyan : float, magenta : float, yellow : float, key : float): ...
    def CreateFromHSL (hsl : ColorHSL) -> ColorCMYK: ...
    def CreateFromXYZ (xyz : ColorXYZ) -> ColorCMYK: ...
    def CreateFromLAB (lab : ColorLAB) -> ColorCMYK: ...
    def CreateFromLCH (lch : ColorLCH) -> ColorCMYK: ...
    def CreateFromHSV (hsv : ColorHSV) -> ColorCMYK: ...
    def op_Implicit (cmyk : ColorCMYK) -> Color: ...
    @property
    def C (self) -> float: ...
    @C.setter
    def C (self, value : float): ...
    @property
    def M (self) -> float: ...
    @M.setter
    def M (self, value : float): ...
    @property
    def Y (self) -> float: ...
    @Y.setter
    def Y (self, value : float): ...
    @property
    def K (self) -> float: ...
    @K.setter
    def K (self, value : float): ...
    @property
    def A (self) -> float: ...
    @A.setter
    def A (self, value : float): ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def ToString (self) -> str: ...
    def GetType (self) -> Type: ...
class ColorXYZ:
    def __init__(self, rgb : Color): ...
    def __init__(self, x : float, y : float, z : float): ...
    def __init__(self, alpha : float, x : float, y : float, z : float): ...
    def CreateFromHSL (hsl : ColorHSL) -> ColorXYZ: ...
    def CreateFromCMYK (cmyk : ColorCMYK) -> ColorXYZ: ...
    def CreateFromLAB (lab : ColorLAB) -> ColorXYZ: ...
    def CreateFromLCH (lch : ColorLCH) -> ColorXYZ: ...
    def CreateFromHSV (hsv : ColorHSV) -> ColorXYZ: ...
    def op_Implicit (xyz : ColorXYZ) -> Color: ...
    @property
    def X (self) -> float: ...
    @X.setter
    def X (self, value : float): ...
    @property
    def Y (self) -> float: ...
    @Y.setter
    def Y (self, value : float): ...
    @property
    def Z (self) -> float: ...
    @Z.setter
    def Z (self, value : float): ...
    @property
    def A (self) -> float: ...
    @A.setter
    def A (self, value : float): ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def ToString (self) -> str: ...
    def GetType (self) -> Type: ...
class ColorLAB:
    def __init__(self, rgb : Color): ...
    def __init__(self, lightness : float, a : float, b : float): ...
    def __init__(self, alpha : float, lightness : float, a : float, b : float): ...
    def CreateFromHSL (hsl : ColorHSL) -> ColorLAB: ...
    def CreateFromCMYK (cmyk : ColorCMYK) -> ColorLAB: ...
    def CreateFromXYZ (xyz : ColorXYZ) -> ColorLAB: ...
    def CreateFromLCH (lch : ColorLCH) -> ColorLAB: ...
    def CreateFromHSV (hsv : ColorHSV) -> ColorLAB: ...
    def op_Implicit (lab : ColorLAB) -> Color: ...
    @property
    def L (self) -> float: ...
    @L.setter
    def L (self, value : float): ...
    @property
    def A (self) -> float: ...
    @A.setter
    def A (self, value : float): ...
    @property
    def B (self) -> float: ...
    @B.setter
    def B (self, value : float): ...
    @property
    def Alpha (self) -> float: ...
    @Alpha.setter
    def Alpha (self, value : float): ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def ToString (self) -> str: ...
    def GetType (self) -> Type: ...
class ColorLCH:
    def __init__(self, rgb : Color): ...
    def __init__(self, lightness : float, chroma : float, hue : float): ...
    def __init__(self, alpha : float, lightness : float, chroma : float, hue : float): ...
    def CreateFromHSL (hsl : ColorHSL) -> ColorLCH: ...
    def CreateFromCMYK (cmyk : ColorCMYK) -> ColorLCH: ...
    def CreateFromXYZ (xyz : ColorXYZ) -> ColorLCH: ...
    def CreateFromLAB (lab : ColorLAB) -> ColorLCH: ...
    def op_Implicit (lch : ColorLCH) -> Color: ...
    @property
    def L (self) -> float: ...
    @L.setter
    def L (self, value : float): ...
    @property
    def C (self) -> float: ...
    @C.setter
    def C (self, value : float): ...
    @property
    def H (self) -> float: ...
    @H.setter
    def H (self, value : float): ...
    @property
    def A (self) -> float: ...
    @A.setter
    def A (self, value : float): ...
    def MakePositive (self): ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def ToString (self) -> str: ...
    def GetType (self) -> Type: ...
class ColorHSV:
    def __init__(self, rgb : Color): ...
    def __init__(self, hue : float, saturation : float, value : float): ...
    def __init__(self, alpha : float, hue : float, saturation : float, value : float): ...
    def CreateFromHSL (hsl : ColorHSL) -> ColorHSV: ...
    def CreateFromCMYK (cmyk : ColorCMYK) -> ColorHSV: ...
    def CreateFromXYZ (xyz : ColorXYZ) -> ColorHSV: ...
    def CreateFromLAB (lab : ColorLAB) -> ColorHSV: ...
    def CreateFromLCH (lch : ColorLCH) -> ColorHSV: ...
    def op_Implicit (hsv : ColorHSV) -> Color: ...
    @property
    def H (self) -> float: ...
    @H.setter
    def H (self, value : float): ...
    @property
    def S (self) -> float: ...
    @S.setter
    def S (self, value : float): ...
    @property
    def V (self) -> float: ...
    @V.setter
    def V (self, value : float): ...
    @property
    def A (self) -> float: ...
    @A.setter
    def A (self, value : float): ...
    def ToArgbColor (self) -> Color: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def ToString (self) -> str: ...
    def GetType (self) -> Type: ...
class Text3d:
    def __init__(self, text : str): ...
    def __init__(self, text : str, plane : Plane, height : float): ...
    def Dispose (self): ...
    @property
    def Text (self) -> str: ...
    @Text.setter
    def Text (self, value : str): ...
    @property
    def TextPlane (self) -> Plane: ...
    @TextPlane.setter
    def TextPlane (self, value : Plane): ...
    @property
    def Height (self) -> float: ...
    @Height.setter
    def Height (self, value : float): ...
    @property
    def FontFace (self) -> str: ...
    @FontFace.setter
    def FontFace (self, value : str): ...
    @property
    def Bold (self) -> bool: ...
    @Bold.setter
    def Bold (self, value : bool): ...
    @property
    def Italic (self) -> bool: ...
    @Italic.setter
    def Italic (self, value : bool): ...
    @property
    def BoundingBox (self) -> BoundingBox: ...
    @property
    def HorizontalAlignment (self) -> TextHorizontalAlignment: ...
    @HorizontalAlignment.setter
    def HorizontalAlignment (self, value : TextHorizontalAlignment): ...
    @property
    def VerticalAlignment (self) -> TextVerticalAlignment: ...
    @VerticalAlignment.setter
    def VerticalAlignment (self, value : TextVerticalAlignment): ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class BlendMode:
    Zero = 0
    One = 1
    SourceColor = 768
    OneMinusSourceColor = 769
    SourceAlpha = 770
    OneMinusSourceAlpha = 771
    DestinationAlpha = 772
    OneMinusDestinationAlpha = 773
    DestinationColor = 774
    OneMinusDestinationColor = 775
    SourceAlphaSaturate = 776
class DisplayBitmap:
    def __init__(self, bitmap : Bitmap): ...
    def Load (path : str) -> DisplayBitmap: ...
    def SetBlendFunction (self, source : BlendMode, destination : BlendMode): ...
    def GetBlendModes (self) -> Tuple(BlendMode, BlendMode): ...
    def Dispose (self): ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class DisplayBitmapDrawList:
    def __init__(self): ...
    @property
    def BoundingBox (self) -> BoundingBox: ...
    @property
    def MaximumCachedSortLists (self) -> int: ...
    @MaximumCachedSortLists.setter
    def MaximumCachedSortLists (self, value : int): ...
    @property
    def SortAngleTolerance (self) -> float: ...
    @SortAngleTolerance.setter
    def SortAngleTolerance (self, value : float): ...
    def Sort (self, cameraDirection : Vector3d) -> Set(int): ...
    def SetPoints (self, points : Iterable(Point3d)): ...
    def SetPoints (self, points : Iterable(Point3d), blendColor : Color): ...
    def SetPoints (self, points : Iterable(Point3d), colors : Iterable(Color)): ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class DisplayModeDescription:
    def Dispose (self): ...
    def GetDisplayModes () -> Set(DisplayModeDescription): ...
    def GetDisplayMode (id : Guid) -> DisplayModeDescription: ...
    def FindByName (englishName : str) -> DisplayModeDescription: ...
    def AddDisplayMode (displayMode : DisplayModeDescription) -> Guid: ...
    def AddDisplayMode (name : str) -> Guid: ...
    def CopyDisplayMode (id : Guid, name : str) -> Guid: ...
    def UpdateDisplayMode (displayMode : DisplayModeDescription) -> bool: ...
    def DeleteDiplayMode (id : Guid) -> bool: ...
    def ImportFromFile (filename : str) -> Guid: ...
    def ExportToFile (displayMode : DisplayModeDescription, filename : str) -> bool: ...
    @property
    def InMenu (self) -> bool: ...
    @InMenu.setter
    def InMenu (self, value : bool): ...
    @property
    def SupportsShadeCommand (self) -> bool: ...
    @SupportsShadeCommand.setter
    def SupportsShadeCommand (self, value : bool): ...
    @property
    def SupportsShading (self) -> bool: ...
    @SupportsShading.setter
    def SupportsShading (self, value : bool): ...
    @property
    def AllowObjectAssignment (self) -> bool: ...
    @AllowObjectAssignment.setter
    def AllowObjectAssignment (self, value : bool): ...
    @property
    def ShadedPipelineRequired (self) -> bool: ...
    @ShadedPipelineRequired.setter
    def ShadedPipelineRequired (self, value : bool): ...
    @property
    def WireframePipelineRequired (self) -> bool: ...
    @WireframePipelineRequired.setter
    def WireframePipelineRequired (self, value : bool): ...
    @property
    def PipelineLocked (self) -> bool: ...
    @PipelineLocked.setter
    def PipelineLocked (self, value : bool): ...
    @property
    def DisplayAttributes (self) -> DisplayPipelineAttributes: ...
    @property
    def EnglishName (self) -> str: ...
    @EnglishName.setter
    def EnglishName (self, value : str): ...
    @property
    def Id (self) -> Guid: ...
    @property
    def LocalName (self) -> str: ...
    @property
    def GhostedId () -> Guid: ...
    @property
    def PenId () -> Guid: ...
    @property
    def RenderedId () -> Guid: ...
    @property
    def RenderedShadowsId () -> Guid: ...
    @property
    def ShadedId () -> Guid: ...
    @property
    def TechId () -> Guid: ...
    @property
    def WireframeId () -> Guid: ...
    @property
    def XRayId () -> Guid: ...
    @property
    def AmbientOcclusionId () -> Guid: ...
    @property
    def RaytracedId () -> Guid: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class DrawFrameStages:
    InitializeFrameBuffer = 1
    SetupFrustum = 2
    ObjectCulling = 4
    CalculateBoundingBox = 8
    CalculateClippingPlanes = 16
    SetupLighting = 32
    DrawBackground = 64
    PreDrawObjects = 128
    DrawObject = 256
    PostDrawObjects = 512
    DrawMiddleGround = 896
    DrawForeGround = 1024
    DrawOverlay = 2048
    PostProcessFrameBuffer = 4096
    MeshingParameters = 8192
    ObjectDisplayAttributes = 16384
    PreObjectDraw = 32768
    PostObjectDraw = 65536
    ObjectBasedChannel = 114948
    ViewExtents = 131072
    All = 4294836223
class DisplayConduit:
    @property
    def Enabled (self) -> bool: ...
    @Enabled.setter
    def Enabled (self, value : bool): ...
    @property
    def SpaceFilter (self) -> ActiveSpace: ...
    @SpaceFilter.setter
    def SpaceFilter (self, value : ActiveSpace): ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class DisplayEngine:
    def Dispose (self): ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class DepthMode:
    Neutral = 0
    AlwaysInFront = 1
    AlwaysInBack = 2
class ZBiasMode:
    Neutral = 0
    TowardsCamera = 1
    AwayFromCamera = 2
class CullFaceMode:
    DrawFrontAndBack = 0
    DrawFrontFaces = 1
    DrawBackFaces = 2
class DisplayPipeline:
    def Draw2dText (self, text : str, color : Color, worldCoordinate : Point3d, middleJustified : bool, height : int, fontface : str): ...
    def Draw3dText (self, text : str, color : Color, textPlane : Plane, height : float, fontface : str, bold : bool, italic : bool, horizontalAlignment : TextHorizontalAlignment, verticalAlignment : TextVerticalAlignment): ...
    def Draw3dText (self, text : str, color : Color, textPlane : Plane, height : float, fontface : str, bold : bool, italic : bool): ...
    def Draw3dText (self, text : str, color : Color, textPlane : Plane, height : float, fontface : str): ...
    def Draw3dText (self, text : Text3d, color : Color): ...
    def Draw3dText (self, text : Text3d, color : Color, textPlane : Plane): ...
    def Draw3dText (self, text : Text3d, color : Color, textPlaneOrigin : Point3d): ...
    def DrawText (self, text : TextEntity, color : Color): ...
    def DrawText (self, text : TextEntity, color : Color, scale : float): ...
    def DrawText (self, text : TextEntity, color : Color, xform : Transform): ...
    def Measure2dText (self, text : str, definitionPoint : Point2d, middleJustified : bool, rotationRadians : float, height : int, fontFace : str) -> Rectangle: ...
    def DrawObject (self, rhinoObject : RhinoObject): ...
    def DrawObject (self, rhinoObject : RhinoObject, xform : Transform): ...
    def DrawAnnotation (self, annotation : AnnotationBase, color : Color): ...
    def DrawAnnotationArrowhead (self, arrowhead : Arrowhead, xform : Transform, color : Color): ...
    def DrawArc (self, arc : Arc, color : Color): ...
    def DrawArc (self, arc : Arc, color : Color, thickness : int): ...
    def DrawCircle (self, circle : Circle, color : Color): ...
    def DrawCircle (self, circle : Circle, color : Color, thickness : int): ...
    def DrawSphere (self, sphere : Sphere, color : Color): ...
    def DrawSphere (self, sphere : Sphere, color : Color, thickness : int): ...
    def DrawTorus (self, torus : Torus, color : Color): ...
    def DrawTorus (self, torus : Torus, color : Color, thickness : int): ...
    def DrawCylinder (self, cylinder : Cylinder, color : Color): ...
    def DrawCylinder (self, cylinder : Cylinder, color : Color, thickness : int): ...
    def DrawCone (self, cone : Cone, color : Color): ...
    def DrawCone (self, cone : Cone, color : Color, thickness : int): ...
    def DrawCurve (self, curve : Curve, color : Color): ...
    def DrawCurve (self, curve : Curve, color : Color, thickness : int): ...
    def DrawCurvatureGraph (self, curve : Curve, color : Color): ...
    def DrawCurvatureGraph (self, curve : Curve, color : Color, hairScale : int): ...
    def DrawCurvatureGraph (self, curve : Curve, color : Color, hairScale : int, hairDensity : int, sampleDensity : int): ...
    def DrawSurface (self, surface : Surface, wireColor : Color, wireDensity : int): ...
    def DrawSprite (self, bitmap : DisplayBitmap, worldLocation : Point3d, size : Single, sizeInWorldSpace : bool): ...
    def DrawSprite (self, bitmap : DisplayBitmap, worldLocation : Point3d, size : Single, blendColor : Color, sizeInWorldSpace : bool): ...
    def DrawSprite (self, bitmap : DisplayBitmap, screenLocation : Point2d, size : Single): ...
    def DrawSprite (self, bitmap : DisplayBitmap, screenLocation : Point2d, size : Single, blendColor : Color): ...
    def DrawSprites (self, bitmap : DisplayBitmap, items : DisplayBitmapDrawList, size : Single, sizeInWorldSpace : bool): ...
    def DrawSprites (self, bitmap : DisplayBitmap, items : DisplayBitmapDrawList, size : Single, translation : Vector3d, sizeInWorldSpace : bool): ...
    def DrawParticles (self, particles : ParticleSystem): ...
    def DrawParticles (self, particles : ParticleSystem, bitmap : DisplayBitmap): ...
    def DrawParticles (self, particles : ParticleSystem, bitmaps : Set(DisplayBitmap)): ...
    def Draw2dRectangle (self, rectangle : Rectangle, strokeColor : Color, thickness : int, fillColor : Color): ...
    def DrawRoundedRectangle (self, center : PointF, pixelWidth : Single, pixelHeight : Single, cornerRadius : Single, strokeColor : Color, strokeWidth : Single, fillColor : Color): ...
    def Draw2dLine (self, from_ : Point, to : Point, color : Color, thickness : Single): ...
    def Draw2dLine (self, from_ : PointF, to : PointF, color : Color, thickness : Single): ...
    def IsVisible (self, worldCoordinate : Point3d) -> bool: ...
    def IsVisible (self, rhinoObject : RhinoObject) -> bool: ...
    def IsVisible (self, bbox : BoundingBox) -> bool: ...
    def IsActive (self, rhinoObject : RhinoObject) -> bool: ...
    def InterruptDrawing (self) -> bool: ...
    def GetOpenGLWorldToCamera (self, includeModelTransform : bool) -> Set(Single): ...
    def GetOpenGLWorldToClip (self, includeModelTransform : bool) -> Set(Single): ...
    def GetOpenGLCameraToClip (self) -> Set(Single): ...
    def DrawToBitmap (viewport : RhinoViewport, width : int, height : int) -> Bitmap: ...
    def DrawStereoFrameBuffer (self, viewportLeft : ViewportInfo, viewportRight : ViewportInfo) -> Tuple(bool, UInt32, UInt32): ...
    def DrawMeshWires (self, mesh : Mesh, color : Color): ...
    def DrawMeshWires (self, mesh : Mesh, color : Color, thickness : int): ...
    def DrawMeshVertices (self, mesh : Mesh, color : Color): ...
    def DrawMeshShaded (self, mesh : Mesh, material : DisplayMaterial): ...
    def DrawMeshShaded (self, mesh : Mesh, material : DisplayMaterial, faceIndices : Set(int)): ...
    def DrawMeshFalseColors (self, mesh : Mesh): ...
    def DrawBrepShaded (self, brep : Brep, material : DisplayMaterial): ...
    def DrawBrepWires (self, brep : Brep, color : Color): ...
    def DrawBrepWires (self, brep : Brep, color : Color, wireDensity : int): ...
    def DrawExtrusionWires (self, extrusion : Extrusion, color : Color): ...
    def DrawExtrusionWires (self, extrusion : Extrusion, color : Color, wireDensity : int): ...
    def DrawActivePoint (self, point : Point3d): ...
    def DrawPoint (self, point : Point3d): ...
    def DrawPoint (self, point : Point3d, color : Color): ...
    def DrawPoint (self, point : Point3d, style : PointStyle, radius : Single, color : Color): ...
    def DrawPoints (self, points : Iterable(Point3d), style : PointStyle, radius : Single, color : Color): ...
    def DrawPoint (self, point : Point3d, style : PointStyle, strokeColor : Color, fillColor : Color, radius : Single, strokeWidth : Single, secondarySize : Single, rotationRadians : Single, diameterIsInPixels : bool, autoScaleForDpi : bool): ...
    def DrawPoints (self, points : Iterable(Point3d), style : PointStyle, strokeColor : Color, fillColor : Color, radius : Single, strokeWidth : Single, secondarySize : Single, rotationRadians : Single, diameterIsInPixels : bool, autoScaleForDpi : bool): ...
    def DrawPoint (self, point : Point3d, style : PointStyle, radius : int, color : Color): ...
    def DrawPoints (self, points : Iterable(Point3d), style : PointStyle, radius : int, color : Color): ...
    def DrawPointCloud (self, cloud : PointCloud, size : Single): ...
    def DrawPointCloud (self, cloud : PointCloud, size : Single, color : Color): ...
    def DrawPointCloud (self, cloud : PointCloud, size : int): ...
    def DrawPointCloud (self, cloud : PointCloud, size : int, color : Color): ...
    def DrawDirectionArrow (self, location : Point3d, direction : Vector3d, color : Color): ...
    def DrawArrow (self, line : Line, color : Color): ...
    def DrawArrow (self, line : Line, color : Color, screenSize : float, relativeSize : float): ...
    def DrawArrows (self, lines : Set(Line), color : Color): ...
    def DrawArrows (self, lines : Iterable(Line), color : Color): ...
    def DrawArrowHead (self, tip : Point3d, direction : Vector3d, color : Color, screenSize : float, worldSize : float): ...
    def DrawLineArrow (self, line : Line, color : Color, thickness : int, size : float): ...
    def DrawLine (self, line : Line, color : Color): ...
    def DrawLine (self, line : Line, color : Color, thickness : int): ...
    def DrawLine (self, from_ : Point3d, to : Point3d, color : Color): ...
    def DrawLine (self, from_ : Point3d, to : Point3d, color : Color, thickness : int): ...
    def DrawDottedLine (self, line : Line, color : Color): ...
    def DrawPatternedLine (self, line : Line, color : Color, pattern : int, thickness : int): ...
    def DrawDottedLine (self, from_ : Point3d, to : Point3d, color : Color): ...
    def DrawPatternedLine (self, from_ : Point3d, to : Point3d, color : Color, pattern : int, thickness : int): ...
    def DrawDottedPolyline (self, points : Iterable(Point3d), color : Color, close : bool): ...
    def DrawPatternedPolyline (self, points : Iterable(Point3d), color : Color, pattern : int, thickness : int, close : bool): ...
    def DrawLines (self, lines : Iterable(Line), color : Color): ...
    def DrawLines (self, lines : Iterable(Line), color : Color, thickness : int): ...
    def DrawPolyline (self, polyline : Iterable(Point3d), color : Color): ...
    def DrawPolyline (self, polyline : Iterable(Point3d), color : Color, thickness : int): ...
    def DrawPolygon (self, points : Iterable(Point3d), color : Color, filled : bool): ...
    def DrawBitmap (self, bitmap : DisplayBitmap, left : int, top : int): ...
    def DrawDot (self, screenX : Single, screenY : Single, text : str, dotColor : Color, textColor : Color): ...
    def DrawDot (self, screenX : Single, screenY : Single, text : str): ...
    def DrawDot (self, worldPosition : Point3d, text : str, dotColor : Color, textColor : Color): ...
    def DrawDot (self, worldPosition : Point3d, text : str): ...
    def DrawDot (self, dot : TextDot, fillColor : Color, textColor : Color, borderColor : Color): ...
    def DrawHatch (self, hatch : Hatch, hatchColor : Color, boundaryColor : Color): ...
    def DrawGradientHatch (self, hatch : Hatch, color1 : Color, color2 : Color, point1 : Point3d, point2 : Point3d, linearGradient : bool, boundaryThickness : Single, boundaryColor : Color): ...
    def DrawGradientHatch (self, hatch : Hatch, colors : Set(Color), stops : Set(Single), point1 : Point3d, point2 : Point3d, linearGradient : bool, repeat : Single, boundaryThickness : Single, boundaryColor : Color): ...
    def DrawGradientMesh (self, mesh : Mesh, colors : Set(Color), stops : Set(Single), point1 : Point3d, point2 : Point3d, linearGradient : bool, repeat : Single): ...
    def DrawBox (self, box : BoundingBox, color : Color): ...
    def DrawBox (self, box : BoundingBox, color : Color, thickness : int): ...
    def DrawBox (self, box : Box, color : Color): ...
    def DrawBox (self, box : Box, color : Color, thickness : int): ...
    def DrawBoxCorners (self, box : BoundingBox, color : Color): ...
    def DrawBoxCorners (self, box : BoundingBox, color : Color, size : float): ...
    def DrawBoxCorners (self, box : BoundingBox, color : Color, size : float, thickness : int): ...
    def DrawMarker (self, tip : Point3d, direction : Vector3d, color : Color): ...
    def DrawMarker (self, tip : Point3d, direction : Vector3d, color : Color, thickness : int): ...
    def DrawMarker (self, tip : Point3d, direction : Vector3d, color : Color, thickness : int, size : float): ...
    def DrawMarker (self, tip : Point3d, direction : Vector3d, color : Color, thickness : int, size : float, rotation : float): ...
    def DrawConstructionPlane (self, constructionPlane : ConstructionPlane): ...
    def Draw2dText (self, text : str, color : Color, screenCoordinate : Point2d, middleJustified : bool): ...
    def Draw2dText (self, text : str, color : Color, screenCoordinate : Point2d, middleJustified : bool, height : int): ...
    def Draw2dText (self, text : str, color : Color, screenCoordinate : Point2d, middleJustified : bool, height : int, fontface : str): ...
    def Draw2dText (self, text : str, color : Color, worldCoordinate : Point3d, middleJustified : bool): ...
    def Draw2dText (self, text : str, color : Color, worldCoordinate : Point3d, middleJustified : bool, height : int): ...
    def add_ObjectCulling (value : EventHandler): ...
    def remove_ObjectCulling (value : EventHandler): ...
    def add_CalculateBoundingBox (value : EventHandler): ...
    def remove_CalculateBoundingBox (value : EventHandler): ...
    def add_CalculateBoundingBoxZoomExtents (value : EventHandler): ...
    def remove_CalculateBoundingBoxZoomExtents (value : EventHandler): ...
    def add_PreDrawObjects (value : EventHandler): ...
    def remove_PreDrawObjects (value : EventHandler): ...
    def add_PreDrawTransparentObjects (value : EventHandler): ...
    def remove_PreDrawTransparentObjects (value : EventHandler): ...
    def add_PreDrawObject (value : EventHandler): ...
    def remove_PreDrawObject (value : EventHandler): ...
    def add_PostDrawObjects (value : EventHandler): ...
    def remove_PostDrawObjects (value : EventHandler): ...
    def add_DrawForeground (value : EventHandler): ...
    def remove_DrawForeground (value : EventHandler): ...
    def add_DrawOverlay (value : EventHandler): ...
    def remove_DrawOverlay (value : EventHandler): ...
    @property
    def FrameSize (self) -> Size: ...
    @property
    def FrameBuffer (self) -> Bitmap: ...
    @property
    def DefaultCurveThickness (self) -> int: ...
    @property
    def IsOpenGL (self) -> bool: ...
    @property
    def IsStereoMode (self) -> bool: ...
    @property
    def IsPrinting (self) -> bool: ...
    @property
    def IsInViewCapture (self) -> bool: ...
    @property
    def IsDynamicDisplay (self) -> bool: ...
    @property
    def SupportsShading (self) -> bool: ...
    @property
    def StereoProjection (self) -> int: ...
    @property
    def ModelTransform (self) -> Transform: ...
    @ModelTransform.setter
    def ModelTransform (self, value : Transform): ...
    @property
    def ModelTransformIsIdentity (self) -> bool: ...
    @property
    def RenderPass (self) -> int: ...
    @property
    def NestLevel (self) -> int: ...
    @property
    def ActiveTopLevelObject (self) -> RhinoObject: ...
    @property
    def ActiveObject (self) -> RhinoObject: ...
    @property
    def ActiveObjectNestingLevel (self) -> int: ...
    @property
    def ActiveObjectNestingStack (self) -> Set(RhinoObject): ...
    def Open (self) -> bool: ...
    @property
    def IsOpen (self) -> bool: ...
    def Close (self) -> bool: ...
    def Clone (self, viewport : RhinoViewport) -> DisplayPipeline: ...
    @property
    def DrawingWires (self) -> bool: ...
    @property
    def DrawingGrips (self) -> bool: ...
    @property
    def DrawingSurfaces (self) -> bool: ...
    @property
    def ShadingRequired (self) -> bool: ...
    @ShadingRequired.setter
    def ShadingRequired (self, value : bool): ...
    @property
    def Viewport (self) -> RhinoViewport: ...
    @property
    def DisplayPipelineAttributes (self) -> DisplayPipelineAttributes: ...
    @property
    def DpiScale (self) -> Single: ...
    def PushModelTransform (self, xform : Transform): ...
    def PopModelTransform (self): ...
    def EnableDepthTesting (self, enable : bool): ...
    def EnableDepthWriting (self, enable : bool): ...
    def EnableColorWriting (self, enable : bool): ...
    def EnableLighting (self, enable : bool): ...
    def GetLights (self) -> Set(Light): ...
    def EnableClippingPlanes (self, enable : bool): ...
    def AddClippingPlane (self, point : Point3d, normal : Vector3d) -> int: ...
    def RemoveClippingPlane (self, index : int): ...
    def PushDepthTesting (self, enable : bool): ...
    def PopDepthTesting (self): ...
    def PushDepthWriting (self, enable : bool): ...
    def PopDepthWriting (self): ...
    def PushClipTesting (self, enable : bool): ...
    def PopClipTesting (self): ...
    def PushCullFaceMode (self, mode : CullFaceMode): ...
    def PopCullFaceMode (self): ...
    @property
    def DepthMode (self) -> DepthMode: ...
    @DepthMode.setter
    def DepthMode (self, value : DepthMode): ...
    @property
    def ZBiasMode (self) -> ZBiasMode: ...
    @ZBiasMode.setter
    def ZBiasMode (self, value : ZBiasMode): ...
    def CullControlPolygon () -> bool: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class DrawEventArgs:
    @property
    def Viewport (self) -> RhinoViewport: ...
    @property
    def Display (self) -> DisplayPipeline: ...
    @property
    def RhinoDoc (self) -> RhinoDoc: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class DrawForegroundEventArgs:
    @property
    def WorldAxesDrawn (self) -> bool: ...
    @WorldAxesDrawn.setter
    def WorldAxesDrawn (self, value : bool): ...
    @property
    def DrawWorldAxes (self) -> bool: ...
    @DrawWorldAxes.setter
    def DrawWorldAxes (self, value : bool): ...
    @property
    def Viewport (self) -> RhinoViewport: ...
    @property
    def Display (self) -> DisplayPipeline: ...
    @property
    def RhinoDoc (self) -> RhinoDoc: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class CullObjectEventArgs:
    @property
    def RhinoObject (self) -> RhinoObject: ...
    @property
    def RhinoObjectSerialNumber (self) -> UInt32: ...
    @property
    def CullObject (self) -> bool: ...
    @CullObject.setter
    def CullObject (self, value : bool): ...
    @property
    def Viewport (self) -> RhinoViewport: ...
    @property
    def Display (self) -> DisplayPipeline: ...
    @property
    def RhinoDoc (self) -> RhinoDoc: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class DrawObjectEventArgs:
    @property
    def RhinoObject (self) -> RhinoObject: ...
    @property
    def DrawObject (self) -> bool: ...
    @DrawObject.setter
    def DrawObject (self, value : bool): ...
    @property
    def Viewport (self) -> RhinoViewport: ...
    @property
    def Display (self) -> DisplayPipeline: ...
    @property
    def RhinoDoc (self) -> RhinoDoc: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class CalculateBoundingBoxEventArgs:
    @property
    def BoundingBox (self) -> BoundingBox: ...
    def IncludeBoundingBox (self, box : BoundingBox): ...
    @property
    def Viewport (self) -> RhinoViewport: ...
    @property
    def Display (self) -> DisplayPipeline: ...
    @property
    def RhinoDoc (self) -> RhinoDoc: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class ZBufferCapture:
    def __init__(self, viewport : RhinoViewport): ...
    def Dispose (self): ...
    def SetDisplayMode (self, modeId : Guid): ...
    def ShowIsocurves (self, on : bool): ...
    def ShowMeshWires (self, on : bool): ...
    def ShowCurves (self, on : bool): ...
    def ShowPoints (self, on : bool): ...
    def ShowText (self, on : bool): ...
    def ShowAnnotations (self, on : bool): ...
    def ShowLights (self, on : bool): ...
    def HitCount (self) -> int: ...
    def MaxZ (self) -> Single: ...
    def MinZ (self) -> Single: ...
    def ZValueAt (self, x : int, y : int) -> Single: ...
    def WorldPointAt (self, x : int, y : int) -> Point3d: ...
    def GrayscaleDib (self) -> Bitmap: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class DisplayPipelineAttributes:
    @PointRadius.setter
    def PointRadius (self, value : Single): ...
    @property
    def ShowPointClouds (self) -> bool: ...
    @ShowPointClouds.setter
    def ShowPointClouds (self, value : bool): ...
    @property
    def Id (self) -> Guid: ...
    @property
    def EnglishName (self) -> str: ...
    @EnglishName.setter
    def EnglishName (self, value : str): ...
    @property
    def LocalName (self) -> str: ...
    @property
    def ContextForDraw (self) -> ContextsForDraw: ...
    @property
    def RealtimeDisplayId (self) -> Guid: ...
    @property
    def RealtimeRenderPasses (self) -> int: ...
    @RealtimeRenderPasses.setter
    def RealtimeRenderPasses (self, value : int): ...
    @property
    def ShowRealtimeRenderProgressBar (self) -> bool: ...
    @ShowRealtimeRenderProgressBar.setter
    def ShowRealtimeRenderProgressBar (self, value : bool): ...
    @property
    def StereoRenderContext (self) -> StereoRenderContextEnum: ...
    @StereoRenderContext.setter
    def StereoRenderContext (self, value : StereoRenderContextEnum): ...
    def GetObjectData (self, info : SerializationInfo, context : StreamingContext): ...
    def Dispose (self): ...
    @property
    def XrayAllObjects (self) -> bool: ...
    @XrayAllObjects.setter
    def XrayAllObjects (self, value : bool): ...
    @property
    def IgnoreHighlights (self) -> bool: ...
    @IgnoreHighlights.setter
    def IgnoreHighlights (self, value : bool): ...
    @property
    def DisableConduits (self) -> bool: ...
    @DisableConduits.setter
    def DisableConduits (self, value : bool): ...
    @property
    def DisableTransparency (self) -> bool: ...
    @DisableTransparency.setter
    def DisableTransparency (self, value : bool): ...
    @property
    def ObjectColor (self) -> Color: ...
    @ObjectColor.setter
    def ObjectColor (self, value : Color): ...
    @property
    def ShowGrips (self) -> bool: ...
    @ShowGrips.setter
    def ShowGrips (self, value : bool): ...
    @property
    def FillMode (self) -> FrameBufferFillMode: ...
    @FillMode.setter
    def FillMode (self, value : FrameBufferFillMode): ...
    @property
    def BoundingBoxMode (self) -> BoundingBoxDisplayMode: ...
    @BoundingBoxMode.setter
    def BoundingBoxMode (self, value : BoundingBoxDisplayMode): ...
    @property
    def ShowClippingPlanes (self) -> bool: ...
    @ShowClippingPlanes.setter
    def ShowClippingPlanes (self, value : bool): ...
    @property
    def ViewSpecificAttributes (self) -> ViewDisplayAttributes: ...
    @property
    def UseAssignedObjectMaterial (self) -> bool: ...
    @property
    def UseCustomObjectColor (self) -> bool: ...
    @property
    def UseCustomObjectMaterial (self) -> bool: ...
    @property
    def ShowCurves (self) -> bool: ...
    @ShowCurves.setter
    def ShowCurves (self, value : bool): ...
    @property
    def UseSingleCurveColor (self) -> bool: ...
    @UseSingleCurveColor.setter
    def UseSingleCurveColor (self, value : bool): ...
    @property
    def CurveThickness (self) -> int: ...
    @CurveThickness.setter
    def CurveThickness (self, value : int): ...
    @property
    def CurveColor (self) -> Color: ...
    @CurveColor.setter
    def CurveColor (self, value : Color): ...
    @property
    def ShadingEnabled (self) -> bool: ...
    @ShadingEnabled.setter
    def ShadingEnabled (self, value : bool): ...
    @property
    def ShadeVertexColors (self) -> bool: ...
    @ShadeVertexColors.setter
    def ShadeVertexColors (self, value : bool): ...
    @property
    def ShowIsoCurves (self) -> bool: ...
    @ShowIsoCurves.setter
    def ShowIsoCurves (self, value : bool): ...
    @property
    def ShowSurfaceEdges (self) -> bool: ...
    @ShowSurfaceEdges.setter
    def ShowSurfaceEdges (self, value : bool): ...
    @property
    def ShowTangentEdges (self) -> bool: ...
    @ShowTangentEdges.setter
    def ShowTangentEdges (self, value : bool): ...
    @property
    def ShowTangentSeams (self) -> bool: ...
    @ShowTangentSeams.setter
    def ShowTangentSeams (self, value : bool): ...
    @property
    def SurfaceEdgeThickness (self) -> int: ...
    @SurfaceEdgeThickness.setter
    def SurfaceEdgeThickness (self, value : int): ...
    @property
    def LockedObjectsDrawBehindOthers (self) -> bool: ...
    @LockedObjectsDrawBehindOthers.setter
    def LockedObjectsDrawBehindOthers (self, value : bool): ...
    @property
    def MeshSpecificAttributes (self) -> MeshDisplayAttributes: ...
    @property
    def ShowText (self) -> bool: ...
    @ShowText.setter
    def ShowText (self, value : bool): ...
    @property
    def ShowAnnotations (self) -> bool: ...
    @ShowAnnotations.setter
    def ShowAnnotations (self, value : bool): ...
    @property
    def ShowLights (self) -> bool: ...
    @ShowLights.setter
    def ShowLights (self, value : bool): ...
    @property
    def AmbientLightingColor (self) -> Color: ...
    @AmbientLightingColor.setter
    def AmbientLightingColor (self, value : Color): ...
    @property
    def ShadowColor (self) -> Color: ...
    @ShadowColor.setter
    def ShadowColor (self, value : Color): ...
    @property
    def CastShadows (self) -> bool: ...
    @CastShadows.setter
    def CastShadows (self, value : bool): ...
    @property
    def ShowPoints (self) -> bool: ...
    @ShowPoints.setter
    def ShowPoints (self, value : bool): ...
    @property
    def PointStyle (self) -> PointStyle: ...
    @PointStyle.setter
    def PointStyle (self, value : PointStyle): ...
    @property
    def PointRadius (self) -> Single: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class DisplayMaterial:
    def __init__(self): ...
    def __init__(self, other : DisplayMaterial): ...
    def __init__(self, material : Material): ...
    def __init__(self, diffuse : Color): ...
    def __init__(self, diffuse : Color, transparency : float): ...
    def __init__(self, diffuse : Color, specular : Color, ambient : Color, emission : Color, shine : float, transparency : float): ...
    def Dispose (self): ...
    @property
    def Diffuse (self) -> Color: ...
    @Diffuse.setter
    def Diffuse (self, value : Color): ...
    @property
    def BackDiffuse (self) -> Color: ...
    @BackDiffuse.setter
    def BackDiffuse (self, value : Color): ...
    @property
    def Specular (self) -> Color: ...
    @Specular.setter
    def Specular (self, value : Color): ...
    @property
    def BackSpecular (self) -> Color: ...
    @BackSpecular.setter
    def BackSpecular (self, value : Color): ...
    @property
    def Ambient (self) -> Color: ...
    @Ambient.setter
    def Ambient (self, value : Color): ...
    @property
    def BackAmbient (self) -> Color: ...
    @BackAmbient.setter
    def BackAmbient (self, value : Color): ...
    @property
    def Emission (self) -> Color: ...
    @Emission.setter
    def Emission (self, value : Color): ...
    @property
    def BackEmission (self) -> Color: ...
    @BackEmission.setter
    def BackEmission (self, value : Color): ...
    @property
    def Shine (self) -> float: ...
    @Shine.setter
    def Shine (self, value : float): ...
    @property
    def BackShine (self) -> float: ...
    @BackShine.setter
    def BackShine (self, value : float): ...
    @property
    def Transparency (self) -> float: ...
    @Transparency.setter
    def Transparency (self, value : float): ...
    @property
    def BackTransparency (self) -> float: ...
    @BackTransparency.setter
    def BackTransparency (self, value : float): ...
    @property
    def IsTwoSided (self) -> bool: ...
    @IsTwoSided.setter
    def IsTwoSided (self, value : bool): ...
    def GetBitmapTexture (self, front : bool) -> Texture: ...
    def SetBitmapTexture (self, filename : str, front : bool) -> bool: ...
    def SetBitmapTexture (self, texture : Texture, front : bool) -> bool: ...
    def GetBumpTexture (self, front : bool) -> Texture: ...
    def SetBumpTexture (self, filename : str, front : bool) -> bool: ...
    def SetBumpTexture (self, texture : Texture, front : bool) -> bool: ...
    def GetEnvironmentTexture (self, front : bool) -> Texture: ...
    def SetEnvironmentTexture (self, filename : str, front : bool) -> bool: ...
    def SetEnvironmentTexture (self, texture : Texture, front : bool) -> bool: ...
    def GetTransparencyTexture (self, front : bool) -> Texture: ...
    def SetTransparencyTexture (self, filename : str, front : bool) -> bool: ...
    def SetTransparencyTexture (self, texture : Texture, front : bool) -> bool: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class RhinoPageView:
    def Duplicate (self, duplicatePageGeometry : bool) -> RhinoPageView: ...
    @property
    def ActiveViewport (self) -> RhinoViewport: ...
    def SetPageAsActive (self): ...
    def SetActiveDetail (self, detailId : Guid) -> bool: ...
    def SetActiveDetail (self, detailName : str, compareCase : bool) -> bool: ...
    @property
    def PageIsActive (self) -> bool: ...
    def AddDetailView (self, title : str, corner0 : Point2d, corner1 : Point2d, initialProjection : DefinedViewportProjection) -> DetailViewObject: ...
    def GetDetailViews (self) -> Set(DetailViewObject): ...
    @property
    def PageNumber (self) -> int: ...
    @PageNumber.setter
    def PageNumber (self, value : int): ...
    @property
    def PageWidth (self) -> float: ...
    @PageWidth.setter
    def PageWidth (self, value : float): ...
    @property
    def PageHeight (self) -> float: ...
    @PageHeight.setter
    def PageHeight (self, value : float): ...
    @property
    def PageName (self) -> str: ...
    @PageName.setter
    def PageName (self, value : str): ...
    def add_PageViewSpaceChange (value : EventHandler): ...
    def remove_PageViewSpaceChange (value : EventHandler): ...
    def GetHashCode (self) -> int: ...
    def Equals (self, obj : Object) -> bool: ...
    @property
    def RuntimeSerialNumber (self) -> UInt32: ...
    @property
    def Handle (self) -> IntPtr: ...
    @property
    def DisplayPipeline (self) -> DisplayPipeline: ...
    @property
    def RealtimeDisplayMode (self) -> RealtimeDisplayMode: ...
    @property
    def Bounds (self) -> Rectangle: ...
    @property
    def ClientRectangle (self) -> Rectangle: ...
    @property
    def ScreenRectangle (self) -> Rectangle: ...
    @property
    def Size (self) -> Size: ...
    @Size.setter
    def Size (self, value : Size): ...
    def ScreenToClient (self, screenPoint : Point) -> Point: ...
    def ScreenToClient (self, screenPoint : Point2d) -> Point2d: ...
    def ClientToScreen (self, clientPoint : Point) -> Point: ...
    def ClientToScreen (self, clientPoint : Point2d) -> Point2d: ...
    def Redraw (self): ...
    def SpeedTest (self, frameCount : int, freezeDrawList : bool, direction : int, angleDeltaRadians : float) -> float: ...
    def CreateWireframePreviewImage (self, imagePath : str, size : Size, ignoreHighlights : bool, drawConstructionPlane : bool) -> bool: ...
    def CreateShadedPreviewImage (self, imagePath : str, size : Size, ignoreHighlights : bool, drawConstructionPlane : bool, useGhostedShading : bool) -> bool: ...
    def CaptureToBitmap (self) -> Bitmap: ...
    def CaptureToBitmap (self, size : Size) -> Bitmap: ...
    def CaptureToBitmap (self, size : Size, grid : bool, worldAxes : bool, cplaneAxes : bool) -> Bitmap: ...
    def CaptureToBitmap (self, grid : bool, worldAxes : bool, cplaneAxes : bool) -> Bitmap: ...
    def CaptureToBitmap (self, size : Size, mode : DisplayModeDescription) -> Bitmap: ...
    def CaptureToBitmap (self, mode : DisplayModeDescription) -> Bitmap: ...
    def CaptureToBitmap (self, size : Size, attributes : DisplayPipelineAttributes) -> Bitmap: ...
    def CaptureToBitmap (self, attributes : DisplayPipelineAttributes) -> Bitmap: ...
    @property
    def Document (self) -> RhinoDoc: ...
    @property
    def MainViewport (self) -> RhinoViewport: ...
    @property
    def ActiveViewportID (self) -> Guid: ...
    @property
    def TitleVisible (self) -> bool: ...
    @TitleVisible.setter
    def TitleVisible (self, value : bool): ...
    @property
    def Maximized (self) -> bool: ...
    @Maximized.setter
    def Maximized (self, value : bool): ...
    @property
    def Floating (self) -> bool: ...
    @Floating.setter
    def Floating (self, value : bool): ...
    def Close (self) -> bool: ...
    def MouseCaptured (self, bIncludeMovement : bool) -> bool: ...
    def ToString (self) -> str: ...
    def GetType (self) -> Type: ...
class ViewCapture:
    def __init__(self): ...
    def CaptureToBitmap (self, sourceView : RhinoView) -> Bitmap: ...
    @property
    def Width (self) -> int: ...
    @Width.setter
    def Width (self, value : int): ...
    @property
    def Height (self) -> int: ...
    @Height.setter
    def Height (self, value : int): ...
    @property
    def ScaleScreenItems (self) -> bool: ...
    @ScaleScreenItems.setter
    def ScaleScreenItems (self, value : bool): ...
    @property
    def DrawGrid (self) -> bool: ...
    @DrawGrid.setter
    def DrawGrid (self, value : bool): ...
    @property
    def DrawAxes (self) -> bool: ...
    @DrawAxes.setter
    def DrawAxes (self, value : bool): ...
    @property
    def DrawGridAxes (self) -> bool: ...
    @DrawGridAxes.setter
    def DrawGridAxes (self, value : bool): ...
    @property
    def TransparentBackground (self) -> bool: ...
    @TransparentBackground.setter
    def TransparentBackground (self, value : bool): ...
    @property
    def Preview (self) -> bool: ...
    @Preview.setter
    def Preview (self, value : bool): ...
    @property
    def RealtimeRenderPasses (self) -> int: ...
    @RealtimeRenderPasses.setter
    def RealtimeRenderPasses (self, value : int): ...
    def CaptureToBitmap (settings : ViewCaptureSettings) -> Bitmap: ...
    def CaptureToSvg (settings : ViewCaptureSettings) -> XmlDocument: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class ViewCaptureSettings:
    def __init__(self): ...
    def __init__(self, sourceView : RhinoView, mediaSize : Size, dpi : float): ...
    def __init__(self, sourcePageView : RhinoPageView, dpi : float): ...
    def SetLayout (self, mediaSize : Size, cropRectangle : Rectangle): ...
    @property
    def MediaSize (self) -> Size: ...
    @property
    def Resolution (self) -> float: ...
    @Resolution.setter
    def Resolution (self, value : float): ...
    @property
    def CropRectangle (self) -> Rectangle: ...
    def GetMargins (self, lengthUnits : UnitSystem) -> Tuple(bool, float, float, float, float): ...
    def SetMargins (self, lengthUnits : UnitSystem, left : float, top : float, right : float, bottom : float) -> bool: ...
    def SetOffset (self, lengthUnits : UnitSystem, fromMargin : bool, x : float, y : float): ...
    def GetOffset (self, lengthUnits : UnitSystem) -> Tuple(bool, float, float): ...
    @property
    def OffsetAnchor (self) -> AnchorLocation: ...
    @OffsetAnchor.setter
    def OffsetAnchor (self, value : AnchorLocation): ...
    @property
    def IsValid (self) -> bool: ...
    @property
    def DrawBackground (self) -> bool: ...
    @DrawBackground.setter
    def DrawBackground (self, value : bool): ...
    @property
    def DrawGrid (self) -> bool: ...
    @DrawGrid.setter
    def DrawGrid (self, value : bool): ...
    @property
    def DrawAxis (self) -> bool: ...
    @DrawAxis.setter
    def DrawAxis (self, value : bool): ...
    @property
    def DrawLockedObjects (self) -> bool: ...
    @DrawLockedObjects.setter
    def DrawLockedObjects (self, value : bool): ...
    @property
    def DrawMargins (self) -> bool: ...
    @DrawMargins.setter
    def DrawMargins (self, value : bool): ...
    @property
    def DrawSelectedObjectsOnly (self) -> bool: ...
    @DrawSelectedObjectsOnly.setter
    def DrawSelectedObjectsOnly (self, value : bool): ...
    @property
    def DrawClippingPlanes (self) -> bool: ...
    @DrawClippingPlanes.setter
    def DrawClippingPlanes (self, value : bool): ...
    @property
    def DrawLights (self) -> bool: ...
    @DrawLights.setter
    def DrawLights (self, value : bool): ...
    @property
    def DrawBackgroundBitmap (self) -> bool: ...
    @DrawBackgroundBitmap.setter
    def DrawBackgroundBitmap (self, value : bool): ...
    @property
    def DrawWallpaper (self) -> bool: ...
    @DrawWallpaper.setter
    def DrawWallpaper (self, value : bool): ...
    @property
    def OutputColor (self) -> ColorMode: ...
    @OutputColor.setter
    def OutputColor (self, value : ColorMode): ...
    def Dispose (self): ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class RhinoView:
    def FromRuntimeSerialNumber (serialNumber : UInt32) -> RhinoView: ...
    def GetHashCode (self) -> int: ...
    def Equals (self, obj : Object) -> bool: ...
    @property
    def RuntimeSerialNumber (self) -> UInt32: ...
    @property
    def Handle (self) -> IntPtr: ...
    @property
    def DisplayPipeline (self) -> DisplayPipeline: ...
    @property
    def RealtimeDisplayMode (self) -> RealtimeDisplayMode: ...
    @property
    def Bounds (self) -> Rectangle: ...
    @property
    def ClientRectangle (self) -> Rectangle: ...
    @property
    def ScreenRectangle (self) -> Rectangle: ...
    @property
    def Size (self) -> Size: ...
    @Size.setter
    def Size (self, value : Size): ...
    def ScreenToClient (self, screenPoint : Point) -> Point: ...
    def ScreenToClient (self, screenPoint : Point2d) -> Point2d: ...
    def ClientToScreen (self, clientPoint : Point) -> Point: ...
    def ClientToScreen (self, clientPoint : Point2d) -> Point2d: ...
    def Redraw (self): ...
    @property
    def EnableDrawing () -> bool: ...
    @EnableDrawing.setter
    def EnableDrawing (value : bool): ...
    def SpeedTest (self, frameCount : int, freezeDrawList : bool, direction : int, angleDeltaRadians : float) -> float: ...
    def CreateWireframePreviewImage (self, imagePath : str, size : Size, ignoreHighlights : bool, drawConstructionPlane : bool) -> bool: ...
    def CreateShadedPreviewImage (self, imagePath : str, size : Size, ignoreHighlights : bool, drawConstructionPlane : bool, useGhostedShading : bool) -> bool: ...
    def CaptureToBitmap (self) -> Bitmap: ...
    def CaptureToBitmap (self, size : Size) -> Bitmap: ...
    def CaptureToBitmap (self, size : Size, grid : bool, worldAxes : bool, cplaneAxes : bool) -> Bitmap: ...
    def CaptureToBitmap (self, grid : bool, worldAxes : bool, cplaneAxes : bool) -> Bitmap: ...
    def CaptureToBitmap (self, size : Size, mode : DisplayModeDescription) -> Bitmap: ...
    def CaptureToBitmap (self, mode : DisplayModeDescription) -> Bitmap: ...
    def CaptureToBitmap (self, size : Size, attributes : DisplayPipelineAttributes) -> Bitmap: ...
    def CaptureToBitmap (self, attributes : DisplayPipelineAttributes) -> Bitmap: ...
    @property
    def Document (self) -> RhinoDoc: ...
    @property
    def MainViewport (self) -> RhinoViewport: ...
    @property
    def ActiveViewport (self) -> RhinoViewport: ...
    @property
    def ActiveViewportID (self) -> Guid: ...
    @property
    def TitleVisible (self) -> bool: ...
    @TitleVisible.setter
    def TitleVisible (self, value : bool): ...
    @property
    def Maximized (self) -> bool: ...
    @Maximized.setter
    def Maximized (self, value : bool): ...
    @property
    def Floating (self) -> bool: ...
    @Floating.setter
    def Floating (self, value : bool): ...
    def Close (self) -> bool: ...
    def MouseCaptured (self, bIncludeMovement : bool) -> bool: ...
    def add_Create (value : EventHandler): ...
    def remove_Create (value : EventHandler): ...
    def add_Destroy (value : EventHandler): ...
    def remove_Destroy (value : EventHandler): ...
    def add_SetActive (value : EventHandler): ...
    def remove_SetActive (value : EventHandler): ...
    def add_Rename (value : EventHandler): ...
    def remove_Rename (value : EventHandler): ...
    def ToString (self) -> str: ...
    def GetType (self) -> Type: ...
class ViewEventArgs:
    @property
    def View (self) -> RhinoView: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class PageViewSpaceChangeEventArgs:
    @property
    def PageView (self) -> RhinoPageView: ...
    @property
    def NewActiveDetailId (self) -> Guid: ...
    @property
    def OldActiveDetailId (self) -> Guid: ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class DefinedViewportProjection:
    None = 0
    Top = 1
    Bottom = 2
    Left = 3
    Right = 4
    Front = 5
    Back = 6
    Perspective = 7
    TwoPointPerspective = 8
class RhinoViewport:
    def __init__(self): ...
    def __init__(self, other : RhinoViewport): ...
    def GetCameraFrame (self) -> Tuple(bool, Plane): ...
    @property
    def CameraX (self) -> Vector3d: ...
    @property
    def CameraY (self) -> Vector3d: ...
    @property
    def CameraZ (self) -> Vector3d: ...
    def GetFrustum (self) -> Tuple(bool, float, float, float, float, float, float): ...
    @property
    def FrustumAspect (self) -> float: ...
    def GetFrustumCenter (self) -> Tuple(bool, Point3d): ...
    def GetDepth (self, point : Point3d) -> Tuple(bool, float): ...
    def GetDepth (self, bbox : BoundingBox) -> Tuple(bool, float, float): ...
    def GetDepth (self, sphere : Sphere) -> Tuple(bool, float, float): ...
    def GetFrustumNearPlane (self) -> Tuple(bool, Plane): ...
    def GetFrustumFarPlane (self) -> Tuple(bool, Plane): ...
    def GetFrustumLeftPlane (self) -> Tuple(bool, Plane): ...
    def GetFrustumRightPlane (self) -> Tuple(bool, Plane): ...
    def GetFrustumBottomPlane (self) -> Tuple(bool, Plane): ...
    def GetFrustumTopPlane (self) -> Tuple(bool, Plane): ...
    def GetNearRect (self) -> Set(Point3d): ...
    def GetFarRect (self) -> Set(Point3d): ...
    def GetScreenPort (self) -> Tuple(bool, int, int, int, int, int, int): ...
    @property
    def Bounds (self) -> Rectangle: ...
    @property
    def ScreenPortAspect (self) -> float: ...
    def GetCameraAngle (self) -> Tuple(bool, float, float, float): ...
    @property
    def Camera35mmLensLength (self) -> float: ...
    @Camera35mmLensLength.setter
    def Camera35mmLensLength (self, value : float): ...
    def GetTransform (self, sourceSystem : CoordinateSystem, destinationSystem : CoordinateSystem) -> Transform: ...
    def GetFrustumLine (self, screenX : float, screenY : float) -> Tuple(bool, Line): ...
    def GetWorldToScreenScale (self, pointInFrustum : Point3d) -> Tuple(bool, float): ...
    def WorldToClient (self, worldPoint : Point3d) -> Point2d: ...
    def ClientToScreen (self, clientPoint : Point2d) -> Point: ...
    def ClientToScreen (self, clientPoint : Point) -> Point: ...
    def ScreenToClient (self, screenPoint : Point) -> Point: ...
    def ClientToWorld (self, clientPoint : Point) -> Line: ...
    def ClientToWorld (self, clientPoint : Point2d) -> Line: ...
    @property
    def WallpaperFilename (self) -> str: ...
    @property
    def WallpaperGrayscale (self) -> bool: ...
    @property
    def WallpaperVisible (self) -> bool: ...
    def SetWallpaper (self, imageFilename : str, grayscale : bool) -> bool: ...
    def SetWallpaper (self, imageFilename : str, grayscale : bool, visible : bool) -> bool: ...
    def ClearTraceImage (self): ...
    def SetTraceImage (self, bitmapFileName : str, plane : Plane, width : float, height : float, grayscale : bool, filtered : bool) -> bool: ...
    @property
    def ViewportType (self) -> ViewportType: ...
    @property
    def DisplayMode (self) -> DisplayModeDescription: ...
    @DisplayMode.setter
    def DisplayMode (self, value : DisplayModeDescription): ...
    def Dispose (self): ...
    def FromId (id : Guid) -> RhinoViewport: ...
    @property
    def ParentView (self) -> RhinoView: ...
    @property
    def Id (self) -> Guid: ...
    @property
    def ChangeCounter (self) -> UInt32: ...
    def IsVisible (self, bbox : BoundingBox) -> bool: ...
    def IsVisible (self, point : Point3d) -> bool: ...
    @property
    def Size (self) -> Size: ...
    @Size.setter
    def Size (self, value : Size): ...
    def SetClippingPlanes (self, box : BoundingBox): ...
    @property
    def Name (self) -> str: ...
    @Name.setter
    def Name (self, value : str): ...
    @property
    def CameraTarget (self) -> Point3d: ...
    def SetCameraTarget (self, targetLocation : Point3d, updateCameraLocation : bool): ...
    def SetCameraLocations (self, targetLocation : Point3d, cameraLocation : Point3d): ...
    def SetCameraLocation (self, cameraLocation : Point3d, updateTargetLocation : bool): ...
    def SetCameraDirection (self, cameraDirection : Vector3d, updateTargetLocation : bool): ...
    def GetCameraExtents (self, points : Iterable(Point3d)) -> BoundingBox: ...
    def ConstructionPlane (self) -> Plane: ...
    def GetConstructionPlane (self) -> ConstructionPlane: ...
    def SetConstructionPlane (self, plane : Plane): ...
    def SetConstructionPlane (self, cplane : ConstructionPlane): ...
    def PushConstructionPlane (self, cplane : ConstructionPlane): ...
    def PopConstructionPlane (self) -> bool: ...
    def NextConstructionPlane (self) -> bool: ...
    def PreviousConstructionPlane (self) -> bool: ...
    @property
    def ConstructionGridVisible (self) -> bool: ...
    @ConstructionGridVisible.setter
    def ConstructionGridVisible (self, value : bool): ...
    @property
    def ConstructionAxesVisible (self) -> bool: ...
    @ConstructionAxesVisible.setter
    def ConstructionAxesVisible (self, value : bool): ...
    @property
    def WorldAxesVisible (self) -> bool: ...
    @WorldAxesVisible.setter
    def WorldAxesVisible (self, value : bool): ...
    def SetToPlanView (self, planeOrigin : Point3d, planeXaxis : Vector3d, planeYaxis : Vector3d, setConstructionPlane : bool) -> bool: ...
    def SetProjection (self, projection : DefinedViewportProjection, viewName : str, updateConstructionPlane : bool) -> bool: ...
    def PushViewProjection (self): ...
    def SetViewProjection (self, projection : ViewportInfo, updateTargetLocation : bool) -> bool: ...
    def PopViewProjection (self) -> bool: ...
    def PushViewInfo (self, viewinfo : ViewInfo, includeTraceImage : bool) -> bool: ...
    def NextViewProjection (self) -> bool: ...
    def PreviousViewProjection (self) -> bool: ...
    @property
    def IsPlanView (self) -> bool: ...
    def ZoomExtents (self) -> bool: ...
    def ZoomExtentsSelected (self) -> bool: ...
    def ZoomBoundingBox (self, box : BoundingBox) -> bool: ...
    def MouseRotateAroundTarget (self, mousePreviousPoint : Point, mouseCurrentPoint : Point) -> bool: ...
    def MouseRotateCamera (self, mousePreviousPoint : Point, mouseCurrentPoint : Point) -> bool: ...
    def MouseInOutDolly (self, mousePreviousPoint : Point, mouseCurrentPoint : Point) -> bool: ...
    def MouseMagnify (self, mousePreviousPoint : Point, mouseCurrentPoint : Point) -> bool: ...
    def MouseTilt (self, mousePreviousPoint : Point, mouseCurrentPoint : Point) -> bool: ...
    def MouseAdjustLensLength (self, mousePreviousPoint : Point, mouseCurrentPoint : Point, moveTarget : bool) -> bool: ...
    def MouseDollyZoom (self, mousePreviousPoint : Point, mouseCurrentPoint : Point) -> bool: ...
    def MouseLateralDolly (self, mousePreviousPoint : Point, mouseCurrentPoint : Point) -> bool: ...
    def KeyboardRotate (self, leftRight : bool, angleRadians : float) -> bool: ...
    def KeyboardDolly (self, leftRight : bool, amount : float) -> bool: ...
    def KeyboardDollyInOut (self, amount : float) -> bool: ...
    def Magnify (self, magnificationFactor : float, mode : bool) -> bool: ...
    def Magnify (self, magnificationFactor : float, mode : bool, fixedScreenPoint : Point) -> bool: ...
    def GetPickTransform (self, clientX : int, clientY : int) -> Transform: ...
    def GetPickTransform (self, clientPoint : Point) -> Transform: ...
    def GetPickTransform (self, clientRectangle : Rectangle) -> Transform: ...
    def GetFrustumBoundingBox (self) -> BoundingBox: ...
    def Rotate (self, angleRadians : float, rotationAxis : Vector3d, rotationCenter : Point3d) -> bool: ...
    @property
    def IsValidCamera (self) -> bool: ...
    @property
    def IsValidFrustum (self) -> bool: ...
    @property
    def IsPerspectiveProjection (self) -> bool: ...
    @property
    def IsTwoPointPerspectiveProjection (self) -> bool: ...
    @property
    def IsParallelProjection (self) -> bool: ...
    def ChangeToParallelProjection (self, symmetricFrustum : bool) -> bool: ...
    def ChangeToPerspectiveProjection (self, symmetricFrustum : bool, lensLength : float) -> bool: ...
    def ChangeToPerspectiveProjection (self, targetDistance : float, symmetricFrustum : bool, lensLength : float) -> bool: ...
    def ChangeToTwoPointPerspectiveProjection (self, lensLength : float) -> bool: ...
    def ChangeToTwoPointPerspectiveProjection (self, targetDistance : float, up : Vector3d, lensLength : float) -> bool: ...
    @property
    def CameraLocation (self) -> Point3d: ...
    @property
    def CameraDirection (self) -> Vector3d: ...
    @property
    def CameraUp (self) -> Vector3d: ...
    @CameraUp.setter
    def CameraUp (self, value : Vector3d): ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class ViewportType:
    StandardModelingViewport = 0
    PageViewMainViewport = 1
    DetailViewport = 2
class EdgeContinuityMode:
    Distance = 0
    Tangency = 1
    Curvature = 2
class AnalysisStyle:
    Wireframe = 1
    Texture = 2
    FalseColor = 4
class FrameBufferFillMode:
    DefaultColor = 1
    SolidColor = 2
    Gradient2Color = 3
    Gradient4Color = 4
    Bitmap = 5
    Renderer = 6
    Transparent = 7
class BoundingBoxDisplayMode:
    None = 0
    OnAlways = 1
    OnDuringDynamicDisplay = 2
class ViewDisplayAttributes:
    @property
    def UseDocumentGrid (self) -> bool: ...
    @UseDocumentGrid.setter
    def UseDocumentGrid (self, value : bool): ...
    @property
    def DrawGrid (self) -> bool: ...
    @DrawGrid.setter
    def DrawGrid (self, value : bool): ...
    @property
    def DrawGridAxes (self) -> bool: ...
    @DrawGridAxes.setter
    def DrawGridAxes (self, value : bool): ...
    @property
    def DrawZAxis (self) -> bool: ...
    @DrawZAxis.setter
    def DrawZAxis (self, value : bool): ...
    @property
    def DrawWorldAxes (self) -> bool: ...
    @DrawWorldAxes.setter
    def DrawWorldAxes (self, value : bool): ...
    @property
    def ShowGridOnTop (self) -> bool: ...
    @ShowGridOnTop.setter
    def ShowGridOnTop (self, value : bool): ...
    @property
    def BlendGrid (self) -> bool: ...
    @BlendGrid.setter
    def BlendGrid (self, value : bool): ...
    @property
    def DrawTransparentGridPlane (self) -> bool: ...
    @DrawTransparentGridPlane.setter
    def DrawTransparentGridPlane (self, value : bool): ...
    @property
    def WorldAxisColorX (self) -> Color: ...
    @WorldAxisColorX.setter
    def WorldAxisColorX (self, value : Color): ...
    @property
    def WorldAxisColorY (self) -> Color: ...
    @WorldAxisColorY.setter
    def WorldAxisColorY (self, value : Color): ...
    @property
    def WorldAxisColorZ (self) -> Color: ...
    @WorldAxisColorZ.setter
    def WorldAxisColorZ (self, value : Color): ...
    @property
    def HorizontalViewportScale (self) -> float: ...
    @HorizontalViewportScale.setter
    def HorizontalViewportScale (self, value : float): ...
    @property
    def VerticalViewportScale (self) -> float: ...
    @VerticalViewportScale.setter
    def VerticalViewportScale (self, value : float): ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class MeshDisplayAttributes:
    @property
    def HighlightMeshes (self) -> bool: ...
    @HighlightMeshes.setter
    def HighlightMeshes (self, value : bool): ...
    @property
    def AllMeshWiresColor (self) -> Color: ...
    @AllMeshWiresColor.setter
    def AllMeshWiresColor (self, value : Color): ...
    @property
    def MeshWireThickness (self) -> int: ...
    @MeshWireThickness.setter
    def MeshWireThickness (self, value : int): ...
    @property
    def ShowMeshWires (self) -> bool: ...
    @ShowMeshWires.setter
    def ShowMeshWires (self, value : bool): ...
    @property
    def ShowMeshVertices (self) -> bool: ...
    @ShowMeshVertices.setter
    def ShowMeshVertices (self, value : bool): ...
    def ToString (self) -> str: ...
    def Equals (self, obj : Object) -> bool: ...
    def GetHashCode (self) -> int: ...
    def GetType (self) -> Type: ...
class ContextsForDraw:
    Unset = 0
    FilePreview = 1
    ViewCapture = 2
    Printing = 3
    UIPreview = 4
    Mask = 5
    RenderOverlays = 6
class StereoRenderContextEnum:
    NotApplicable = 0
    RenderingLeftEye = 1
    RenderingRightEye = 2
    RenderingBothEyes = 3
class AnchorLocation:
    LowerLeft = 0
    UpperLeft = 1
    LowerRight = 2
    UpperRight = 3
    Center = 4
class ColorMode:
    DisplayColor = 0
    PrintColor = 1
    BlackAndWhite = 2
