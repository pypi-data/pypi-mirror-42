...
# internal libs
from ..handlers import BaseHandler, ConsoleHandler, FileHandler
...
FILE_PATHS       = os.getenv('LOGALPHA_FILE_PATHS', default='')
...
def get_variables(template: str) -> List[str]:
    ...
    return [match.groups()[0] for match in VAR_REGEX.findall(template)]
...
def process_template(template: str) -> Dict[str, str]:
    ...
    return {name: (lambda: shell(os.getenv(name)))
