    ...
    def thread_monitor(self):
        ...
        while not pipeline_finished:
            ...
            if counter % STATS_COUNTER_MODULO == 0:
                ...
                logging.debug('\n' + str(table))
                if error:
                    logging.debug('Terminating due to error.')
            ...
            for node in self.all_connected():
                ...
                #logging.debug('QUEUE SIZES: {node} {q}'.format(node=node.name, q=str(input_queue_sizes)))
                logjam = not node.is_source and all(input_queue_full) and not any(output_queue_full)
                #logging.debug('INPUT QUEUE STATE: {node} {full}'.format(node=node.name, full=str(input_queue_full)))
                #logging.debug('OUTPUT QUEUE STATE: {node} {full}'.format(node=node.name, full=str(outputput_queue_full)))
