...
def main_dict(gracedb_id, client):
    ...
    for voevent in voevents:
        ...
        url = root.find('./What/Group/Param[@name="skymap_fits_shib"]')
        if url is None:
            continue
...
def uncertainty_ellipse(graceid, filename, client, cl=90):
    ...
    if filename.endswith('.gz'):
        multires_filename = filename[:-3]
        try:
            skymap_table = read_map_gracedb(graceid, multires_filename, client,
                                            moc_table=True)
            prob = read_map_gracedb(graceid, multires_filename, client)[0]
        except (IOError, rest.HTTPError):
            skymap_table = read_map_gracedb(graceid, filename, client,
                                            moc_table=True)
