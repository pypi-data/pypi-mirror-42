...
class QII(object):
    ...
    def compute(self, x_0, predictor, show_approx=False, evaluated_features=None,
                ...
                pool=600, n_samplings=600, method='shapley'):
        """
        @params:
            ...
            - pool: it could be number of instances to be sampled from, an integer,
            or the index list of specific instances, a 1-D integer list.
        ...
        if method not in ['shapley', 'banzhaf']:
            ...
            self._determine_params(evaluated_features, data_exhaustive, 
                                   feature_exhaustive, pool, n_samplings, method)
    ...
    def compute_banzhaf(self):
        ...
        if (self._feature_exhaustive is True):
            ...
            # or approximately
            for count_samplings in range(0, self._numb_samplings):
                    
                r = np.random.ranf(self._n_features)
                subset = [i for i in range(self._n_features) if r[i] > 0.5]
    ...
    def _determine_params(self, evaluated_features, data_exhaustive, 
        ...
        if method == 'shapley':
            if (data_exhaustive is True):
                pool = self._data.shape[0]
            else:
                if (isinstance(pool, int)):
                    pool = min(pool, self._data.shape[0])
                    
            if (feature_exhaustive is True):
                self._n_samplings = math.factorial(self._n_features)
            else:
                self._n_samplings = min(n_samplings, math.factorial(self._n_features))
            ...
            if (feature_exhaustive is True):
                self._n_samplings = 2**self._n_features - 1
            else:
                self._n_samplings = min(n_samplings, 2**self._n_features - 1)
       
        if (isinstance(pool, int)):
            pool_idxs = np.random.randint(0, self._data.shape[0], pool)
        else:
            pool_idxs = pool
        
        self._x_pool = self._data[pool_idxs] 
