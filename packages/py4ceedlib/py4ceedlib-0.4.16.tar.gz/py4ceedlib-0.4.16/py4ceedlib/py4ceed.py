...
def get_all_collections(key):
    url = '%s/api/collections?key=%s' % (base_url, key)
    #url = '%s/api/collections' % (base_url)
    colls = requests.get(url).json()
    detailed_colls = []
    for coll in colls:
...
########### Start  API calls to 4Ceed  with credentials ################
'''
...
def get_template_by_dataset_id(dataset_id, credentials):
    ...
    return requests.get(url, auth=credentials, verify=False).json()
'''
########### End API calls to 4Ceed with credentials ################
...
def read_datasets_metadata():
    ...
    check_items = build_tree_view(this.key, show_files=False)
