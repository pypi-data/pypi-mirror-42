...
def device_setter(device=None):
    ...
    def device_fn(op: tf.Operation):
        cpu_ops = ['Trainable_Variables', 'RandomShuffle', 'Summary', 'Const', 'Assert', 'StringJoin']
...
class Model:
    def __init__(self, name, export_dir, version, device=None):
        self._name = name
    ...
    def load(self):
        ...
        with graph.device(device_setter(self.device)):
            meta_graph_def = tf.saved_model.loader.load(self._sess, [tf.saved_model.tag_constants.SERVING],
                                                        self.model_dir)
