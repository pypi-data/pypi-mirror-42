...
from datapool.instance.config_handling import config_for_develop_db, read_config
from datapool.instance.domain_objects_checker import DomainObjectsChecker
from datapool.instance.landing_zone_structure import (
    required_folders,
...
from ..errors import DataPoolException, InvalidLandingZone, InvalidOperationError
...
def _run_script_checks(
    ...
    for script_path, data_path in lz.conversion_scripts_and_data():
        ...
        if not ok:
            ...
            print_ok(
                "  - first conversion needed {:.0f} msec".format(needed_conv * 1000)
            )
            if run_twice:
                ok, needed_conv, __ = _check_conversion(
                    runner,
                    lz,
                    script_path,
                    data_path,
                ...
                if not ok:
                    ...
                    print_ok(
                        "  - second conversion needed {:.0f} msec".format(
                            needed_conv * 1000
                        )
                    )
...
def _check_conversion(
    ...
    for result in runner.check_conversion(
        lz.p(script_path), lz.p(data_path), status_callback=False, verbose=verbose
    ):
