...
class Pypykatz(common.WindowsCommandPlugin):
	...
	__args = [
		dict(name="override-timestamp", type="bool", required=False,
	...
	def collect(self):
		...
		if self.plugin_args.kerberos_dir:
			dir = os.path.abspath(self.plugin_args.kerberos_dir)
			self.session.logging.info('Writing kerberos tickets to %s' % dir)
			base_filename = ntpath.basename(filename)
			ccache_filename = '%s_%s.ccache' % (base_filename, os.urandom(4).hex()) #to avoid collisions
			mimi.kerberos_ccache.to_file(os.path.join(dir, ccache_filename))
			for luid in mimi.logon_sessions:
