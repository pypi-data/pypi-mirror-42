...
class LocationFileLine(object):
    ...
    def add_aug(self, trigger_services, output, aug):
        ...
        def code_object_callback(code_object, filename, aug_id):
            ...
            try:
                if aug_id == aug.aug_id:
                    trigger_services.get_service(BdbLocationService.NAME).mark_aug_active(aug)
                    return
        ...
        if import_service.register_post_import_notification(aug.aug_id, self.module_name, self.filename, self.include_externals, callback, code_object_callback, removed):
            aug.set_pending()
