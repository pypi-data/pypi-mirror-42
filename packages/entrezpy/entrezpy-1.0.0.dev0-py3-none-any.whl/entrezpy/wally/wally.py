...
class Wally:
  ...
  class Query:
    ...
    def __init__(self, function, parameter, dependency=None, analyzer=None):
      if not parameter and not dependency:
        logger.error(json.dumps({__name__:{"Error": "Missing expected parameters 'parameter' and/or `dependency`.",
                                                    "action":"abort"}}))
        sys.exit()
      if not parameter:
        parameter = {}
  ...
  class Pipeline:
    ...
    def add_fetch(self, parameter=None, dependency=None, analyzer=None):
      if analyzer == None:
        logger.error(json.dumps({__name__:{"Error": "Missing function parameter" : "analyzer",
                                                    "action":"abort"}}))
  ...
  def run(self, qry):
    while not qry.queries.empty():
      ...
      logger.info(json.dumps({"Inquire" : {"id" : q.id, "function": q.function}}))
