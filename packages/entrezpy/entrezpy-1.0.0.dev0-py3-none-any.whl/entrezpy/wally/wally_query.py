...
class WallyQuery:
  """ Basic entrezpy query for Wally pipeline

  Wally retursn It's responsible for storing and resolving query dependencies
  """
  ##Ctor
  ...
  # @param[in] analyzer analyzer instance for this query
  def __init__(self, category, parameter, dependency=None, analyzer=None):
    if not parameter and not dependency:
      logger.error(json.dumps({__name__:{"Error": "Missing parameter and dependency",
    ...
    if not parameter:
      parameter = {}
    self.id = base64.urlsafe_b64encode(uuid.uuid4().bytes).decode() ##< Breid test
