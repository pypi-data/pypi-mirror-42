...
class RandomGrid(OptimizationMethod):
    ...
    """
    def __init__(self, seed, x0_square_size=1e-05, x0_square_samples=0.1):
        """

        """
        self.x0_square_size = x0_square_size
        self.x0_square_samples = x0_square_samples

        super(RandomGrid, self).__init__(seed)

    def minimize(self, fun, x0, args=(),
                 bounds=(), maxiter=200, **unknown_options):
        """
        ...
        x_t1 = random_sample(
            bounds,
            int(maxiter * (1.0-self.x0_square_samples))
        )
        ...
        x_t2 = random_sample(
            [
                (
                    x0_i - (self.x0_square_size * (bounds_i[1]-bounds_i[0])),
                    x0_i + (self.x0_square_size * (bounds_i[1]-bounds_i[0]))
                )
                for x0_i, bounds_i in zip(x0, bounds)
            ],
            int(maxiter * self.x0_square_samples)
        )
