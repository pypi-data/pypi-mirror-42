...
class EngineBase(metaclass=ABCMeta):
    ...
    def __eq__(self, other):
        ...
        if self.__class__ != other.__class__:
            logger.debug('{} and {}.__class__ differs'.format(self, other))
            return False

        if set(self.__dict__) != set(other.__dict__):
            logger.debug('{} and {}.__dict__ differs'.format(self, other))
            return False
        ...
        for i in ['engine_type', 'needs_order',
            ...
            if hasattr(self, i):
                if getattr(self, i) != getattr(other, i):
                    logger.debug('{} for {} and {}, attributes are {} and '
                                 '{}'.format(i, self, other,
                                             getattr(self, i),
                                             getattr(other, i)))
                    return False
        ...
        if hasattr(self, 'rgen'):
            ...
            for att1, att2 in zip(self.rgen.__dict__, other.rgen.__dict__):
                ...
                if self.rgen.__dict__[att1] != other.rgen.__dict__[att2]:
                    logger.debug('rgen class attribute {} and {} '
                                 'differs'.format(att1, att2))
                    return False
