...
class CP2KEngine(ExternalMDEngine):
    ...
    def __init__(self, cp2k, input_path, timestep, subcycles, input_files,
        ...
        for key in {'conf', 'template'}:
            ...
            self.input_files[key] = input_files.get(
                key, os.path.join(self.input_path, default_files[key]))

            if not os.path.isfile(self.input_files[key]):
                msg = 'CP2K engine could not find file "{}"!'.format(
                    default_files[key])
    ...
    def integrate(self, ensemble, steps, thermo='full'):
        ...
        # Get the order parameter before the run:
        if order_function:
            order = self.calculate_order({'order_function': order_function,
                                          'system': system},
                                         xyz=xyz, vel=vel, box=box)
        else:
            order = None
            ...
            if i < steps - 1:
                ...
                if order_function:
                    order = self.calculate_order(
                        {'order_function': order_function, 'system': system},
                        xyz=xyz, vel=vel, box=box)
    ...
    def modify_velocities(self, ensemble, vel_settings):
        ...
        if 'aimless' in vel_settings and vel_settings['aimless']:
            ...
            logger.error(msgtxt)
            raise NotImplementedError(msgtxt)
        if 'momentum' in vel_settings and vel_settings['momentum']:
            pass
