...
class PathEnsembleTest(unittest.TestCase):
    ...
    def test_logic(self):
        """Test eq and ne."""
        path_ensemble1 = PathEnsemble(0, [-1, 0, 1], detect=0, maxpath=10,
                                      exe_dir=None)
        path_ensemble2 = PathEnsemble(0, [-1, 0, 1], detect=0, maxpath=10,
                                      exe_dir=None)
        path_ensemble2.rgen = path_ensemble1.rgen
        self.assertTrue(path_ensemble2 == path_ensemble1)

        path_ensemble2 = PathEnsemble(0, [-2, 0, 1], detect=0, maxpath=10,
                                      exe_dir=None)
        ...
        path_ensemble2 = PathEnsemble(0, [-1, 0, 1], detect=16, maxpath=10,
                                      exe_dir=None)
        with patch('sys.stdout', new=StringIO()):
            self.assertTrue(path_ensemble2 != path_ensemble1)
    ...
    def test_init(self):
        """Test initiation."""
        path_ensemble = PathEnsemble(0, [-1, 0, 1], detect=0, maxpath=10,
                                     exe_dir=None)
        self.assertEqual(path_ensemble.start_condition, 'R')
        self.assertEqual(path_ensemble.ensemble_name_simple, '000')
        path_ensemble = PathEnsemble(11, [-1, 0, 1], detect=0, maxpath=10,
                                     exe_dir=None)
    ...
    def test_add_path(self):
        ...
        path_ensemble = PathEnsemble(1, [-1, 0, 1], detect=0, maxpath=10)
        # Add a path
    ...
    def test_looping(self):
        ...
        path_ensemble = PathEnsemble(1, [-1, 0, 1], detect=0, maxpath=20)
    ...
    def test_restart_info1(self):
        ...
        path_ensemble2 = PathEnsemble(10, [0, 0, 0], detect=100, maxpath=1)
    ...
    def test_restart_info2(self):
        """Test that we can make restart info."""
        path_ensemble = PathEnsemble(1, [-1, 0, 1], detect=0)
        for i in range(5):
            path = make_fake_path(length=10+i)
            path_ensemble.add_path_data(path, 'ACC')
        path_ensemble.add_path_data(None, 'KOB')
        path_ensemble.add_path_data(None, 'KOB')
        info = path_ensemble.restart_info()
        path_ensemble2 = PathEnsemble(10, [0, 0, 0], detect=100)
        path_ensemble2.load_restart_info(info)
