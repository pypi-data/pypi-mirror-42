...
It contains some slave functions that are used in the in/output fucntion
of PyRETIS.
...
def create_empty_ensembles(settings):
    """Create missing ensemble in the settings.
    ...
    add0 = 0
    if not settings['simulation']['flux']:
        add0 += 1
    if not settings['simulation']['zero_ensemble']:
        add0 += 1
    ...
    # if in the main settings an ensmeble_number is defined, than only
    ...
    # todo remove this labelling missmatch, and give to the flux
    ...
    # nb. in the settings, specific input for an ensemable can be now given.
    for i_ens, ens in enumerate(settings['ensemble']):
        for sav in orig_set:
            if 'ensemble_number' in sav:
                if ens['tis']['ensemble_number'] == sav['ensemble_number']:
                    settings['ensemble'][i_ens].update(sav)
            elif 'tis ensemble_number' in sav:
                if ens['tis']['ensemble_number'] == sav['tis ensemble_number']:
                    settings['ensemble'][i_ens].update(sav)
