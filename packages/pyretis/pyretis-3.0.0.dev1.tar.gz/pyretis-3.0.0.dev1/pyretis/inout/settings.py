...
from pyretis.inout.checker import check_interfaces
    ...
    for i_ens, ens in enumerate(ensemble_save):
        for key in list(ens):
            ...
            if clean_key in SPECIAL_MULTIPLE:
                ...
                if len(key.split()) == 2:
                    for key_run in\
                            [x for x in range(clean_num+1) if x == clean_num]:
                ...
                if len(key.split()) == 2:  # and key_list[0] != 'heading':
                    if not key_list[0] in ensemble_save[i_ens]:
...
def _add_default_settings2(settings):
    """Add default settings.  # todo, to complete this.
    ...
    for sec in SECTIONS:
        if sec not in settings:
            if sec in SPECIAL_MULTIPLE:
                settings[sec] = [i for i in SECTIONS[sec]]
            else:
                settings[sec] = {}
...
def _add_specific_default_settings(settings):
    ...
    if settings['engine'].get('class') in {'gromacs2', 'gromacs',
                                           'cp2k', 'lammps'}:
        ...
        # Input files for cp2k runners:
        if 'cp2k' in settings['engine']['class']:
            default_files = {'conf': 'initial.xyz',
                             'template': 'cp2k.inp'}
        ...
        # Defualt input files for gromacs and gromacs 2 runners:
        if 'gromacs' in settings['engine']['class'][0:7]:
            default_files = {'conf': 'conf.gro',
                             'input_o': 'grompp.mdp',
                             'topology': 'topol.top',
                             'index': 'index.ndx'}
            settings['engine']['input_files'] = {}
