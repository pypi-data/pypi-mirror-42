...
def initiate_kicki(simulation, settings, cycle):
    ...
    for ensemble, set_ens in zip(simulation.ensembles, settings['ensemble']):
        print_to_screen('Running "kick" initiation in: {}'
                        .format(ensemble['path_ensemble'].ensemble_name))
...
def initiate_kick_max(simulation, settings, cycle):
    ...
    for ensemble, set_ens in zip(simulation.ensembles, settings['ensemble']):
        ...
        print_to_screen('Running kick-max initiation in: {}'.format(
...
def generate_initial_path_kick(ensemble, tis_settings):
    ...
    while True:
        ...
        # todo interfaces shall be moved from simulation to tis
        ensemble['interfaces'] = interfaces
        leftpoint, _ = engine.kick_across_middle(ensemble,
                                                 interfaces[1],
                                                 tis_settings)
        ...
        if start == ensemble['path_ensemble'].start_condition:  # Case 0) & 1):
            break
        else:
            # Now we do the other cases:
            if end == ensemble['path_ensemble'].start_condition:
...
def fix_path_by_tis(initial_path, ensemble, tis_settings):
    ...
    while not path_ok:
        ...
        accept, trial, _ = make_tis_step(ensemble,
                                         local_tis_settings,
