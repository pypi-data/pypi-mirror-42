...
def initial_positions_file(settings):
    ...
    if vel_read:
        logger.info('Read velocoties from file: "%s".',
                    pos_settings['input_file'])
...
def create_system(settings):
    ...
    if 'restart' in settings:
        klass = get_particle_type(
            settings['restart']['particles'].get('type', 'internal')
        )
        particles = klass(dim=settings['restart']['particles'].get('dim', 0))
        system = System()
        system.box = box_from_restart(settings['restart']['system'])
        ...
        if ('engine' not in settings or
            ...
            particles.set_pos(
                (settings['engine']['input_files'].get('conf'), None))
            # todo Should be also possible to use velocities
            particles.set_vel(False)
            box = set_up_box(settings, None, dim=3)
        ...
        if 'input_file' not in settings['system']:
