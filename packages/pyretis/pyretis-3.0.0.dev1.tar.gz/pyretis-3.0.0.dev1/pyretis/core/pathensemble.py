...
class PathEnsemble:
    ...
    def __init__(self, ensemble_number, interfaces,
                 rgen=None, maxpath=10000, detect=None, exe_dir=None):
        ...
        for i in ['directory', 'interfaces', 'nstats', 'paths']:
            if hasattr(self, i):
                for j, k in zip(getattr(self, i), getattr(other, i)):
                    if j != k:
                        logger.debug('{} for {} and {},'
                                     ' attributes are {} and '
                                     '{}'.format(i, self, other, j, k))
                        return False

        for i in ['detect', 'ensemble_name',
