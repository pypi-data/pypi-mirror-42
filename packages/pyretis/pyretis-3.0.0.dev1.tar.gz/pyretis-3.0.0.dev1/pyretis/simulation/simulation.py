...
class Simulation:
    ...
    def __init__(self, settings, controls):
        ...
        end = controls.get('end', steps)
        self.cycle = {'step': startcycle, 'end': end,
    ...
    def restart_info(self):
        ...
        """
        if self.settings:
            info = copy.deepcopy(self.settings)
        else:
            info = {}

        if 'simulation' not in info:
            info['simulation'] = {}
    ...
    def load_restart_info(self, info):
        ...
        if info.get('particles') is not None and hasattr(self, 'particles') \
                and self.system.particles is not None:
            self.system.particles.load_restart_info(info['particles'])
