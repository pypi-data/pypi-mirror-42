...
class Client(object):
  ...
  def config(self, project_id, service, service_version, enable_cpu_profiling,
             enable_wall_profiling, period_ms, discovery_service_url):
    ...
    Args:
      ...
      enable_cpu_profiling: A bool specifying whether or not the CPU time
        profiling should be enabled. See docs in __init__.py for more details.
      enable_wall_profiling: A bool specifying whether or not the WALL time
        profiling should be enabled. See docs in __init__.py for more details.
    ...
    Raises:
      ValueError: If the project ID or service can't be determined from the
        ...
        enabled.
      NotImplementedError: If enable CPU time profiling when Python version
        is older than 3.2.
    """
    self._profilers = {}
    cpu_profiling_supported = cpu_profiler is not None
    if enable_cpu_profiling is None:
      enable_cpu_profiling = cpu_profiling_supported

    if enable_cpu_profiling:
      if not cpu_profiling_supported:
        raise NotImplementedError('CPU profiling is supported only on Python '
                                  '3.2 and higher.')
      else:
        self._profilers['CPU'] = cpu_profiler.CPUProfiler(period_ms)
    if enable_wall_profiling:
