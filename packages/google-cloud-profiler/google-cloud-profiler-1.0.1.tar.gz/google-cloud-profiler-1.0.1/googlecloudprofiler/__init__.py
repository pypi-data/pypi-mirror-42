...
def start(service=None,
          ...
          verbose=0,
          enable_cpu_profiling=None,
          enable_wall_profiling=True,
          period_ms=10):
  """Starts the profiler.

  This function should only be called once. If using Python2-compatible mode,
  this function must be called on the main thread. It starts a daemon thread
  which polls the profiler server for instructions, and collects and uploads
  ...
  Args:
    ...
    enable_cpu_profiling: An optional bool specifying whether or not the CPU
      time profiling should be enabled. CPU profiling is only supported for
      Python 3.2 or higher. Setting this flag to True with an older Python
      results in a NotImplementedError. Defaults to True when Python version
      equals to or is higher than 3.2, defaults to False otherwise.
    enable_wall_profiling: An optional bool specifying whether or not the Wall
      time profiling should be enabled. The current wall time profiling avoids
      dependency on any native code by using Python signal module. It only
      profiles the main thread. The start function must be called from the main
      thread if wall time profiling is enabled. Using SIGALRM signal after
      starting the profiler will cause problems: registering a handler for
      SIGALRM will prevent the profiler from working. SIGALRM will be trigger
      by the profiler at unpredictable time. Wall profiling has some other
      limitations as documented in the pythonprofiler module. Defaults to True.
  ...
  Raises:
    ...
    ValueError: If arguments are invalid or if necessary information can't be
      ...
      mode is enabled.
    NotImplementedError: If run on Windows. Or if enable CPU time profiling
      when Python version is older than 3.2.
  """
  global _started
  if _started:
  ...
  profiler_client.config(project_id, service, service_version,
                         enable_cpu_profiling, enable_wall_profiling, period_ms)
