...
def generate_continue_series_based_on_napoints(data,fac = 'ClosePrice',napoint_name=None):
    '''
    :param data:
    :param fac:
    :param napoint_name: data的分割序列为0和1
    :return:
    '''
    data[napoint_name].iloc[0] = 1
    data[napoint_name].iloc[-1] = 1

    data[fac + '_z'] = np.where(data[napoint_name]!=0, data[fac],np.nan)
    data[fac + '_z'].iloc[0] = data[fac].iloc[0]
    data['steps'] = np.arange(0, len(data))
    data_z = data[data[napoint_name]!=0]
    data_z[fac + '_z_delta'] = data_z[fac + '_z'].shift(-1) - data_z[fac + '_z']
    data_z['steps_delta'] = data_z['steps'].shift(-1) - data_z['steps']
    data_z['add_fac'] = data_z[fac + '_z_delta'] / data_z['steps_delta']
    data['add_fac'] = data_z['add_fac']
    data['add_fac'] = data['add_fac'].fillna(method='ffill')
    data['cum_add_fac'] = data['add_fac'].cumsum()
    data['z_result'] = data[fac + '_z'].iloc[0] + data['cum_add_fac'].shift(1)
    data['z_result'].iloc[0] = data[fac + '_z'].iloc[0]
    data[fac+'_z'] = data['z_result']
    return data[[fac + '_z']]
