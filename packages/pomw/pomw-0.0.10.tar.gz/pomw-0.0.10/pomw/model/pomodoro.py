...
class Pomodoro(Interval):
    def __init__(
        ...
        super().__init__(task=task, start_time=start_time, end_time=end_time)
    ...
    def __json_decode__(d):
        return Pomodoro(
            task=d.get("_Interval__task", None),
    ...
    def time(self, time):
        if isinstance(time, datetime):
            ...
            raise TypeError("'time' should be of type %s or None" % (datetime.__name__))
