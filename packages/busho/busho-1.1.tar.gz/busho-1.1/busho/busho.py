...
class Busho:
    ...
    def deploy(self):
        if self.local_host_info['host'] == self.running_host:
            ...
            for host_info in self.remote_host_set.values():
                engine.add_task(self._init_project, host_info)
            engine.run()
    ...
    def _init_project(self, host_info):
        with SSHClient() as ssh:
            ...
            _, stdout, _ = ssh.exec_command('pip3 install --upgrade busho')
            assert stdout.read().decode(), 'pip3 install error: pip3 install --upgrade busho'
