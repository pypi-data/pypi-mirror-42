...
def partition_inference(traj_mat, comp, timetol, trajsize, skiptol):
    vecinfo = {}
    vecinfo['vec'] = []
        ...
        if len(tcomp) > 0:
            for traj in tcomp:
                ...
                if ignore == 0:
                    ...
                    if (stopind - i) >= timetol and len(traj) >= trajsize:
    ...
    vecinfo = vecinfo[vecinfo['length'] >= timetol]
    vecinfo = vecinfo[vecinfo['size'] >= trajsize]
...
def find_tctc_hdf(data, timetol, disttol, trajsize, skiptol=0, rule='convoy', noise=None, largestonly=False, raw_signal='amplitude', return_array=False, return_df=True, tempdir=None, savedf=False, savename=None, njobs=1):
    # Data is time,node
    # Get distance matrix
...
def find_tctc(data, timetol, disttol, trajsize, skiptol=0, largedataset=False, rule='convoy', noise=None, largestonly=False, raw_signal='amplitude', return_df=True, return_array=False, tempdir=None, savedf=False, savename=None, njobs=1):
    # Get distance matrix
    if largedataset:
        return find_tctc_hdf(data, timetol, disttol, trajsize, skiptol=skiptol, rule=rule, noise=noise, largestonly=largestonly, raw_signal=raw_signal, return_df=return_df, return_array=return_array, tempdir=tempdir, savedf=savedf, savename=savename, njobs=njobs)
    else:
        N_data = data.shape[1]
        if noise is not None:
            if len(noise.shape) == 1:
        ...
        if return_array == True:
            ...
            if np.sum(traj_mat) != 0:
                df = partition_inference(
                    traj_mat, cliques, timetol, trajsize, skiptol)
                return df
            else:
                return []
