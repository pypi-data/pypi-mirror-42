...
class ArtifactManager(object):
  ...
  def __init__(self, store, id, index=0, valid_seconds=None):
    ...
    *store* must be able to store pairs float, string.
    ...
    self._valid = valid_seconds if valid_seconds is not None and valid_seconds > 0 else self.VALID_SECONDS

  def store(self, doc):
    return self._store.store((time(), doc))
  ...
  @classmethod
  def from_uri(cls_, store, uri, *args, **kw):
    from hashlib import sha1
    b_uri = to_bytes(uri)
    m = cls_(store, sha1(b_uri).digest(), *args, **kw)
    m.uri = uri
    return m
