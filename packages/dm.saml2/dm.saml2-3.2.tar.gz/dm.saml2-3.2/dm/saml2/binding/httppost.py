...
def encode(message, relay_state=None):
  ...
  *message* is a ``PyXB`` binding for an SAML request or response.
  ...
  params[isinstance(message, ResponseType) and 'SAMLResponse' or 'SAMLRequest'] = \
                             to_str(b64encode(message.toxml("utf-8")))
  return params


def as_controls(params):
  """encode *params* (a dict) as hidden form controls (an XHTML fragment)."""
  cl = []
...
def decode(params):
  ...
  """
  return (b64decode(to_bytes(
    params.get('SAMLRequest') or params.get('SAMLResponse')
    )),
