...
from sweetpea.metrics import __count_solutions, __count_sublists
...
@pytest.mark.skip
@pytest.mark.parametrize('l,k,mag_v,expected', [
    (6, 2, 2, 5),
    (6, 2, 3, 16),
    (6, 2, 4, 15)
])
def test_count_sublists(l, k, mag_v, expected):
    assert __count_sublists(l, k, mag_v) == expected
