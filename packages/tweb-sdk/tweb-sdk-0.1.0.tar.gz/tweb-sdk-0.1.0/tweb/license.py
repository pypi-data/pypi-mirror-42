...
class License:
    ...
    def __init__(self, text=None):
        ...
        self.authority = 'world'  # 发证机构代码，一般在子类中修改
        self.secret = 'Lic123!@#'  # 发证机构代签名密码，一般在子类中修改
        self.timeout = 604800  # 有效性超时时间, 秒
        self.idle_time = 86400  # 闲置超时时间, 秒
    ...
    def parse(self):
        ...
        if not re.match(r'[0-1]{%s};[\-0-9,]*;[\-0-9~,]*' % len(self.SWITCH), self.text):
            raise self._err_text()

        domains = self.text.split(';')

        # 开关相关
        sw_str = domains[0]
        if len(sw_str) != len(self.SWITCH):
            raise self._err_text()
    ...
    def update(self, resource, owner, json, operator_license):
        ...
        if not self.editing:
            raise ErrException(ERROR.E40300, extra='forbidden because not in editing mode')
    ...
    def verify(self, signed, nonce):
        """
        校验授权签名的合法性
        :param signed: 签名的授权字符串
        :param nonce: 临时一致性标示，需与生成签名时使用的nonce相同
        :return: 授权对象
        """
    ...
    def operable(self, cur_resource, action):
        """
        检查是否允许进行指定操作
        :param cur_resource: 将被操作的资源路径，如'root/club/table'
        :param action: 操作，如create, read...
        """
        if self.editing:
            raise ErrException(ERROR.E40300, extra='forbidden in editing mode')
    ...
    def _key(self, nonce=''):
        return f'license/{self.authority}/{tools.gen_md5(self.resource + self.owner)}/{nonce}'
