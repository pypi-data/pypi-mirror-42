...
class GetHitsThread(threading.Thread):
	...
	def run(self):
		self.hits = es.scrollSearch('statistics_works_hot_fixed_backups', 'recommendviewrecordsearch', {"query":{"bool":{"must":[{"range":{"time":{"lte":1550567166000}}},{"range":{"uid":{"gte":self.first,"lte":self.last}}}]}},"size":10000})
...
def getHits(uids):
	...
	while firstIndex < len(uids):
		if len(threading.enumerate()) > 5:
			...
			lastIndex = min(firstIndex + 2499, len(uids) - 1)
...
def recordEsToRedisBatch():
	...
	while True:
		...
		for hit in hits:
			uid = hit['_source']['uid']
			vid = str(es.getIntByVid(hit['_parent']))
