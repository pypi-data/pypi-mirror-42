...
#dataEsclient = Elasticsearch(['10.168.0.119:9200', '10.168.0.121:9200', '10.168.0.118:9200', '10.168.0.127:9200'], http_auth=('huangzhen', 'df25j0934j4y09hjoijoigef'), timeout=120)
...
def initVidDict():
	...
	for item in searchDb(db, 'select * from tb_videointid limit 100'):
...
def getAllUids(lastUid):
	...
	uids = []
	for item in searchDb(db, 'select uid from tb_user where uid > ' + str(lastUid) + ' order by uid asc limit 5'):
		uids.append(item[0])
	db.close()
	return uids
