...
from amino import Empty, Just, __, Map, List, Either, _, Maybe, L
...
from series.tvdb.api import airdate, show_by_id
...
class RestApi(RestApiBase):
    ...
    def create_release(self, series, season, episode):
        if self._releases.find_by_metadata(series, season, episode):
            ...
            def create(show: Show) -> str:
                ad = airdate(show, season, episode).attempt.join.get_or_strict(None)
                search_name = show.search_name
                self._releases.create(series, season, episode, airdate=ad, search_name=search_name)
    ...
    def _add_show(self, name: str, id: int) -> None:
        def add(show: Show) -> None:
            ...
            return 'done'
        return show_by_id(id).cata(error(f'invalid tvdb id {id} for {name}', 400), add)
