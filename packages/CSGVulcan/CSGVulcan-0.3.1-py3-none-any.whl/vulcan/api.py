...
from flask import Flask, Response, json, request, g, Blueprint
...
def register_vulcan_blueprint(resource, flask_app, name=None):
    VulcanAPIBlueprint = Blueprint(name, __name__, '', url_prefix='/api')

    @VulcanAPIBlueprint.before_request
    def handle_before_request():
        """Before each request, start timer to capture request start-time in epochs"""
        g.start = time()

    @VulcanAPIBlueprint.after_request
    ...
    @VulcanAPIBlueprint.route("/init/<id>", methods=['GET', 'PUT', 'POST'])
    def handle_init_with_param(id):
        """Calls VulcanResource.init with params"""
        try:
            context = {
                'id': id
            }
            res = resource.init(context)
            payload = json.dumps({'data': res})
            return Response(payload, mimetype="application/json")
        except Exception:
            raise VulcanAPIError("Init method failure")

    @VulcanAPIBlueprint.route("/init", methods=['GET', 'PUT', 'POST'])
    def handle_init():
        """Calls VulcanResource.init method"""
        try:
            res = resource.init()
    ...
    @VulcanAPIBlueprint.route("/fetch-one", methods=['GET', 'PUT', 'POST'])
    def handle_fetch_one():
        """Calls VulcanResource.fetch_one method"""
        try:
            context = request.get_json()
            res = resource.fetch_one(context)
            payload = json.dumps({'data': res})
            return Response(payload, mimetype="application/json")
        except Exception:
            raise VulcanAPIError("FetchOne method failure")

    @VulcanAPIBlueprint.route("/fetch-all", methods=['GET', 'PUT', 'POST'])
    def handle_fetch_all():
        """Calls VulcanResource.fetch_all method"""
        try:
            context = request.get_json()
            res = resource.fetch_all(context)
            payload = json.dumps({'data': res})
            return Response(payload, mimetype="application/json")
        except Exception:
            raise VulcanAPIError("FetchAll method failure")

    @VulcanAPIBlueprint.errorhandler(VulcanAPIError)
    def handle_vulcan_api_error(error):
        """Handle exceptions when raised"""
        payload = json.dumps({'data': None, 'errors': [error.to_dict()]})
        return Response(
            payload,
    ...
    flask_app.register_blueprint(VulcanAPIBlueprint)
