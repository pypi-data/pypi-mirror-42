...
class transcribe:
    ...
    def __init__(self, filepath):
        self._setup()
        self.tr = boto3.client('transcribe')
    ...
    def check_for_bucket(bucket_name):
        return bool(self.s3.Bucket(bucket_name).creation_date)
    ...
    def get_transcription_jobs(status=None, job_name_query=None):
        ...
        jobs_data = self.tr.list_transcription_jobs(**kwargs)
        jobs = homogenize_transcription_job_data(jobs_data['TranscriptionJobSummaries'])
        while jobs_data.get('NextToken'):
            jobs_data = self.tr.list_transcription_jobs(NextToken=jobs_data['NextToken'])
    ...
    def retrieve_transcript(transcription_job_name):
        job = self.tr.get_transcription_job(
        ...
        s3_object = self.s3.Object(transcript_bucket, transcript_key).get()
