        ...
        for key, profile in load_profiles(self.config).items():
            ...
            if not profile.watcher.consumes_self:
                ...
                for consumer in profile.consumers:
                    ...
                    thread = Thread(target=consume_changes, name=name, args=(self.config, profile, consumer))
                    thread.daemon = True
                    thread.start()
...
def consume_changes(config, profile, consumer):
    """
    ...
    while True:
        ...
        if first:
            ...
            changes = session.query(model.DataSyncChange)\
                             .filter(model.DataSyncChange.source == profile.key)\
                             .filter(model.DataSyncChange.consumer == consumer.key)\
                             .filter(model.DataSyncChange.obtained == first.obtained)\
                             .all()

            log.debug("found %s changes to process from %s", len(changes),
                      localtime(config, first.obtained, from_utc=True))
            ...
            try:
                ...
                # TODO: should send configurable email here (w/ prettier template)
        ...
        if error: # from consumer.process_changes()
            session.rollback()
            session.close()
            break # exit the perma-loop; this thread is now dead

        else: # all good, so commit
            session.commit()
            session.close()
