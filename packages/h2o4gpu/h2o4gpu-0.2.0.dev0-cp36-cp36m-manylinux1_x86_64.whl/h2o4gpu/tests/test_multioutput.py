...
from h2o4gpu.datasets import fetch_mldata
...
from h2o4gpu.multioutput import ClassifierChain
...
def test_multiclass_multioutput_estimator_predict_proba():
    ...
    clf = MultiOutputClassifier(LogisticRegressionSklearn(random_state=seed))
...
def test_classifier_chain_fit_and_predict_with_sparse_data_and_cv():
    ...
    X, Y = generate_multilabel_dataset_with_correlations()
    X_sparse = sp.csr_matrix(X)
    classifier_chain = ClassifierChain(LogisticRegressionSklearn(), cv=3)
    classifier_chain.fit(X_sparse, Y)
...
def test_classifier_chain_random_order():
    ...
    X, Y = generate_multilabel_dataset_with_correlations()
    classifier_chain_random = ClassifierChain(LogisticRegressionSklearn(),
                                              order='random',
                                              random_state=42)
...
def test_classifier_chain_crossval_fit_and_predict():
    ...
    # performance
    X, Y = generate_multilabel_dataset_with_correlations()
    classifier_chain_cv = ClassifierChain(LogisticRegressionSklearn(), cv=3)
