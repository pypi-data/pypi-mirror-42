...
class Kernel:
    ...
    def _processCondition(self, elem, sessionID):
        ...
        if 'name' in attr and 'value' in attr:
            ...
            if val == attr['value']:
                ...
                return response
        elif 'name' in attr and 'contains' in attr:
            val = self.getPredicate(attr['name'], sessionID)
            if len(attr['contains'].split(',')) > 0:
                for _li in attr['contains'].split(','):
                    if _li in val:
                        for e in elem[2:]:
                            response += self._processElement(e,sessionID)
                        return response

            if attr['contains'] in val:
                for e in elem[2:]:
                    response += self._processElement(e,sessionID)
                return response
        else:
                ...
                for li in listitems:
                    try:
                        ...
                        if not 'contains' in liAttr:
                            ...
                            liValue = liAttr['contains']
                            try:
                                liValueArr = liValue.split(',')
                                for _li in liValueArr:
                                    if _li.strip() in self.getPredicate(liName, sessionID):
                                        foundMatch = True
                                        response += self._processElement(li,sessionID)
                                        break
                            except:
                                liValue = liValue
                                if liValue in self.getPredicate(liName, sessionID):
                                    foundMatch = True
                                    response += self._processElement(li,sessionID)
                                    break
                    except:
    ...
    # <acumulador>
    def _processAcum(self, elem, sessionID):
        """Process a <acumulador> AIML element.
        ...
        for e in elem[2:]:
            value += self._processElement(e, sessionID)
        self.setPredicate(elem[1]['name'], str(int(value) + 1), sessionID)
        return value


    # <suma>
    def _processSuma(self, elem, sessionID):
        """Process a <suma> AIML element.
        ...
        cleanning = value.split('+')
        add = int(cleanning[0].strip()) + int(cleanning[1].strip())
