...
def tst():
    print(teste.get_value("rrf"))


def tst2():
    teste.set_value("rrf", ["sdf"])
...
if __name__ == "__main__":
    global teste
    teste = LineIntf("Teste")
    teste.new_title()
    teste.new_line()
    teste.new_entry()
    lst = teste.new_list(new_values=["df", "sdf"], new_name="rrf", with_label="teste")
    teste.new_ruler()
    teste.new_button(new_command=lambda: tst())
    teste.new_button(new_command=lambda: tst2())
    teste.open()
