...
class ImageTransform:
    ...
    """
    def __init__(self,name,suffix=0,prefix='',output_width=1024,output_height=704,r=1.,
                 c=1.,d=0.,blur=False,data=None,shift=0,smoothshift=0.):
    ...
    def _barycentre(self,indices):
            iu,iv=np.nan_to_num(indices.real),np.nan_to_num(indices.imag)
            if self.blur:
    ...
    def _trouveCouleurs(self,infini=True,liste=[[0,0]],MaxX=np.NaN,
                        ...
                        fade_color=[255]*3,fade_pos=0.,MinY=np.nan, pipe=False,
                        grid=None):
    ...
    def transform(self, infinite=True, lst=[[0,0]], MaxX=np.NaN, color=[255]*3,
        ...
        Couleurs=self._trouveCouleurs(infini=infinite,liste=lst,color=color,
                                      ...
                                      MinY=MinY, pipe=pipe,grid=grid)
        if verbose:print("Calcul :",time.time()-start)
        if print_and_save and not pipe:
...
#im=ImageTransform('pi9.png',1,data=None,
#                   r=0.01,c=1.,d=0.-0.j,
#                   output_height=900,output_width=1600,
#                   blur=False)
#im.mirror(X=2,Y=2,nbpix=0,color=[255]*3)
##im.video("power(exponent=i/99+1,c=-1j/2,d=1j+1)",100,"princiaux2.gif",pause=2)#,gif=False)
##im.power(-1,d=1j,c=2j)
#im.sin()
#im.transform()

# Standard transformation :
#T=ImageTransform("mouette5.jpg",0
#                ,r=1.*(1.+0.j),c=0.013*(0.25-0.9j)
#                ,d=-0.-0.85j,output_width=1800
#                ,output_height=3000,blur=False,smoothshift=-0,shift=0.)
#T.exp(Q=0,N=2,P=3)
#T.transform(infinite=False,lst=[[1,0],[0,1],[0,0]],color=[255,100,100])
#,
# Video from a picture : 
#T=ImageTransform("Sentinel.jpg",0
#                ,r=1.*(1.-0.j),c=1.*(1.-0.j)
...
#T=ImageTransform("Sentinel.jpg","out"
...
#msk=Mask((1600,1600))
#msk.ellipse([800,800],[[4,4],3],1450)
#msk.parallelogram([400,500],[[100,200],[-100,100]],50)
#image=msk.asimage()
#plt.figure()
#plt.imshow(image)
#T=ImageTransform("mask.jpg","out",data=image
#                ,c=0.2*(1.-0.3j),r=1.*(1.-0.j)
#                ,d=0.7-0.6j,output_width=2400
#                ,output_height=1600,blur=True)
#T.exp(N=11,P=13)
#mask=T.transform(infinite=True,print_and_save=False)
#plt.figure()
#plt.imshow(mask)
#plt.figure()
#im1=ImageTransform("libellule.jpg","out"
#                ,c=2.*(1.-0.j),r=1.*(1.-0.j)
#                ,d=-0.2-0.1j,output_width=2400
#                ,output_height=1600,blur=True).transform(print_and_save=False)
#im1=np.array(im1,dtype=np.uint16)
#it=ImageTransform("servieres.jpg","out"
#                ,c=0.15*(1.-0.j),r=1.*(1.-0.j)
#                ,d=-0.6+0.75j,output_width=2400
#                ,output_height=1600,blur=True)
#it.mirror(X=2,Y=2)
#it.sin(P=2,N=1)
#im2=it.transform(print_and_save=False)
#plt.imshow(im2)
#plt.figure()
#im2=np.array(im2,dtype=np.uint16)
#res=np.array(((im1*mask)+(im2*~mask))//256,dtype=np.uint8)
#sgn=plt.imread('signature.png')
#plt.imshow(res)
#plt.imsave("servieres-out2.jpg",res)
