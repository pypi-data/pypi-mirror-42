...
class HyperbolicTiling:
    ...
    """
    def __init__(self,name,N,P,prefix='',suffix=0,output_width=900,
    ...
    def getVertices(self,*symetries):
        n=len(symetries)
...
#        c=_translation(th,r)(np.copy(c))
    ...
    def tiles(self,c=1.,d=0.,backcolor=[0,0,0],vanishes=False,print_and_save=True,
              coloris=np.pi/5,nbit=5,delta=0.005,sommets=None,pipe=False,grid=None):
        im2=self.data
        a1=self.a1
        a2=self.a2
        N=self.N
        ...
        im=self._couleur(z,LPu,LTr,im2,nbit,cc=c,dX=d.real,dY=d.imag,
                         delta=delta*(1-abs(zero)**2),pipe=pipe)
        if pipe:
            return z
        if vanishes:
            for k in range(3):
